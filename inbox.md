# Team runtime semantics foundation retry
...
Scope: retry original request 5 and prerequisite pieces for request 1 after correcting the Aevatar frontend-loop protected-path policy. This task replaces the stale 0001-team-runtime-semantics attempt that escalated because frontend page runtime helpers were incorrectly treated as backend/runtime infrastructure.
...
Required work:
- Add a small shared Team frontend runtime semantics module. It may live under apps/aevatar-console-web/src/pages/teams/runtime/** because frontend page runtime helpers are allowed by the current Host protected path policy.
