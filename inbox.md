# Team cleanup and tabs accessibility policy-v2
...
Scope: retry original requests 4 and 8 after correcting FKST policy gates. This replaces failed task 0007, which reached 3/3 approach consensus but failed before implementation under old host policy handling.
...
- Update `TeamDetailChrome.tsx` tabs to use proper tablist semantics: `role="tab"`, `aria-selected`, stable ids/aria-controls where safely wired to an existing panel, and keyboard navigation for ArrowLeft/ArrowRight/Home/End with roving focus.
- Add focused tests for tab accessibility attributes and keyboard behavior.
