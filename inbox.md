# Team runtime run semantics consolidation
...
Rationalized source requirements covered: original item 5.

Current repo facts:
- Duplicated run ordering/status helpers exist around `apps/aevatar-console-web/src/pages/teams/home.tsx` and `apps/aevatar-console-web/src/pages/teams/workflowOperationalUnits.ts`.
- Runtime lens files live under `apps/aevatar-console-web/src/pages/teams/runtime/`.

Goal:
Create one shared frontend helper for Team run semantics so home, workflow operational units, and runtime lens code cannot drift on run ordering or status meaning.

Expected implementation:
- Add or reuse a focused helper such as `apps/aevatar-console-web/src/pages/teams/runtime/runtimeRunSemantics.ts` or another locally consistent Team runtime module.
- Centralize `compareRuns`, successful/failed run checks, status normalization, and status label helpers only where duplicates actually exist.
- Replace duplicated logic in `home.tsx`, `workflowOperationalUnits.ts`, and any runtime lens file that independently interprets the same run status semantics.
- Preserve existing public UI wording unless the current duplicate code already creates inconsistent labels; if changing wording, explain it in the FKST verdict.

Acceptance criteria:
- Home and workflow operational unit selection use the same run ordering and success/failure definitions.
- Focused tests cover the shared helper and at least one caller path.
- Existing Team tests still pass or any unrelated failures are recorded with evidence.
