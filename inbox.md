# Team detail route extraction policy-v2
...
Scope: retry original requests 2, 3, and 6 after correcting FKST policy gates. This replaces failed task 0006, which reached route/topology approach work but failed under old host policy handling.

Goal:
Reduce Team detail page risk with a staged, low-risk extraction. Use current repository facts: if Team Detail topology production code is absent after the required sync, do not invent topology abstractions; implement the route-state hook and only extract current-production DTO/view-model helpers when there is a real caller.
...
- Reuse existing shared navigation helpers such as `readTeamDetailRouteState` and `buildTeamDetailHref`; do not create a second route parser.
...
- Extract one small pure detail DTO/view-model helper only if it has a current production caller and reduces risk without a broad `detail.tsx` rewrite.
...
- Do not perform the data-loading/view-model follow-up from the separate detail data/view-model task.
...
Implementation note:
- Preserve the successful parts of failed 0006 approach framing unless current files have materially changed.
