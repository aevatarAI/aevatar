# Team detail data and view-model extraction policy-v2
...
Scope: staged follow-up for original request 2, excluding request 7 style tokenization and avoiding overlap with the active route-state extraction task. This replaces stale/failed task 0008 generated before the policy gate fix.

Goal:
Reduce `apps/aevatar-console-web/src/pages/teams/detail.tsx` risk by extracting current-production Team Detail data loading and pure tab DTO/view-model derivation into small testable frontend modules, after or alongside route-state hook work without duplicating it.
