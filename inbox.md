# Team cleanup and tabs accessibility retry
...
Scope: retry original requests 4 and 8 after correcting the Aevatar frontend-loop host policy. This task replaces stale 0004-team-cleanup-a11y, which reached consensus but failed before implementation under the old Host protected path policy.

Goal:
Confirm unused old Team home implementations are absent or unreferenced, and make Team detail tabs accessible.
...
- LegacyTeamsHome.tsx and TeamsHomeRosterV0.tsx appeared absent/unreferenced in the current tree, but implementation must re-check with current files after the required sync.
- TeamDetailChrome.tsx has role="tablist" but tab buttons use aria-current rather than role="tab" / aria-selected and do not implement roving keyboard focus.

Required work:
- Confirm LegacyTeamsHome.tsx and TeamsHomeRosterV0.tsx have no hidden references, dynamic imports, route configs, or test dependencies. Delete only if they exist and are unreferenced; if already absent, do not create archive/doc churn.
- Update TeamDetailChrome tabs to use proper tablist semantics: role="tab", aria-selected, stable ids/aria-controls where safely wired to an existing panel, and keyboard navigation for ArrowLeft/ArrowRight/Home/End with roving focus.
