...
Both delivery URLs are deployment configuration. The Mainnet deployment mounts `appsettings.json` and `appsettings.Distributed.json` from cluster ConfigMaps over the copies baked into the image, so the values committed in this repository are the local and reference defaults — they do not configure production. `Aevatar:Delivery:ConsoleBaseUrl` and `Aevatar:Delivery:ConsoleWebBaseUrl` must be present in the deployed ConfigMap. Without the first, NyxID connect-link creation fails closed with `503 DELIVERY_CALLBACK_BASE_URL_UNAVAILABLE` in any non-loopback environment; without the second, delivery responses carry no console link.
