...
Only `service.connect` currently has the complete registration set, so v4 is `Ready`. Revisions v5, v6, and v7 are `Partial`: `service.reauthorize`, `key.create`, and `key.rotate` remain fail closed at request admission and producer resolution. In particular, the two key actions are pinned but are not executable because exact key-mutation authority resolution and authority-bound retry/generation registrations do not yet exist. No key authority or retry capability is inferred from parsers, AG-UI mapping, safe resource references, postcondition readers, or the registry payload itself. Catalog and custom connection remain distinct typed variants; a boolean such as `custom: true` never changes the meaning of one shared field set.
