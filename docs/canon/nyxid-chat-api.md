...
The typed registry recognizes closed action schemas, but executable v1 handoff is narrower: an action must also have an Aevatar producer, wire mapper, and typed postcondition reader. Revisions v4, v5, and v6 execute only `service.connect`. In v6 the typed key-create parser rejects missing, empty, over-limit, or duplicate service identities, while actor admission continues to reject every `key.create` request until its producer and AG-UI wire mapper exist. The existing postcondition reader also remains fail closed: a future completion cannot become verified until an owner-scoped exact-key read proves the safe key ID, `proxy`-only scope, the exact requested nonempty service set, `allow_all_services=false`, no node IDs, `allow_all_nodes=false`, and a creation timestamp no earlier than the committed action request. `service.reauthorize`, `key.create`, and `key.rotate` therefore remain fail-closed at the executable gate. In particular, a browser report or a permitted service access value is not effect proof. Catalog and custom connection are distinct variants; a boolean such as `custom: true` never changes the meaning of one shared field set.
