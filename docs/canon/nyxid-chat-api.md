...
Conversation resource creation and deletion use a separate closed, Actor-owned
Protobuf lifecycle. Creation commits `registration_pending` before registry I/O
and reaches `active` only after admission is visible. A failed local creation
commits compensation before unregistering and, when required, advances through
final actor cleanup. Creation compensation and deletion both commit the exact
`registryActorId` and stable `registryUnregistrationOperationId` before using the
narrow typed registry-unregistration port. The request's target actor and
completion actor are the same lifecycle owner. `AcceptedForDispatch` is not
removal proof: the lifecycle stays pending until
`GAgentRegistryUnregistrationCompleted` matches the committed operation,
registry Actor, scope, kind, target, completion Actor, and the inbound envelope
publisher. The per-scope registry Actor commits either removal or authoritative
absence before emitting that completion from its own Actor identity. Exact
duplicates redeliver the immutable completion. Its ordered Protobuf ledger is
bounded to 256 operations; completion dispatches admitted to the target inbox
are evictable oldest-first, while rejected dispatches remain pinned for exact
redrive. If every retained callback is pending, new operations fail before
commit instead of evicting active recovery authority.

After exact registry completion, deletion commits a stable
`historyDeletionOperationId` before dispatching one typed
`ChatHistoryConversationDeletionRequest`. The lifecycle adapter addresses both
deterministic history owners directly: the collision-resistant canonical Actor
and the historical legacy-address Actor. It never uses a read-model miss as an
absence proof. A pristine canonical Actor commits an idempotent tombstone. A
matching live legacy Actor commits its tombstone; a pristine or tuple-colliding
legacy Actor commits an owner-scoped authoritative-absence acknowledgement
without mutating another conversation. Each owner emits
`ChatHistoryConversationDeletionCommitted` from its actual Actor identity, and
the controller requires payload owner identity to match the inbound envelope
publisher. Cleanup requires both owner acknowledgements; canonical absence is
not accepted. The history port's `Accepted` result still means inbox admission
only. Query-oriented public deletion may return `NotFound`, but that weak replica
result cannot complete the lifecycle. Exceptions, non-accepted results, and
ambiguous dispatches stay pending and are activation-redrivable with the same
operation IDs. Every outstanding phase advances only through exact typed
completion or a self-message whose lifecycle snapshot still equals committed
state. If both owner acknowledgements were committed before aggregate history
completion was interrupted, activation commits that aggregate completion
without issuing another owner deletion command. Final cleanup uses the
runtime-neutral Actor lifecycle port only after both history owners have
acknowledged and the cleanup-dispatching waterline is committed.

```mermaid
%%{init: {"maxTextSize": 100000, "flowchart": {"useMaxWidth": false, "nodeSpacing": 10, "rankSpacing": 50}, "themeVariables": {"fontSize": "10px"}}}%%
sequenceDiagram
    participant C as "Conversation controller Actor"
    participant G as "Per-scope Registry Actor"
    participant HC as "Canonical history Actor"
    participant HL as "Legacy history Actor"

    C->>C: "Commit registry operation tuple"
    C->>G: "Typed unregistration request"
    G->>G: "Commit removal or authoritative absence"
    G-->>C: "Owner-sourced typed completion"
    C->>C: "Commit history operation tuple"
    C->>HC: "Typed deletion request"
    C->>HL: "Same typed deletion request"
    HC->>HC: "Commit tombstone or exact replay acknowledgement"
    HL->>HL: "Commit tombstone or owner-scoped absence"
    HC-->>C: "Canonical owner completion"
    HL-->>C: "Legacy owner completion"
    C->>C: "Commit cleanup only after both"
```
...
| `registryUnregistrationOperationId` | Conversation actor, one compensation or deletion unregister | Correlates dispatch with the Registry Actor's committed completion and remains stable across activation redrives. |
| `registryActorId` | Deterministic per-scope Registry Actor | Authoritative source and required envelope publisher for one registry unregistration completion. |
| `historyDeletionOperationId` | Conversation actor, one deletion lifecycle | Correlates history deletion admission with its committed completion and remains stable across activation redrives. |
| Profile `attempt` | Conversation actor, one profiled turn authority attempt | Fences one transient exact catalog across successor steps; it is distinct from each step's `operationGeneration`. |
...
`clientRequestId` may be supplied in the body or `Idempotency-Key`; the body wins. The server derives a stable actor-scoped `turnId` when a key is present and creates a fresh `turnId` otherwise. The deprecated `sessionId` field is ignored.

The controller commits a SHA-256 admission digest over deterministic Protobuf
bytes containing the prompt, sanitized durable input parts, per-part hashes of
valid decoded inline media bytes, and typed non-secret execution controls. Raw
inline bytes never enter Actor state or events. Stable model routing, caller authority, channel
identity, sender binding, workflow/schedule context, tool visibility, and
NyxID authority are request semantics. Command/correlation tracing, access
tokens, credential or delivery references, `external_metadata`, and
connected-services JSON are not. Canonical direct-chat caller/channel identity
values are derived from the stable scope and turn, so a transport retry may use
new command/correlation IDs or refreshed credentials without changing the
digest.

Reuse of either the committed `turnId` or `clientRequestId` is a same-turn
admission collision. Replay requires exact actor, scope, turn, task, and client
identity plus a fixed-time digest match. A mismatch commits a typed
`NYXID_CHAT_TURN_IDEMPOTENCY_CONFLICT` and dispatches no model or tool work. An
exact active or terminal retry commits `NyxIdChatTurnReplayCommittedEvent` with
the already committed safe turn/task snapshots. Terminal admissions remain in
an Actor-owned `NyxIdChatTurnReplayRecord` ledger keyed by both `turnId` and
`clientRequestId`, even after later turns replace `activeTurn`. The ledger is
bounded to 32 records and evicts oldest-first deterministically. Records carry
the admission digest, terminal identities/status/timestamps, and a safe task
snapshot; prompt bodies, input parts, inline data, credentials, headers, and
transport bags are absent. Current and retained identities are resolved before
active-turn steering rejection, so an exact old retry replays while a collision
on either identity fails closed without work. Monotonic late operation evidence
refreshes the matching terminal record atomically in its existing list position,
so FIFO eviction order is unchanged and later reconstruction/replay exposes the
latest safe effect and operation facts. The existing Projection Pipeline maps
the replay fact to current task frames and, when terminal, the same terminal
AGUI result; it never redispatches execution, primes a projection, or synthesizes
completion in the endpoint.
...
The enabled startup fetch has a 1 MiB raw-response limit. It retains the
`Content-Length` precheck and also counts every streamed response byte before
decoding, so chunked or unknown-length multibyte content cannot evade the
limit. A bounded payload is decoded only with strict UTF-8; malformed byte
sequences fail startup rather than being replacement-decoded.
