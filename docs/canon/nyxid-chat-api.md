...
Aevatar snapshots `GET /api/v1/assistant/actions` once at startup and accepts schema version `4` with registry revision `nyxid-assistant-actions.v4`, `nyxid-assistant-actions.v5`, `nyxid-assistant-actions.v6`, or `nyxid-assistant-actions.v7`. Each revision has an immutable descriptor set. Every pinned descriptor must be present exactly once; a missing, duplicate, or fingerprint-mismatched pinned descriptor rejects the registry. A non-pinned descriptor is tolerated and ignored, so a future or revision-foreign action cannot become executable merely because it appears in the payload.

This startup dependency is active only when `Aevatar:NyxId:AssistantActions:Enabled=true`. The reusable NyxIdChat composition default is `false`: a host that does not opt in does not call the registry endpoint and injects an immutable registry with no external registry-backed executable actions, so those browser-action requests fail closed with `NYXID_ACTION_UNSUPPORTED` without preventing unrelated capabilities from starting. Mainnet explicitly enables assistant actions and fetches the registry from the public `Aevatar:NyxId:ApiBaseUrl`, never from `InternalApiBaseUrl`. A fetch, timeout, read, JSON, schema, or revision failure initializes the same immutable disabled registry and emits a scrubbed error, allowing ordinary chat and the Host to start. Host cancellation still aborts startup. The registry remains a one-shot process snapshot, so a degraded process does not enable external registry-backed browser actions until its next successful start.

`service.access_review` is a separate server-authored built-in action under revision `aevatar-nyxid-actions.v1`; it is not loaded from the external registry and is not accepted through the generic caller-supplied action parser. Aevatar can create it only from an exact `USER_SERVICE_ACCESS_REQUIRED` tool receipt carrying one UserService identity, service slug, and safe HTTPS resource URI. Its completion still requires owner-matched transient `ToolContext` and exact provider read-back before the action is verified.

Descriptor fingerprints are SHA-256 over canonical recursively key-sorted JSON containing only `action`, `params_schema`, `remember_eligible`, `risk`, and `tier`. The pinned snapshots are:

| Revision | Pinned descriptor fingerprints |
|---|---|
| v4 | `service.connect=20a4dc5fe13a30882a1f84085ace2d04a93081829ecb31e3c6a0f2bff94ec0a3` |
| v5 | `service.connect=20a4dc5fe13a30882a1f84085ace2d04a93081829ecb31e3c6a0f2bff94ec0a3`; `service.reauthorize=b6a16985e083b1fa71ab99f0fcede9ae69415d9e71f7e078789bcaeadb8ff0b8`; relaxed `key.create=d5db2d5b1e34db1b8c727271f745c47c575947f027da9685bb76096f545c7975`; `key.rotate=e65c6d81a00bf980ad3ac63bb44f6cbe901da73f6d825a4545aacf0108cc4643` |
| v6 | `service.connect=20a4dc5fe13a30882a1f84085ace2d04a93081829ecb31e3c6a0f2bff94ec0a3`; least-scope `key.create=ce94e23543aad2417260f25a07eac15369c007d14d77963daaed7b5730e98e07` |
| v7 | `service.connect=20a4dc5fe13a30882a1f84085ace2d04a93081829ecb31e3c6a0f2bff94ec0a3`; least-scope `key.create=ce94e23543aad2417260f25a07eac15369c007d14d77963daaed7b5730e98e07`; `key.rotate=e65c6d81a00bf980ad3ac63bb44f6cbe901da73f6d825a4545aacf0108cc4643` |

The v6/v7 least-scope `key.create` schema requires 1 to 64 unique exact `allowedServiceIds`; v7 `key.rotate` carries one exact `keyId`. The registry's `risk` and `remember_eligible` values remain registry-owned advisory inputs to Aevatar presentation/planning. The caller cannot submit or lower them, and NyxID recomputes and enforces authorization at execution time.

Executability is derived rather than declared in a handwritten revision table. A pinned action is executable only when its revision snapshot and semantic contract agree and compatible typed registrations exist for all eight capabilities: request producer, admission parser, AG-UI mapper, safe-resource predicate, postcondition verifier, evidence predicate, owner-bound execution-context resolver, and retry/generation policy. The compatibility checks include the exact wire action, pinned `params_schema_variant`, protobuf params cases, completed resource case, evidence strategy, transient execution-context requirement, retry strategy, and replay policy.

Revisions v4, v6, and v7 are `Ready` for their pinned descriptor sets. Revision v4 executes `service.connect`; v6 executes `service.connect` and least-scope `key.create`; v7 additionally executes `key.rotate`. Revision v5 remains `Partial`: `service.connect` is executable, while `service.reauthorize`, relaxed-schema `key.create`, and `key.rotate` remain fail closed because their revision-specific compatible registration sets are incomplete. The v6/v7 key actions are executable only because owner-bound transient `ToolContext` resolution and transient-context generation renewal are registered for those revisions; no execution credential or retry policy is inferred from parsers, AG-UI mapping, safe resource references, postcondition readers, or the registry payload itself. Catalog and custom connection remain distinct typed variants; a boolean such as `custom: true` never changes the meaning of one shared field set.

`Aevatar:NyxId:AssistantActions:Enabled` controls whether the host fetches the startup registry. When it is `false`, the host publishes `Disabled` readiness and an immutable registry with no executable actions. When it is `true`, `Aevatar:NyxId:AssistantActions:Required` controls startup failure semantics. With `Required=false` (the reusable composition default), fetch or validation failure publishes `Unavailable` readiness plus an empty fail-closed registry and allows unrelated host capabilities to start. Generic availability failures use `NYXID_ACTION_REGISTRY_UNAVAILABLE`; registry validation failures preserve their exact registry error code. With `Required=true`, the same unavailable snapshot is published and the original failure is rethrown so host startup stops. A successful snapshot publishes `Ready` only when every pinned action is executable; otherwise it publishes `Partial` with each action's exact missing capability set.

Mainnet supplies `AssistantActions:Enabled=true` as its production default but does not override `Required`; the default remains `false`. A registry fetch or validation failure therefore disables Assistant actions for that process while ordinary chat and the Host remain available. `Required=true` is an explicit host opt-in for deployments that intentionally prefer startup failure over this fail-closed capability degradation.
...
For an action whose semantic contract requires owner-scoped provider read-back, the authenticated Host maps the current request bearer into request-transient `AgentToolExecutionContextPayload.ToolContext`. The conversation actor passes that context only on the current dispatch after matching its caller scope and owner to the action owner. No bearer, credential reference, replay token, expiry fact, or revocation fact enters actor state, continuation admission, committed events, read models, projection, AGUI, history, generic metadata, or logs. Missing or mismatched transient context fails closed. An exact replay may dispatch only with the `ToolContext` supplied by that replay request; activation recovery is credential-free and never reconstructs or renews credentials from durable state.
...
Report admission is atomic at the actor boundary, while each accepted report's postcondition is reconciled independently. Any duplicate normalized `actionRequestId` within one batch is rejected, even when the reports are otherwise identical. Replaying the same `clientRequestId` is idempotent only when the complete normalized report set matches the committed admission; changed content conflicts. A different `clientRequestId` is adjudicated from actor-owned pending and recent action state: an all-fresh batch may be accepted, an all-persisted equal batch is actor-idempotent, persisted conflicts are rejected, and a mixed persisted/fresh batch fails closed as a conflict. Cross-scope, cross-conversation, cross-origin, unknown-action, and wrong-resource reports fail closed.

### Verified action continuation

A verified browser action does not resume the original service request through assistant prose. After the exact typed postcondition commits `done / confirmed`, the conversation actor may dispatch a credential-free `NyxIdChatVerifiedAuthorizationContinuation`. Despite the historical type name, this payload represents any supported verified NyxID browser action, including `service.connect`, `service.access_review`, `key.create`, and `key.rotate`. The closed protobuf payload carries only `action_request_id`, `origin_turn_id`, optional `source_tool_step_id`, `postcondition_step_id`, the typed `action`, the action-specific safe verified resource reference, the frozen `service_slug` when the action has one, `verified_at`, and one typed `resume_requirement`. It has no token, credential, secret, or generic metadata field. The durable LLM step source retains only the exact action request identity and frozen resume requirement needed for replay-safe lifecycle decisions.
...
The actor freezes one of two obligations. `COMPLETE_ORIGINAL_SERVICE_REQUEST` applies when authorization interrupted an ordinary connected-service request. It requires a valid typed `UserService` resource and non-empty frozen `ServiceSlug`; an inconsistent resource or missing slug fails before LLM execution with `NYXID_AUTHORIZATION_CONTINUATION_INVALID`. The resumed LLM request then rematerializes the current request-local catalog and exposes only operations whose typed admission matches both identities. An unprofiled turn discovers the current NyxID Chat route toolset; a profiled turn requires the committed profile identity to be present and exact, then applies the current `MaximumToolPolicy` as its upper bound. The OAuth-before turn/task authority ceiling is not reused because it cannot contain a capability that the verified action has only just established. A missing admitted operation, route drift, policy exclusion, or missing committed profile capability fails before LLM execution with `NYXID_AUTHORIZATION_CONTINUATION_CAPABILITY_UNAVAILABLE`; unrelated route tools and global tools are not fallback authority. `COMMUNICATE_AUTHORIZATION_COMPLETION` applies both to a dedicated authorization request and to a verified non-service action such as `key.create` or `key.rotate`. It requires a valid typed `action`, a valid action-specific typed resource, and an exact action/resource match. The executor forces the request-local catalog empty, clears any transient tool authorization, bypasses model generation, and returns server-authored terminal text containing only the safe resource identity, such as `KeyId` or `UserServiceId`. An absent or mismatched action/resource fails before generation with `NYXID_AUTHORIZATION_CONTINUATION_INVALID`; an unrecognized resume requirement fails with `NYXID_AUTHORIZATION_CONTINUATION_REQUIREMENT_UNSUPPORTED`.
...
For `COMPLETE_ORIGINAL_SERVICE_REQUEST`, the executor appends the server-authored continuation instruction only to the request-local `AgentRunReplyStepState.Messages` after rebuilding the transient session. It is excluded from pending/appended history, actor state, committed events, read models, projections, generic metadata, and logs. `COMMUNICATE_AUTHORIZATION_COMPLETION` does not append an instruction or invoke the LLM. Runtime credentials are resolved separately for requests that need them and are excluded from the typed continuation and every durable or observable artifact.
...
If a pending postcondition is already terminal failed/unverified, a new authenticated state-change wake is a semantic retry rather than activation recovery. The actor creates operation generation `N+1` with a generation-derived operation ID, clears the prior generation's terminal/evidence fields, uses only the fresh transient `ToolContext` supplied by that wake when provider read-back requires it, and keeps the original browser action request identity. For `key.create` and `key.rotate`, the pending action must already contain a completed report with the exact key resource hint; a signal-only wake never invents a key identity. A late result from a superseded generation is fenced and cannot complete the pending action. An exact replay of the accepted wake remains idempotent within the resulting generation.
...
- an exact browser-action postcondition still in `Requested`, `Dispatched`, or `Running` may be redispatched under the same operation key and generation because it is the same typed read-model query; the recovery command is credential-free, never reconstructs `ToolContext`, and therefore fails closed if that exact provider read requires a fresh bearer; activation never creates a successor generation for terminal work;
- a terminal failed/unverified browser-action postcondition remains waiting after restart and advances only through a new authenticated state-change wake, which creates the next operation generation as described above;
