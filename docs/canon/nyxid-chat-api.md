...
Mainnet explicitly sets both `AssistantActions:Enabled=true` and `AssistantActions:Required=true`, and its host composition supplies those values as production defaults when absent. Mainnet therefore fails startup when the assistant-action registry cannot be fetched or validated instead of serving with silently unavailable browser actions.
...
For an action whose semantic contract requires owner-scoped provider read-back, the authenticated Host exchanges the request bearer for a TTL-bound `NyxIdReadAuthorityRef`. The raw bearer exists only in the secret vault. Actor-owned committed state may retain only the opaque purpose-, scope-, owner-, version-, and request-bound reference; the executor resolves it only into request-local tool context, and public current-state projection and AGUI never expose the reference. Missing, mismatched, revoked, expired, or unavailable authority fails closed. When the same deterministic continuation identity is replayed after authority expiry, the durable fence returns only the original opaque replay-only reference: it does not read the bearer, extend the TTL, or issue replacement authority. Such a reference can preserve an exact terminal replay that performs no dispatch, but it cannot authorize in-flight postcondition redispatch. Revocation is recorded by a durable tombstone before bearer deletion, so the same request identity cannot be reissued after vault deletion semantics or replay-window expiry.
...
If a pending postcondition is already terminal failed/unverified, a new authenticated state-change wake is a semantic retry rather than activation recovery. The actor creates operation generation `N+1` with a generation-derived operation ID, clears the prior generation's terminal/evidence fields, binds the authority accepted for the new wake when the action requires one, and keeps the original browser action request identity. A late result from a superseded generation is fenced and cannot complete the pending action. An exact replay of the accepted wake remains idempotent within the resulting generation.
...
- an exact browser-action postcondition still in `Requested`, `Dispatched`, or `Running` may be redispatched under the same operation key and generation because it is the same typed read-model query; activation never creates a successor generation for terminal work;
