...
8. Proof-gated observation 在 live proof 尚不可见时，必须把 exact failed envelope 持久化为 actor-owned durable continuation；持久化成功后 transport delivery 才可 ACK，随后以 `5-6s -> 10-12s -> 20-24s -> 24-30s cap` 的有界指数退避区间重投直到 gate 可见。区间内 delay 由 target actor + callback identity + attempt 计算稳定 jitter，保证同一 durable retry 在 redelivery/restart 后不漂移，同时避免 backlog 同拍唤醒。该 continuation 必须携带权威 source actor id 与 committed state version 组成的 typed coalescing cursor；同一 materializer/source gate 只保留最高 version 的 pending envelope 和持久化 high watermark：新 version 原子覆盖旧 pending，已被覆盖的旧 version 直接确认且不得再写 callback，同 version 在 pending 时复用、在 callback 已触发后允许重排。普通 retry budget 不得把这类等待降级为长期占住 broker offset 的 transport redelivery；只有 durable scheduler 状态写入或 reminder 接受失败时才保留原 delivery 未确认并传播原 handler failure。
