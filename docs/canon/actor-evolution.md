...
8. Proof-gated observation 在 live proof 尚不可见时，必须把 exact failed envelope 持久化为 actor-owned durable continuation；持久化成功后 transport delivery 才可 ACK，随后以 `5-6s -> 10-12s -> 20-24s -> 24-30s cap` 的有界指数退避区间重投直到 gate 可见。区间内 delay 由 target actor + callback identity + attempt 计算稳定 jitter，保证同一 durable retry 在 redelivery/restart 后不漂移，同时避免 backlog 同拍唤醒。该 continuation 的 typed coalescing cursor 必须包含权威 source actor id、committed state version、committed event identity、基于 deterministic Protobuf serialization 的 `state_event + state_root` fingerprint 与同版本 maintenance precedence；同 version/precedence 只有 fingerprint 完全相同才是 exact duplicate，同 event id 但 committed bytes 不同也必须冲突并 fail closed，只有 typed maintenance republish precedence 可以单调覆盖 ordinary publication。每个 materializer/source gate 只保留一个 pending envelope，以及持久化的 pending/completed watermark：更高 cursor 原子覆盖旧 pending，agent handler 完整成功后 runtime 必须用同一 typed cursor 删除 pending 并推进 completed watermark，旧 cursor 的延迟投递不得重新创建 callback；completion 持久化失败必须重新进入 durable coalesced retry。滚动升级读取旧的 sequence-only watermark 时，较低 version 继续被 fence，首个同 version 或更高 version 的 typed cursor 负责升级该 watermark。通用 schedule、cancel、timer API 不得修改 coalesced slot。普通 retry budget 不得把这类等待降级为长期占住 broker offset 的 transport redelivery；只有 durable scheduler 状态写入、completion 持久化或 reminder 接受失败时才保留原 delivery 未确认并传播原 handler failure。
9. 长期 retry 的日志不得随 attempt 线性放大：attempt 1 记录一次完整 exception stack；attempt 2–4 与后续 2 的幂次只记录无 stack warning；其余 attempt 降为 debug。该采样只影响日志，不改变 durable retry、watermark 或 ACK 语义。
