...
├── Aevatar.GAgents.Platform.Lark.Tests/       ← 继承 ChannelAdapterConformanceTests
...
| 新包 | 归属 slnf | 理由 |
|---|---|---|
| `Aevatar.GAgents.Channel.Abstractions` | `aevatar.foundation.slnf`（当前） | 当前先并入 Foundation 维持现有 split guard；后续若独立 transport slice，再拆到 `aevatar.channels.slnf` |
| `Aevatar.GAgents.Channel.Runtime` | `aevatar.foundation.slnf`（当前） | 包含 `ConversationGAgent` 和 projection 订阅；当前随 Foundation 构建，后续可迁到 `aevatar.channels.slnf` |
| `Aevatar.GAgents.Platform.{Lark,Telegram,Slack,Discord}` | `aevatar.foundation.slnf`（当前） | Lark 已先迁到 `platforms/` 轴，当前仍随 Foundation 构建；后续再拆 `aevatar.platforms.slnf` |
...
**后续 split 决策点**：
- 何时把当前并入 Foundation 的 channel/platform 项目拆成独立 `aevatar.channels.slnf` 与 `aevatar.platforms.slnf`
- `solution_split_guards.sh` 和 `solution_split_test_guards.sh` 在拆分时必须同步扩展 cover 新 slice
...
- **从现有 ChannelRuntime 迁移**：
  ...
  - **Group chat 支持（历史约束）**：legacy Lark callback runtime 已识别 `chat_type` 并按 `chat_id` 回发，但 actor 层仍是 sender-keyed；issue `#308` 已移除这条路径。若未来恢复 conversation runtime，核心仍是 `ConversationGAgent + CanonicalKey`，不能回退到 sender-keyed actor
