...
│  │                  ↑                                             │  │
│  │  ChannelReader<ChatActivity>  ← consumer-internal bounded    │  │
│  │                  ↑                bounded working buffer      │  │
│  │  Consumer reads from durable inbox ─────────┐                │  │
│  └──────────────────────────────────────────────┼───────────────┘  │
└────────────────────────────────────────────────┼───────────────────┘
                                                 │
                                 ┌───────────────┴──────────────┐
...
**关键流向澄清**（和 §9.5 对齐）：adapter 的 inbound 路径是 `verify → commit to durable inbox → ack`；**`ChannelReader<ChatActivity>` 不在 adapter 和 pipeline 之间**，它是 pipeline consumer 内部的 bounded 工作缓冲。durability 由 durable inbox + `processed_message_ids` 两条 state 承担。
...
### 4.1.1 Proactive command events — field schemas
...
public interface ICredentialProvider {
    ...
    // ConversationContinueFailedEvent(CREDENTIAL_RESOLUTION_FAILED)（见 §4.1.1）。
}
```
...
- **Reply path（`ITurnContext.SendAsync` / `ReplyAsync`）** — 有 inbound turn，adapter 从 `ctx.Activity.From` sender canonical id 隐式 resolve 到 `ChannelUserBindingGAgent` 拿 user token（如果业务需要 user-bound send）；否则用 bot token（来自 adapter `InitializeAsync` 注入的 `ChannelBotRegistrationEntry` credentials）。业务层无感，不需要显式传 principal。
...
**Runtime 侧**（lifecycle + inbound 观察）：
...
public interface IChannelTransport {
    ...
    Task InitializeAsync(ChannelBotRegistrationEntry registration, CancellationToken ct);
    Task StartReceivingAsync(CancellationToken ct);
    Task StopReceivingAsync(CancellationToken ct);

    // Pipeline consumer 订阅的 inbound 流。durability 不由这个 ChannelReader 承担，
    // 见 §9.5 durable inbox 契约——ChannelReader 仅作 pipeline consumer 内部
    // 的 bounded working buffer，不在 adapter 和 pipeline 之间。真正的 durable
    // 消费是 pipeline 直接从 durable inbox（Orleans Stream）读取。
...
**为什么不允许"独立两个类实现"**（Codex v5 HIGH）：init state（credentials from `ChannelBotRegistrationEntry`）、capabilities 声明、lifecycle 状态都需要在 transport side 和 outbound side 之间共享：
...
**为什么 `InboundStream` 是 `ChannelReader<T>`**：是一个 **in-process queue** 抽象，给 adapter 和下游 pipeline 之间解耦。诚实地说明限制：
- `ChannelReader` 本身**不提供 durable、multi-consumer fan-out、cross-process replay**
- **durable cut point 仍在 adapter 实现的 webhook handler**：Lark / Slack / Telegram 的 webhook 必须"同步 persist inbound 事件 → 200 → 异步入 channel"，而不是"收到就 200 + 入 channel 异步 persist"（否则 ack 早于 durable 会丢事件）。这部分实现约束不因 `ChannelReader` 的存在而改变
- 去重两段式保留：endpoint 层 TTL（防重放攻击）+ grain state 层 durable dedup（防漏处理）——见 §11 现状

这些 durable / exactly-once 的保证来自 **adapter 实现 + grain state**，不来自 `ChannelReader` 本身。`ChannelReader` 只是让 webhook handler 和 middleware pipeline 之间有一个背压友好的中间 buffer。
...
- 这三个方法**默认使用 adapter-init 时注入的 bot credential**（来自 `ChannelBotRegistrationEntry` 的 bot token），这是 "bot says X" 语义，不是 user-bound send
...
**Discord 3s interaction window 不放在 composer**：这是 **transport contract**（收到 interaction 3 秒内 HTTP 200，否则 Discord 重投或标为 failed）。由 `DiscordChannelAdapter` 的 HTTP handler 在 inbound 入 `InboundStream` 前立即 ack + defer；真正的 response 后续通过 interaction token 走 follow-up 发出。composer 只负责最终 payload 的 Block Kit / Embed。详见 §10.4。
...
**为什么 Proto 覆盖**：这些接口本身是 C# 契约，不需要 proto（proto 是数据契约）。接口参数里的 `ConversationReference` / `ParticipantRef` 已经在 §4.1 proto 列表里。`IChannelMentionAdapter.StripMentions` 返回 string，不引入新 proto 类型。
...
public interface ITurnContext {
    ...
    ChannelBotRegistrationEntry Bot { get; }         // bot instance metadata（非 credential）
...
- `Stop` MUST 同步完成 in-flight outbound + close transport，不保证 `InboundStream` drain 到 empty

#### `InboundStream : ChannelReader<ChatActivity>`

- **Single reader**：只能由 middleware pipeline 消费一次；多 reader 消费同一 stream 行为未定义，adapter **不做 fan-out**（需要 fan-out 的消费场景在 pipeline 之后再分流）
- **Bounded capacity**：默认 1000 events，per-adapter config。`Channel<T>` 配 `FullMode = Wait` —— producer 满了阻塞（webhook handler 慢响应会反压到 HTTP 层，或 gateway supervisor 反压到 WebSocket 消费），不静默 drop
- **Producer 满时 timeout**：producer 写入 `ChannelWriter` 超过 500ms 必须升级成错误信号（metric + alert），不能无限等——避免 inbound buffer 饱和时整个 adapter 静默 stuck
- **`ChannelReader` 不承载 durability**（见 §9.5）——它只是 durable inbox consumer 和 bot turn 执行器之间的 working buffer
...
- **Phase 0 前置改动（必需）**：当前 `IProjectionWriteDispatcher<T>` 只有 `UpsertAsync`，没有 `DeleteAsync`。本 RFC 要求先在 `IProjectionWriteDispatcher<T>` 加 `DeleteAsync(string key, CancellationToken ct)` method——否则 `PerEntryDocumentProjector` 的 Tombstone 路径无处落地。早期草稿设想"靠 upsert tombstone document 软删"是错的：当 entry 从 state 里物理移除后，base loop 只枚举 `ExtractEntries(state)`，根本枚举不到已删 entry，`Evaluate()` 永远不会 call，tombstone document 永远不会 upsert——projection store 无限膨胀。这个底层改动独立 PR 先合入，是 §7.1 的前置条件
...
**IHostedService 精神**：
- `ChannelBotRegistrationStartupService` / `AgentRegistryStartupService` 已经是 IHostedService 模式，但它们只做 **projection warm-up**（host 启动时激活一次 singleton）。Discord gateway 的 IHostedService 角色完全不同——是**持久 WebSocket 连接 supervisor**，需要处理 per-silo 重复连接问题（见 §10.4 cluster-singleton）。不是"沿用同一精神"，是新起一个 hosted service 生命周期模型
    ...
    where TAdapter : IChannelTransport, IChannelOutboundPort
{
    ...
    // 验证 §5.8：bounded inbound channel 满时 producer 阻塞 + timeout → 升级成错误信号，不静默 drop
    [Fact] public async Task InboundStream_Saturation_TriggersBackpressureSignal() { ... }
...
4. **Dedup 的 authoritative location = `ConversationGAgent` single-grain atomic turn**（Codex v3 HIGH #2 PARTIAL 修复的核心）：
   ...
   - **authoritative check + side-effect + commit 发生在 grain turn 内**——Orleans single-activation 保证同一 grain key 不并发执行 turn，turn 是天然的 mutual exclusion boundary
   - grain turn 流程（atomic）：
     ```
     Turn start:
       if processed_message_ids.Contains(activityId):  return   // seen in prior turn
       execute bot logic → produce state delta + outbound payload
       emit outbound (adapter SendAsync)                         // side effect
...
### 9.5.5 `ChannelReader<T>` 的修正角色

不再是 "durable cut 之后的 pipeline 入口"，而是 "pipeline 内部 consumer 和 bot turn 执行器之间的 bounded working buffer"。具体行为契约见 §5.8 interface contracts——bounded capacity、single reader、producer 满时阻塞而非 drop。

**本 RFC 的 durability 承诺完全由 inbox（durable store）+ `processed_message_ids` 两个 state 承担，`ChannelReader` 不参与。**
...
| 凭据引用（secret ref / vault key） | `ChannelBotRegistrationEntry` proto | 进 projection 可查，但解引用需要 adapter runtime 的 vault client |
| 非密敏感元数据（team_id / workspace_id / guild_id） | proto 正常字段 | ops 查询需要 |

具体实现：`ChannelBotRegistrationEntry` 加 `credential_ref` 字段 + 运行时 `ICredentialProvider` 按 ref 拉凭据。`IChannelTransport.InitializeAsync` 通过 provider 拿凭据、放进 adapter instance 内存，**不回写 registration**。
...
可新增 `LarkLongConnectionAdapter` 作为 `LarkChannelAdapter` 的第二种 transport 子类，共享 `LarkMessageComposer` 和 `ChannelBotRegistrationEntry` schema（仅 credential 消费路径不同）。和 §10.3 Slack 的 Events API + Socket Mode 同构。
...
- **关键架构决策**：**Gateway 跑在独立 `IHostedService`**，不在 silo grain 里。Inbound events → `ChannelReader<ChatActivity>` → middleware pipeline
  ...
  - **为什么 aevatar 现有 IHostedService 先例不够**：`ChannelBotRegistrationStartupService` / `AgentRegistryStartupService` 只做 host 启动时的 projection warm-up（单个进程起一次就完），不是持久连接 supervisor。Discord gateway 的 hosted service 生命周期和负载模型完全不同——**新起一种 hosted service 角色**，而不是"沿用"
...
| # | 提案 | 判决 | Phase |
|---|---|---|---|
| 17.1 | Projection `DeleteAsync` + per-entry current-state materializer | Refine | **Phase 0 前置**（独立 PR） |
...
### 17.1 Projection `DeleteAsync` + per-entry current-state materializer（P0）

**缺口**：`IProjectionWriteDispatcher<T>` (`src/Aevatar.CQRS.Projection.Runtime.Abstractions/Abstractions/Stores/IProjectionWriteDispatcher.cs:3-7`) 和 `IProjectionWriteSink<T>` (`IProjectionWriteSink.cs:3-12`) 只有 `UpsertAsync`，没有 `DeleteAsync`；`ProjectionStoreDispatcher.cs:58-63` 只是透传。Channel runtime 已经在 workaround——`ChannelBotRegistrationProjector.cs:12-16` 和 `DeviceRegistrationProjector.cs:12-18` 注释承认"孤儿文档残留"；`ChannelRegistrationTool.cs:222-258` 用"version 落后"做软判断。`AgentRegistryGAgent.cs:63-81,121-130` 已经做逻辑 tombstone，`AgentRegistryProjector.cs:42-75` 已经按 per-entry 做 tombstone decision——模式已经出现，只是没有框架级别解。

**Refined 形状**（Codex v6 HIGH）：

1. `IProjectionWriteDispatcher<T>` + `IProjectionWriteSink<T>` **加 `DeleteAsync(string id, CancellationToken ct)`**
...
**Phase 0 前置**：`IProjectionWriteDispatcher<T>.DeleteAsync` 独立 PR 先合入；§7.1 `PerEntryDocumentProjector<TState, TEntry, TDocument>` 在本 RFC 内实现时按 refined 基类落地。未改 `IProjectionWriteDispatcher` 就开 §7.1 等于把 workaround 升级成 RFC 规范——必须前置。
...
- Secret store：`IAevatarSecretsStore.cs:3-10` + `SecretManager.cs:11-84`
...
现有 hosted service（`AgentRegistryStartupService.cs:22-60` / `ChannelBotRegistrationStartupService.cs:33-72`）是 **node-local startup/warmup**——host 启动时 poke 一下 grain 让它 activate，不是 cluster-wide supervisor。
...
### 17.7 框架化工作量估算 + Scope Transfer 沟通口径

| Proposal | 独立 PR 行数估算 | 实施风险 |
|---|---|---|
| 17.1 `DeleteAsync` + per-entry materializer | ~200 行（接口 + base class + 3 个 existing projector refactor） | 低——贴现有抽象 |
| 17.3 `AuthContext` + `ICredentialProvider` + credential_ref | ~400 行（contract + secret store wiring + Channel/LLM/Connector 域调用点迁移） | 中——跨域调用点多，需要渐进迁移 |
| 17.4 Tracing dimension contract + `IErrorRedactor` + 现有泄露修复 | ~300 行（contract + redactor + 2 处现有泄露 fix） | 中——redactor policy 定义需跨 domain 对齐 |
| 17.2 Recurring schedule state + next-fire helper | ~150 行（state record + extension method） | 低——在现有 scheduler 之上薄层 |
| 17.5 Cluster-singleton primitive | Phase 后续 | N/A——等第二场景 |
| 17.6 Durable Inbox | 不做 | — |

**总前置工作量**：约 900 行独立 PR 代码（17.1 + 17.3 + 17.4 三条 Phase 0 前置），在本 RFC 的 chat 层实施 Phase 0-1 之前完成。

**Scope transfer 的意义**：本 RFC 不只是 chat 层拆分，还顺手触发 aevatar core 的三处结构性补强——projection delete 能力 / principal-aware auth / observability redaction 契约。这些 core 改动对 workflow / LLM / NyxID / device / connector 等其他 domain 都有长期价值：

- `DeleteAsync`：`NyxidChat` / `Household` / future user catalog 全都需要的能力
- `AuthContext` + `credential_ref`：`NyxIdProxyTool` 的 ad-hoc fallback / connector auth / LLM metadata 都可以统一走这条
- `IErrorRedactor`：`NyxIdLLMProvider` + `HttpConnector` 的活跃泄露 **本 RFC 不修也该有人修**——借机一起修

**建议拆分 RFC 发布**：如果从 chat 视角单独看 RFC 工作量偏大，应把 §17 这 900 行前置工作**认定为 aevatar core 投资**，不计入 chat 实施成本。推到 Auric 时分两份 RFC 讲会更清晰：

- **RFC-Core**（本节 §17.1 + §17.3 + §17.4，3 独立 PR，工作量 ~900 行）：projection delete / principal-aware auth / error redaction——纯框架补强，受益全仓
- **RFC-Channel**（本文档 §1-§16）：chat-specific 抽象层 + channel adapter 集群；依赖 RFC-Core 前置合入

两 RFC 解耦发布能让 core 改动先过 review + 先合入，chat RFC 的 reviewer 不会被 "为什么 chat 要改核心 projection 框架" 这种问题分心。
