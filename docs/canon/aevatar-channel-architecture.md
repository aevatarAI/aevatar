...
sequenceDiagram
    ...
    participant AD as "Channel Transport Adapter<br/>IChannelTransport impl<br/>(e.g. NyxIdRelayTransport)"
    ...
    participant OP as "Outbound Delivery<br/>(Nyx relay / platform adapter)"
...
legacy `ChannelUserGAgent` 同时持有**用户绑定态**（`nyxid_user_id` / `nyxid_access_token`，用于 outbound 发消息时 fallback 拿 token）和**会话态**（`pending_sessions` / `processed_message_ids`）。issue `#308` 已删除这类 raw token ownership；若未来恢复 conversation-grain 方案，这两类状态仍必须分离：
...
The default workflow notification delivery path is `IChannelInteractionNotificationPort` backed by delivery-target lookup, `MessageContent` composition, and NyxID/channel relay dispatch. Remote tool approval notification follows the same ownership rule: `IRemoteToolApprovalNotificationPort` is composed by the NyxID relay/channel boundary, maps approval facts to `MessageContent`, and delegates native rendering/transport to registered channel producers and senders. Scheduled-agent packages must not own Lark-specific notification ports or register Lark delivery. Adding a new channel therefore means:
...
**旧的“单一类同时实现 transport + outbound”直连平台模型已经退役**：在当前生产主干里，transport 入口是 `Channel.NyxIdRelay`，平台项目只保留渲染/组合职责。上层依赖仍然面向 `IChannelTransport` / outbound 契约，但不再假定 Lark 存在一个 `LarkChannelAdapter : IChannelTransport, IChannelOutboundPort` 的直连实现。
...
├── Aevatar.GAgents.Platform.Lark.Tests/       ← 覆盖 Lark composer / rendering contract
...
| 新包 | 归属 slnf | 理由 |
|---|---|---|
| `Aevatar.GAgents.Channel.Abstractions` | `aevatar.channels.slnf` | transport-facing inbound/outbound contract |
| `Aevatar.GAgents.Channel.Runtime` | `aevatar.channels.slnf` | `ConversationGAgent` + event contracts + projection-facing current-state runtime |
| `Aevatar.GAgents.ChannelRuntime` / `Channel.NyxIdRelay` | `aevatar.channels.slnf` | transport orchestration, relay adapter, shim endpoints |
| `Aevatar.GAgents.Platform.{Lark,Telegram,Slack,Discord}` | `aevatar.platforms.slnf` | platform rendering / composer / PII shaping only |
...
`solution_split_guards.sh` 和 `solution_split_test_guards.sh` 已同步覆盖 `aevatar.channels.slnf` 与 `aevatar.platforms.slnf`。

**依赖方向（csproj 层检查）**：
- `Abstractions` 不依赖任何新包（顶层）
- `Runtime` 依赖 `Abstractions` + `Aevatar.CQRS.Projection.Core.Abstractions`
- `platforms/*` 依赖 `Abstractions` + `Runtime` + 各自 native SDK
- `Authoring` / `Scheduled` / `Device` 依赖 `Abstractions` + `Runtime`
- **禁止** `platforms/*` → `Authoring/Scheduled/Device` 反向依赖（`tools/ci/architecture_guards.sh` 扩展覆盖）
...
- **grain state durable dedup**：`ConversationGAgent.processed_message_ids`。`Activity.Id` 必须是 platform-stable delivery key（见 §5.1）才能正确 dedup；历史 sender-keyed `ChannelUserGAgent` 只是这个约束的反例，不再属于当前实现。
...
- **JSON 展示策略**：Lark presentation boundary 对回复文本中的完整合法 JSON 做有界解析，并由 `LarkMessageComposer` 投影为 Card JSON 2.0 原生 `table`；对象数组映射为行列，嵌套值展开为非 JSON 单元格文本。这个策略只改变 Lark 出站载荷，不改写模型历史、工具结果或内部协议。CardKit 流式 `create/stream` 阶段保持原文，完整最终 JSON 再将流式 shell 替换为原生表格；只有 CardKit/interactive dispatcher 不可用时才降级为逐行 `key: value` 文本，嵌套层级每深入一级增加 4 个空格，不使用 Markdown 管道表格。工具调用轮的模型计划旁白不属于最终回复，不进入 CardKit 或文本流式展示。
...
### 10.4 Discord（`agents/platforms/Aevatar.GAgents.Platform.Discord`）
...
## 11. 历史 sender-key 迁移笔记（非当前 cutover）

本节记录的是**已废弃**的 `ChannelUserGAgent -> ConversationGAgent` sender-key 迁移推演。issue `#308` / ADR-0012 的实际 hard cut 并不沿这条路径执行；当前必须执行的 preflight 是清空 pre-ADR-0012 `channel-bot-registration-store` 持久态，见 ADR-0012 和 `2026-04-22-lark-nyx-cutover-runbook.md`。下面内容仅作为未来如果重新引入 conversation-grain runtime 时的历史约束。

历史 sender-key 方案里，`ChannelUserGAgent` 以 sender id 为 grain key，state 包含 `pending_sessions` / `processed_message_ids` 等。若未来迁到 `ConversationGAgent` 用 canonical key，老 grain state 才需要处理。

前提是**确认 aevatar 是否已经有生产用户**：
- 如果重新进入这类迁移时**无生产用户**，可直接废弃老 `ChannelUserGAgent` state；issue `#308` 这次 hard cut 走得更激进，直接删除旧实现并要求清空旧 registration store
- 如果**已有用户**，需要 shadow 双写 / 灰度切换 / 数据迁移脚本其中之一

这个前提必须在实施前明确 confirm，不能默认某一种。

### 11.1 Cutover in-flight events（未来若重启该迁移也必须处理）

"无生产用户" ≠ "deploy window 内无风险"。Codex review 指出这个盲区。若未来重启这条 sender-key → conversation-key 迁移，部署窗口里：
...
**Cutover 策略（历史 sender-key 方案的 Codex v6 HIGH-6 修正）**：v5 尝试用"grain-level dedup 跨版本兜底"简化 cutover，但 Codex v6 抓出致命漏洞：**v1 用 `ChannelUserGAgent`（sender-keyed）v2 用 `ConversationGAgent`（canonical-keyed）——grain key space 不同，跨版本 dedup 不成立**。v5 方案前提破产，回退到正统 drain-based cutover：
...
**如果未来在有生产用户场景重启这条 sender-key 迁移路线**：原 `ChannelUserGAgent` state（用户绑定 token / preferences）需要迁移到 v2 的 `ChannelUserBindingGAgent`（§5.2b）——超出本 RFC scope，走独立 migration RFC + shadow 双写过渡方案。本 RFC 的当前 hard cut 不承担这类兼容迁移。
...
2. **Lark 作为 reference implementation**
   - `Aevatar.GAgents.Channel.Runtime` + `Aevatar.GAgents.Platform.Lark` 落地
   - 现有 Lark bot 在新抽象下跑通
   - Group chat 支持、streaming reply 重接
   - Gate：行为无回归

3. **Telegram + Conformance Suite**
   - `Aevatar.GAgents.Platform.Telegram` 落地
...
|---|---|---|
| `channel_native_sdk_import_guard.sh` | 禁止 `using Larksuite.Oapi` / `using Telegram.Bot` / `using SlackNet` / `using Discord.*` 出现在 `src/**` 和 `agents/Aevatar.GAgents.*`（除 `agents/platforms/**` 自身）的 .cs 文件 | grep `using SDK.Namespace;` 即可，无歧义 |
...
- Channel bot registration 的 raw token 持久化已在 ADR-0012 / issue `#308` 中移除，但 broader Nyx credential flow 仍未完全统一成 `credential_ref`：LLM / workflow / connector 路径仍可能通过 metadata 或 runtime-only state 传递 Nyx credential material
...
- Raw credential material 仍在**部分内部 runtime / metadata 路径**里活跃流动：
  - `ChannelBotRegistration` proto/state/query 已清掉 raw channel credentials，但 `src/Aevatar.AI.Abstractions/LLMProviders/LLMRequestMetadataKeys.cs:7-8` 仍通过 metadata key 传 `nyxid.access_token` / `nyxid.org_token`
  - `UserAgentCatalogEntry` 这一类内部 runtime state 仍持有 Nyx delivery credential material，公开 query 侧只是不再暴露
  - relay / chat / connector 相关 consumer 仍未统一切到 `credential_ref` + late resolution
...
**明确不升**：legacy `ChannelBotRegistration.nyx_user_token` 式 raw token 持久化已经删除，也**不应**被框架认证为 best practice。未来若再引入同类 raw-secret persistence，等于把最坏的做法重新冻结成平台契约。
...
2. **历史 actor 路径曾做 continuation 耐久化**：
   - 早期 `ChannelUserGAgent` 会把 pending session / processed message ids 放进 actor state
   - issue `#308` 已删除这条 sender-keyed continuation 路径，因此这点只保留为历史背景，不再作为当前实现依赖
   - 当前 relay hard cut 之后，durable 关注点更集中在 ingress receipt / registration-store preflight，而不是复活一个通用 inbox 子系统
