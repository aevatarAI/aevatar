...
2. aevatar 内部创建一个长期 `SkillDefinitionGAgent`（`skill-runner-{guid32}`）；每次 cron/手动触发再派生一个短生命周期 `SkillExecutionGAgent` 执行 daily 报告。
...
| ③ aevatar 内部 | 处理 | 鉴权 → 解析 `/daily` → `AgentBuilderTool.CreateDailyReportAgentAsync` → 创建 `SkillDefinitionGAgent` |
...
```
Lark User      Lark App     NyxID Relay     aevatar(webhook)    SkillDefinition/Execution     NyxID(proxy)     GitHub      LLM
   │              │              │                │                      │                  │              │          │
   │── /daily ───▶│              │                │                      │                  │              │          │
   │              │── event ────▶│                │                      │                  │              │          │
   │              │              │── POST relay ─▶│                      │                  │              │          │
...
注意几个时间窗：
- **webhook 返回窗口**：当前 `HandleRelayWebhookAsync` 会等待 `ConversationGAgent.HandleEventAsync` 返回。对 `/daily run_immediately=true` 来说，创建 agent、GitHub preflight、首次 SkillExecution 执行和创建确认回复都在这条调用链里完成；因此“webhook 必须 ≤3 秒返回”是目标约束，不是当前代码已经保证的行为。
...
| l | 264-265 | 取 / 创建 `SkillDefinitionGAgent` | — |
...
| p | 275-302 | 构造 `InitializeSkillDefinitionCommand`，通过 command port 派发到 definition actor | — |
| q | 304-308 | 若 `run_immediately=true`，再派发 `TriggerSkillDefinitionCommand{Reason="create_agent"}`；definition actor 创建 execution actor | — |
...
### 阶段 ⑤ SkillDefinition / SkillExecution 执行 → NyxID → GitHub

**触发**：
- 立即执行：阶段 ③.q 派发 `TriggerSkillDefinitionCommand{Reason="create_agent"}` 到 `SkillDefinitionGAgent`
- 定时执行：`ChannelScheduleRunner.ScheduleNextRunAsync` → Orleans 持久化回调 → fire `TriggerSkillDefinitionCommand{Reason="schedule"}`
- 手动：`/run-agent <agent_id>` → 同样的 trigger，`Reason="manual"`
- 执行重试：`SkillExecutionGAgent` 失败时 `ScheduleRetryAsync` → 30s 后向 execution actor fire `RetrySkillExecutionCommand{RetryAttempt=N}`

**Handler**：`SkillDefinitionGAgent.HandleTriggerAsync` 创建 `SkillExecutionGAgent` 并发送 `StartSkillExecutionCommand`；终态由 execution actor 回报给 definition actor，再由 definition actor 更新 catalog。
```
if (!State.Enabled) return;                         // 禁用即跳过
try {
    dispatch StartSkillExecutionCommand to SkillExecutionGAgent;
    Scheduler.ScheduleNextRunAsync(now);
    UpdateRegistryStatusAsync(StatusRunning, preserveExecutionState: true);
}
catch (Exception ex) {
    PersistDomainEventAsync(SkillDefinitionExecutionDispatchFailedEvent { Error = ex.Message });
    UpdateRegistryExecutionErrorAsync(...);
}
```
...
### 命令 / 事件
- 命令：`InitializeSkillDefinitionCommand`、`TriggerSkillDefinitionCommand`、`StartSkillExecutionCommand`、`RetrySkillExecutionCommand`、`DisableSkillDefinitionCommand`、`EnableSkillDefinitionCommand`、`UserAgentCatalogUpsertCommand`、`UserAgentCatalogExecutionUpdateCommand`、`UserAgentCatalogTombstoneCommand`
- 事件：`SkillDefinitionInitializedEvent`、`SkillDefinitionNextRunScheduledEvent`、`SkillDefinitionExecutionDispatchedEvent`、`SkillDefinitionExecutionCompletedEvent`、`SkillDefinitionExecutionFailedEvent`、`SkillExecutionStartedEvent`、`SkillExecutionCompletedEvent`、`SkillExecutionFailedEvent`、`SkillDefinitionDisabledEvent`、`SkillDefinitionEnabledEvent`、`UserAgentCatalogUpsertedEvent`、`UserAgentCatalogExecutionUpdatedEvent`、`UserAgentCatalogTombstonedEvent`
...
**默认值**：`agents/Aevatar.GAgents.Scheduled/SkillDefinitionDefaults.cs`
...
**事实源**：`SkillDefinitionGAgent` actor state（每个 agent 一个长期 definition actor）+ `SkillExecutionGAgent` actor state（每次运行一个短生命周期 execution actor）+ `UserAgentCatalogGAgent`（well-known，全局唯一注册表 actor）
...
**默认路径（streaming-edit）**：`SkillExecutionGAgent` 在 LLM 流式输出过程中，第一条非空 delta 走 `POST {NyxID}/api/v1/proxy/s/api-lark-bot/open-apis/im/v1/messages?receive_id_type={primary_type}`（`{receive_id, msg_type:"text", content:"{\"text\":\"...\"}"}`），从响应 `data.message_id` 捕获平台消息 id；之后每条 delta 走 `PUT .../open-apis/im/v1/messages/{message_id}`（`{msg_type:"text", content:"{\"text\":\"...\"}"}`）。Lark 编辑接口按 `msg_type` 分动词：**PUT** 编辑 text/post (rich text)，**PATCH** 编辑 interactive card——发 text 必须用 PUT，否则 Lark 会拒绝每一次编辑，streaming-edit 在占位之后无法增长（参考 [Lark Edit message 文档](https://open.feishu.cn/document/server-docs/im-v1/message/update)）。Throttle 默认 300ms（`SkillDefinitionDefaults.StreamingEditThrottle`）：throttle 窗口内多条 delta 折叠到最新文本，单 turn 平均编辑速率 ≤3.3/s 安全在 Lark 限速以下。Finalize 阶段强制再编辑一次最终文本以保证一致性，跳过 throttle。详见 [SkillRunnerStreamingReplySink.cs](../../agents/Aevatar.GAgents.Scheduled/SkillRunnerStreamingReplySink.cs)。
...
**失败语义**：
- 初次 POST 在流式中段失败：日志告警，**下一条 delta 重试**（流式失败不等于 turn 失败，LLM 仍在出 token）。Finalize 时若 POST 仍失败 → 抛 `InvalidOperationException`，主链路 catch 进 `SkillExecutionFailedEvent` 持久化路径。
...
- 两次都失败：`SkillExecutionFailedEvent` + `TrySendFailureAsync` + 回报 `SkillDefinitionGAgent` 更新 catalog `StatusError` + 仍调度下一次定时
...
### 12.3 单元测试 — SkillDefinition / SkillExecution actors

文件：`test/Aevatar.GAgents.ChannelRuntime.Tests/SkillDefinitionGAgentTests.cs`、`test/Aevatar.GAgents.ChannelRuntime.Tests/SkillExecutionGAgentTests.cs`

应覆盖：
- `HandleInitializeAsync`：`SkillContent` 为空 → 直接返回不持久化（仅 LogWarning）
- `HandleInitializeAsync` 正常 → 持久化 `SkillDefinitionInitializedEvent` + `Scheduler.ScheduleNextRunAsync` + `UpsertRegistryAsync`
- `SkillDefinitionGAgent.HandleTriggerAsync`：`State.Enabled=false` → 跳过；正常时派生 execution actor 并持久化 definition-owned execution sequence
- `SkillExecutionGAgent.HandleStartAsync` 成功 → `SkillExecutionCompletedEvent` + report back 到 definition actor
- `SkillExecutionGAgent.HandleStartAsync` 失败：`RetryAttempt < 1` → `ScheduleRetryAsync(2)` 不发 terminal failure
...
**`social_media` 模板**（同一条 `/daily`-类入口，但走 `WorkflowAgent` 而非 SkillDefinition/SkillExecution actors，由 `AgentBuilderTool.CreateSocialMediaAgentAsync` 处理；`AgentBuilderTemplates.TryBuildSocialMediaSpec` 拼模板）：
...
**LLM provider**：
- 默认 `SkillDefinitionDefaults.DefaultProviderName`（生产用 NyxID 路由；测试可 stub）
...
1. **加 emoji 反应**和**daily 报告**不是同一个 HTTP 请求；emoji 是 fire-and-forget 的 best-effort 反应，报告走 `SkillExecutionGAgent` 的 proactive proxy 投递。两者可以独立失败。
2. **首次 `/daily` 当前通常会产生两条用户可见消息**：一条是 execution actor 实际执行后的报告，另一条是 `AgentBuilderTool` 返回的"agent 已注册/正在跑"短文案。由于 `run_immediately=true` 时首次执行在创建 tool 调用内被 await，报告可能先于创建确认到达。
...
| Skill definition actor | `agents/Aevatar.GAgents.Scheduled/SkillDefinitionGAgent.cs` |
| Skill execution actor | `agents/Aevatar.GAgents.Scheduled/SkillExecutionGAgent.cs` |
| Skill 默认参数 | `agents/Aevatar.GAgents.Scheduled/SkillDefinitionDefaults.cs` |
