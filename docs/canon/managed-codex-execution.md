...
Workflow ingress keeps trusted workflow scope attribution and native NyxID
authority as separate contracts. Credential extraction resolves
`ExternalUserId` only from `uid`, then `sub`, then
`ClaimTypes.NameIdentifier`, then `user_id`; `scope_id` is never a fallback.
The resulting external subject drives binding lookup, tool authority, and
managed-Codex caller mapping. A scope-only principal therefore has no complete
NyxID authority and cannot manufacture one from the workflow scope.
