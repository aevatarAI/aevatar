...
14. `ProjectionScopeStatusDocument` 的写入者由 source projection scope actor 权威决定（`ProjectionScopeState.status_route`，typed、route epoch 单调），status document 只是该决定的副本，任何 writer 不得读 document 来选择路由。durable materialization scope 在 cold ensure / activation 路径上（绝不在每条 observed envelope 上）用一份 fresh 的 `PROJECTION_SCOPE_STATUS_TERMINAL_V1` fleet admission 采用 terminal route：actor 先提交 epoch-fenced `ProjectionScopeStatusRouteActivatedEvent`（committed route 是事实，relay 与 materializer 由它派生并在每次 activation 上自愈），再在自己的 stream 上写入 terminal materializer relay（它拥有自己的 relay，不做跨 actor 等待）、ensure `projection.scope-status-terminal` actor，随后移除 legacy `projection-scope-status` shadow relay、release 该 shadow scope 并提交 `ProjectionScopeStatusLegacyRouteReleasedEvent`；整个采用过程（route 决策 + legacy cleanup）在 cold ensure 和 activation 两条路径上都发生在该 scope 自己的 observation relay（activation service 等待的 evidence）被（重）写入之前，activation service 不会观察到半采用的 scope；没有 fresh grant 时保持 legacy route，并通过 durable callback scheduler 发退避的 self continuation（`RetryProjectionScopeStatusRouteAdoptionCommand`，attempt 随命令携带，30s/1m/2m/4m/8m，之后以 8m 节奏持续重试直到采用，永不停止）在自己的 turn 上重试采用——否则 silo 启动后在 fleet gate 打开之前激活的长生命周期 scope 永远留在 legacy route。已采用的 route 在 activation / 再次 ensure 时只重申 relay、补建缺失的 materializer actor，不重新决策；即使 `LegacyRouteReleased=true`，每次 activation 也通过 authoritative binding evidence 对账：后来重新出现的 legacy relay（旧节点 activation service 再次 ensure shadow、或延迟到达的 legacy relay upsert）会被移除并再次 release shadow actor；被 release 后再 ensure 的 source 重申 relay 即可，materializer 收到 live source 的第一条 routed publication 自行重启（source 已 released 的 straggler 不重启）。terminal materializer 是独立 actor kind 与独立 protobuf state（旧二进制不能激活或修改），只消费 source scope 现有的 `CommittedStateEventPublished`，只在 committed `status_route` 命名其 contract 时写入，且只在 terminal outcome（除 Received / Attempted / Staged 之外的 source event）上写一次；它不再产生 Received / Attempted / WatermarkAdvanced 记账流，写失败先提交 actor-owned `ProjectionScopeStatusWriteDeferredEvent`（typed `failure_kind`：transient / rejected）再确认 envelope；transient 失败通过 durable callback scheduler 做事件化退避重试（`RetryProjectionScopeStatusWriteCommand`，1s/5s/30s/2m/10m，之后以 10m 节奏持续），store 恢复后无需新 source event、手工 ensure 或 actor deactivation 即自动写入并以 `WriteRecoveredEvent` 清除 pending；连续 5 次失败进入显式可观测的 stalled 态（`ProjectionScopeStatusWriteStalledEvent`、`pending_write.stalled`、error log + `IProjectionFailureAlertSink`），重试仍继续；同版本 Conflict / Gap 是 rejected 态（同字节重试无法治愈）：记录、告警、不重试，只由更高版本的成功写入取代；source 已 released 且 detached 时 materializer 随之 release，但只在没有 pending write 时（release 会清除 pending write，否则 source 的最终 released 状态永远写不出去）。legacy shadow projector（新二进制）看到 terminal route 即跳过写入；`ProjectionScopeStatusDocument` 不携带任何 writer 专属事实（`status_route` 只在 source scope state 上，document 的 field 30 已 reserved），因此同一 (state, event, timestamp) 无论由旧二进制的 legacy projector、新二进制的 legacy shadow 还是 terminal materializer 写出都逐字节相同，同版本只能是 exact duplicate（mixed old/new binary 由 `ProjectionScopeStatusMixedBinaryTests` 用 b64c96a4 的真实旧版 projector 代码作为 fixture 验证）。滚动语义：rolling forward——fleet gate 只在所有 silo 都广播 terminal contract 后打开，source 才 flip，旧二进制的延迟 legacy 写入命中已写版本时为 Duplicate/Stale，命中尚未写的更高版本时为 Applied、随后 terminal 同版本为 Duplicate；rollback——旧二进制的 wrapper 重新 ensure legacy shadow（terminal materializer 成为孤儿，旧 kind 无法激活），legacy 写入 terminal 已写版本为 Duplicate、更高版本为 Applied，再次 forward 时 terminal 同版本 Duplicate；drain——任一时刻仍只有一个权威 writer，且没有任何时刻两个 writer 会在同一 (actor, version, event) 上产生不同字节。activation service 在 source ensure 之后先读一次 terminal relay evidence：存在即不再 ensure legacy shadow；不存在才按旧行为 ensure legacy。
