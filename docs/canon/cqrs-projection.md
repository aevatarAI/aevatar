...
8. committed observation continuation 只能做最小 durable handoff（例如投递到 run/session actor）或轻量通知；跨 actor 命令编排、轮询 read model 可见性、等待外部服务完成等业务推进必须离开 projection materialization turn，在独立 actor/command continuation 中执行。
