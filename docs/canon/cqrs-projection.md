...
14. `ProjectionScopeStatusDocument` 的写入者由 source projection scope actor 权威决定（`ProjectionScopeState.status_route`，typed、route epoch 单调），status document 只是该决定的副本，任何 writer 不得读 document 来选择路由。durable materialization scope 在 cold ensure / activation 路径上（绝不在每条 observed envelope 上）用一份 fresh 的 `PROJECTION_SCOPE_STATUS_TERMINAL_V1` fleet admission 采用 terminal route：actor 先提交 epoch-fenced `ProjectionScopeStatusRouteActivatedEvent`（committed route 是事实，relay 与 materializer 由它派生并在每次 activation 上自愈），再在自己的 stream 上写入 terminal materializer relay（它拥有自己的 relay，不做跨 actor 等待）、ensure `projection.scope-status-terminal` actor，随后移除 legacy `projection-scope-status` shadow relay、release 该 shadow scope 并提交 `ProjectionScopeStatusLegacyRouteReleasedEvent`；整个采用过程发生在该 scope 自己的 observation relay（activation service 等待的 evidence）可见之前，activation service 不会观察到半采用的 scope；没有 fresh grant 时保持 legacy route，并通过 durable callback scheduler 发一条有界退避的 self continuation（`RetryProjectionScopeStatusRouteAdoptionCommand`，attempt 随命令携带，约 15 分钟内 5 次）在自己的 turn 上重试采用——否则 silo 启动后在 fleet gate 打开之前激活的长生命周期 scope 永远留在 legacy route。已采用的 route 在 activation / 再次 ensure 时只重申 relay、补建缺失的 materializer actor，不重新决策；被 release 后再 ensure 的 source 重申 relay 即可，materializer 收到 live source 的第一条 routed publication 自行重启（source 已 released 的 straggler 不重启）。terminal materializer 是独立 actor kind 与独立 protobuf state（旧二进制不能激活或修改），只消费 source scope 现有的 `CommittedStateEventPublished`，只在 committed `status_route` 命名其 contract 时写入，且只在 terminal outcome（除 Received / Attempted / Staged 之外的 source event）上写一次；它不再产生 Received / Attempted / WatermarkAdvanced 记账流，写失败先提交 actor-owned `ProjectionScopeStatusWriteDeferredEvent`（有界立即重试 + activation 重放）再确认 envelope，同版本 Conflict 只记录不重试；source 已 released 且 detached 时 materializer 随之 release，但只在没有 pending write 时（release 会清除 pending write，否则 source 的最终 released 状态永远写不出去）。legacy shadow projector（新二进制）看到 terminal route 即跳过写入；同一 (state, event, timestamp) 两个 writer 通过共享 mapper 产生逐字节相同的 document，同版本只能是 exact duplicate。滚动升级由 fleet gate 的全员一致性保证旧二进制不会与已 flip 的 source 共存；回滚到旧二进制时旧 wrapper 重新 ensure legacy shadow、terminal materializer 成为孤儿（不能被旧 kind 激活），任一时刻仍只有一个权威 writer。activation service 在 source ensure 之后先读一次 terminal relay evidence：存在即不再 ensure legacy shadow；不存在才按旧行为 ensure legacy。
