...
NyxID publishes the service inventory and API-key scope-plan contracts used by scheduled authorization. For a personal owner, `NyxIdAuthorizationCatalogRefreshPort` reads active, grant-eligible `GET /api/v1/user-services` entries, requests `POST /api/v1/api-keys/scope-plan` for that exact full service set, and validates the returned authority, actor, owner, contract, policy, completeness, service, and node facts. If that full-set scope plan fails with NyxID's deterministic `api_key_scope_plan_route_unresolved` conflict, the refresh port isolates the provider failure by requesting singleton scope plans for each grant-eligible service in the same sorted order. Singleton successes are validated with the same authority, actor, owner, contract, policy, completeness, service, and node checks; singleton `api_key_scope_plan_route_unresolved` failures are omitted from the observed catalog rather than poisoning unrelated service evidence. Authorization, rate-limit, transport, transient, malformed, and catalog-mismatch failures remain fail-closed and do not commit partial evidence. The owner-scoped catalog actor commits typed per-service resource owners, node-grant requirements and node IDs, provider evaluation time, local observation and freshness times captured after provider work completes, and Aevatar's Protobuf content digest. Normal committed-state projection remains the only path into the catalog read model.
