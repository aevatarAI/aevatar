...
Workflow definition actors compile connector capability references, owner-LLM requirements, exact NyxID service requirements, and the typed service-grant policy during the bind turn. These dependencies are committed with the workflow revision and copied into the workflow current-state read model. Projection and schedule query paths must not parse workflow YAML to recreate them.

Authorization planning consumes five independently versioned actor-scoped current-state replicas: Studio member, workflow definition, connector catalog, owner UserConfig, and NyxID catalog. A binding-run revision cannot substitute for any of these committed versions. Missing or stale input fails closed; an empty service list is exempt only when the committed workflow dependency policy explicitly states that no external service is required.
