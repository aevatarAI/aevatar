...
Workflow capability admission has distinct live and persisted contracts. Live admission derives `nyxid/personal/<subject>` only from the authenticated caller and may use transient bearer credentials to inspect `/api/v1/mcp/config` or canonical code-route readiness. The resulting `external-capability-admission.v6` plan seals that typed durable owner and any response projection into `admission_digest`. After the plan is committed into a service revision or Studio member binding run, prepare, publish, replay, and handoff use credential-free persisted revalidation: they reparse the bound definition, validate schema/capabilities/source freshness/digest, require the caller-owned expected execution mode to match the plan exactly, and require the owner-derived catalog actor id to equal the sealed catalog source id. Existing v4 plans remain supported; their original NyxID proofs are still fully validated, while a legacy `code_execute` call site remains runtime-revalidated without requiring a user rebind. V2, v3, and v5 plans require an explicit rebind before old authoring is parsed; v5 predates the mandatory response-persistence boundary and is not accepted across mixed-version workers. The expected mode is never read back from the plan being validated. These paths never reconstruct a caller from `appId`, `serviceId`, route position, or an empty-identity convention.

Ornn skill scheduling keeps review and mutation separate. The confirmation preview remains a pure read even when the only readiness blocker is `DURABLE_AUTHORIZATION_UNAVAILABLE`: it may return the exact review contract only for a typed read-only NyxID request whose policy explicitly allows Durable execution. Preview never refreshes the authorization catalog, activates projection, polls, or provisions Studio state. The confirmation produced for the skill source is an identity-unbound attestation: both `workflowId` and `revisionId` are empty while call site, request-contract digest, and attested risk remain exact. Studio may bind that attestation once, after it deterministically mints the target workflow and revision identities. A confirmation with either non-empty source identity is never rewritten; a target mismatch remains `NYXID_EXPLICIT_REQUEST_CONFIRMATION_BINDING_MISMATCH`, and Studio mutation stays at zero.
...
`api_key_scope_plan_route_unresolved` is a deterministic route-configuration rejection, not a transient node-offline condition. Schedule mutation surfaces return non-retryable HTTP `409` with the exact required `UserService.id` set; they do not collapse this case into a retryable refresh failure or expose NyxID's free-form provider message. An operator uses `GET /api/v1/keys` to inspect those ids and their same-owner, same-catalog active peers. Route repair uses the exact `PUT /api/v1/user-services/{id}` surface: `{"node_id":"<valid-node-id>"}` restores an intended node route, `{"node_id":""}` switches to direct routing, and `{"node_id":"","is_active":false}` both clears and reversibly disables an obsolete route. Clearing `node_id` on this surface also reconciles the old active binding; changing only `is_active` can leave a same-catalog binding that still poisons a peer's scope plan. If the only copy of a node-managed credential disappeared with the broken node, clearing the route repairs topology but can leave the service awaiting credential reconnection, so the operator must re-read its credential status before scheduling. `DELETE /api/v1/user-services/{id}` is the narrower soft-deactivation and binding-cleanup operation. `DELETE /api/v1/keys/{id}` is instead an explicit credential disconnect and may require the documented OAuth grant-scope/cascade confirmation. Credential status `revoked` alone is not proof of a route-resolution defect: only an active configured route or active same-catalog peer/binding that references no node, a missing node, an inactive node, or a differently owned node satisfies this failure. Operators must repair the reported required service set and its active catalog peers instead of repeatedly refreshing or disabling unrelated owner services.
...
Confirmed Ornn skill provisioning uses the same write-side rule in a bounded form. If initial Durable admission fails only because its catalog proof is unavailable, Studio first performs Interactive admission to resolve the exact typed capabilities and rejects anything not Durable-safe. It refreshes only the ordinal set of required `UserService.id` values, performs one visibility read for the committed refresh version, and retries Durable admission once only after that version is visible. Projection pending, refresh unavailable, and refresh superseded remain typed retry outcomes and occur before member, binding, or schedule mutation. This is write-side preparation, not query-time priming; no preview or query path invokes it.
...
Admission exposes two deliberately different code contracts. `SafeBlockerCode` preserves a syntactically bounded, user-safe blocker such as an explicit-request confirmation mismatch for interactive skill/API diagnosis. `StableCode` remains the narrow scheduled-dispatch persistence vocabulary below; unknown blockers collapse to `WORKFLOW_ADMISSION_REJECTED` there so actor state and operator automation do not acquire an unbounded code surface.
