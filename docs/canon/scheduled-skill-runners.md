...
Catalog refresh completion is actor-owned. The refresh port prepares and attaches a refresh-scoped Projection Pipeline session before dispatch, waits for a committed `Started` outcome before calling NyxID, and derives success, failure, access denial, instability, or supersession only from the matching committed terminal outcome. Dispatch admission is not refresh completion or catalog readiness. The actor fences ownership by the ordinal tuple `(startedAt, refreshId)` and retains the newest watermark across terminal completion and cleanup, so delayed older commands and deterministic equal-time losers commit `Superseded` without replacing or regressing the current refresh. Missing committed outcomes return `ObservationTimedOut`; every path detaches and releases its exact projection session.
...
Immediately before creating a dedicated key, `ScheduledAgentApiKeyIssuer` requests a new scope plan for the validated authorization plan's exact ordinal-sorted service IDs and passes `target_org_id` only for an already validated organization owner. The issuer requires the current authority, contract and policy versions, authenticated actor and intended owner, freshness and completeness declarations, every per-service resource owner and node grant, and both flattened allowlists to match the local plan. Any mismatch returns `authorization_plan_changed` and key creation is not called.
