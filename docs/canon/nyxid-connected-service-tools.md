...
Agent Key turns retain their original OpenAPI discovery contract. The runtime selector provides a validated `service_slug`; Aevatar fetches that catalog service's published OpenAPI document through `GetCatalogOpenApiSpecAsync`, then creates the same generic operation schema and admission proof used by other connected services. Agent Key execution uses the existing slug proxy route and API-key transport. It never reads `/api/v1/keys` to discover or validate an Agent Key service.

The Agent Key selector's `agent-key:<service_slug>` value is a **synthetic route identity candidate**. It is a local pairing of the registration Agent Key context and the selected catalog slug, used only to bind the parsed OpenAPI operations to the slug proxy path. In the typed model, `NyxIdServiceInstance.UserServiceId` and `AgentToolOperationAdmission.ServiceInstanceId` carry this candidate only while the turn is in Agent Key route mode; `NyxIdServiceInstance.RouteConstraint.service_slug` and `CatalogServiceSlug` carry the actual route and catalog identities. These fields do not become a source observation merely because they are populated. The candidate is not a NyxID `UserService.id`, does not prove that a real connected-service instance is present, and must not be copied into sender inventory, service ownership, caller identity, or generic connected-service grant records. The precise approval boundary is the sealed operation contract parsed from the fetched OpenAPI document plus the exact selected slug; admission may authorize only that operation's normalized method, path template, parameters, headers, body and response mode. The proxy must continue to use the Agent Key's slug route and may not turn the synthetic candidate into a real UserService lookup or silently widen it to another slug.

For sender-bound turns, a real `UserService.id` returned by the sender's `/api/v1/keys` inventory remains the authoritative connected-service identity. The synthetic Agent Key candidate and sender inventory identity are intentionally different representations with different authority boundaries; code and tests must keep these cases distinguishable.
