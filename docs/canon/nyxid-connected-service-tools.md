...
NyxID LLM progress 使用容量 32 的 bounded channel。首个 delta 立即提交，后续按固定 250ms deadline 或 64KB UTF-8 payload 上限批量提交；分片按 Unicode rune 边界切割，并保留 text/reasoning segment 顺序。terminal/cancellation 使用独立 tail drain，不得因请求 cancellation 丢失已接收的尾部进度，也不得让 progress batching 阻塞 control actor turn。
