...
Admitted text-mode connected-service reads are model-visible bounded projections, not raw provider passthrough. The runtime reads at most 16 KiB from the NyxID proxy response and returns `NYXID_CONNECTED_SERVICE_READ_TOO_LARGE` without partial or raw provider body when the source response or wrapped model-visible error projection exceeds that bound. Ordinary proofless human `nyxid_proxy` calls and `response_mode=file_artifact` keep their separate contracts.
