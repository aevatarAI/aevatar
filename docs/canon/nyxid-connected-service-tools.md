...
read operation 的 terminal 结果固定投影为 `connected_service_read_projection`：proxy transport 以 headers-first streaming read 将 upstream text 限制为 256 KiB，完整 model-visible typed projection 再以 UTF-8 字节数执行 256 KiB 上限；任一边界超限都只返回 `status=retry_required` 的 bounded typed projection，不返回 partial/raw provider body。该投影的 tool receipt 为 `SUCCESS` 只证明边界执行成功并可靠地产生了“必须缩小查询”的结果，不表示 oversized provider data 已被读取或可用；conversation 因此可以继续下一次 LLM turn，用更窄 query/page size 分页读取，而不是把整个 task 终止为工具不可用。`error_code=NYXID_CONNECTED_SERVICE_READ_TOO_LARGE` 只存在于 model-visible projection，receipt 不伪造 provider error。投影携带 bounded service/operation provenance、`content_boundary=untrusted_external_data_only` 与 `instructions_allowed=false`，外部字符串始终只是 data。effect operation 固定投影为 `connected_service_effect_receipt` 并携带 provider-owned typed receipt；read projection 与 effect receipt 不共用结果 shape，upstream effect body 不进入模型或 durable result。
...
Aevatar registrations use the same generic connected-service operation source as Google Calendar, Google Drive and other published services. The registration endpoint remains a read-only `GET /api/channels/registrations` operation with its existing caller-scope isolation. Its published OpenAPI shape may expose `query.scope`; that is an operation input shape, not an authorization grant. A non-admin sender cannot broaden the result to `scope=all`; the endpoint enforces the caller's scope and admin requirement. There is no registrations-specific ToolSource or capability issuer. Operation discovery, schema validation, admission, execution and audit all use the shared `NyxIdConnectedServiceToolSource` and `NyxIdConnectedServiceOperationToolFactory` path.

New Agent Key turns use the same execution key for `GET /api/v1/keys` and `GET /api/v1/mcp/config`. Discovery intersects executable inventory with normalized MCP services by exact `UserService.id` and route slug. It does not derive instances from runtime selectors, guess catalog-spec keys from slugs, or fetch raw OpenAPI documents. Missing, denied or unavailable catalogs expose no Agent Key operations; the existing non-Agent-Key custom OpenAPI path is unchanged. This requires NyxID Agent Key access to those read surfaces, including a general Agent Key with `proxy` or `proxy:*` scope for MCP config.

This Agent Key discovery change targets Channel service-level selectors with empty `EndpointNames`. Existing endpoint filtering is unchanged and does not resolve MCP operation names to opaque endpoint IDs. Supporting a nonempty name whitelist when those values differ is outside this change; admission payloads and selector digests retain their existing schema.

Before dispatch, new exact Agent Key published operations reread MCP config with the execution key, including when a broader source-readable bearer is present. Missing service authority, route mismatch, endpoint ID mismatch or contract-digest drift rejects the operation before proxy execution. Successful requests retain the exact `_nyxid_via` UserService route; unavailable catalogs do not fall back to a read-only authority bypass for Agent Keys. Existing exposure restrictions, registration grants and the 100 KiB Channel catalog budget still apply.

Previously frozen `agent-key:<service_slug>` admissions retain their existing slug-route execution and selector semantics; they are not rewritten into real UserService IDs. No new synthetic admissions are generated. Sender-bound discovery continues to use the sender's bearer and real `/api/v1/keys` inventory.
