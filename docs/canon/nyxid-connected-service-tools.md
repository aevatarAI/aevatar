...
浏览器授权后的原请求续接仍使用同一条通用 Class-P 主链，不为任何 provider 建立专用分支。只有 actor-owned typed postcondition 已验证一个 exact UserService，且跨 turn correlation 同时匹配 accepted continuation admission、origin/continuation turn、task、完整 operation key、action request 与 postcondition dependency 时，conversation actor 才能派发要求 `COMPLETE_ORIGINAL_SERVICE_REQUEST` 的 `NyxIdChatVerifiedAuthorizationContinuation`。该 closed protobuf 只包含强类型 action、安全 action/step/resource identity、从 typed action params 冻结的 `service_slug`、验证时间与 resume requirement；不包含 token、credential、secret 或 generic metadata bag。相同 payload 也承载不进入 Class-P 的 completion-only browser action，但这类 action 使用自身的 typed resource，并强制空 tool catalog。

`COMPLETE_ORIGINAL_SERVICE_REQUEST` 续接要求 verified resource 是具有有效 identity 的 typed `UserService`，且 `service_slug` 非空；资源类型或 frozen slug 不一致时在 LLM 前以 `NYXID_AUTHORIZATION_CONTINUATION_INVALID` fail closed。通过该结构校验后，执行器使用当前请求 credential 重新物化 request-local catalog，再按 typed `IAgentToolOperationAdmissionOwner.OperationAdmission.ServiceInstanceId == verified UserServiceId` 且 `ServiceSlug == frozen service_slug` 做精确收窄。unprofiled turn 从当前 `agent-profile.nyxid-chat` route toolset 重新发现；profiled turn 必须校验 committed profile identity，并继续受当前 `MaximumToolPolicy` 限制，但不再与 OAuth 前冻结的 turn/task authority ceiling 求交，因为新连接建立的 operation 不可能预先存在于旧 ceiling。结果只保留当前 catalog 中 final-allowed 的 route-owned operation tools；management tools、global tools、其他 UserService、相同 slug 的其他实例或相同实例的不同 slug 都不构成 fallback。没有匹配 operation、route drift、policy exclusion 或 committed profile capability 不可用时返回 restricted empty catalog，并在 LLM 前以 `NYXID_AUTHORIZATION_CONTINUATION_CAPABILITY_UNAVAILABLE` fail closed。

`COMPLETE_ORIGINAL_SERVICE_REQUEST` 的续接 instruction 只追加到本次 transient LLM step state，不进入 pending/appended history、actor state、committed event、read model、projection、metadata 或日志。模型第一次只返回文本而没有调用 exact operation 时，actor 只追加一次携带同一 action identity 与 postcondition dependency 的 `failure_recovery` LLM step；第二次仍无 tool call 时固定失败为 `NYXID_AUTHORIZATION_CONTINUATION_TOOL_REQUIRED`。专门的授权完成请求以及 `key.create`、`key.rotate` 等 completion-only browser action 使用 `COMMUNICATE_AUTHORIZATION_COMPLETION`，要求强类型 action、action-specific typed resource 以及两者的 exact match，并将 request-local catalog 强制置空、清除 transient tool authorization。该分支不调用 LLM，而是由执行器根据 actor-owned typed fact 生成确定性 terminal receipt，只输出 `KeyId` 或 `UserServiceId` 等安全资源 identity；它们不要求 `service_slug`，也不会回退到 Class-P operation。action/resource 缺失或错配以 `NYXID_AUTHORIZATION_CONTINUATION_INVALID` fail closed，无法识别的 resume requirement 以 `NYXID_AUTHORIZATION_CONTINUATION_REQUIREMENT_UNSUPPORTED` fail closed。
