...
- workflow 内部的 `self_reschedule` / `schedule_workflow` step 只向 `ScheduledDispatchGAgent` 发送幂等 ensure 命令；跨 run schedule fact 不归 workflow run actor 持有。创建不同 schedule 时，step 用当前 run 的 scope 构造 typed scope-owner NyxID source 与 mutation context；更新当前 run 所属 schedule 时省略 auth，由 schedule actor 保留其已有 typed source，禁止用本次 run 的 bearer 覆盖 schedule auth。
...
<<<<<<< HEAD
- service invocation schedule 的 required-credential 判定由统一 `IScheduledDispatchCredentialRequirementPolicy` 承担。Application 在 create/ensure/update 进入 actor 前按 typed target kind 校验；`ScheduledDispatchGAgent` 持久化 `credential_requirement_target_kind` 作为 actor-owned input classification，并在 fire/run-now 使用最终状态重新校验。`Envelope`、`StaticService`、`ScriptingService` 默认允许 no-auth；`WorkflowService` 与 `Connector` 必须带 typed service invocation credential source。Host 只负责把 HTTP body、认证 principal 与 service revision snapshot 映射成 typed config，不保留 endpoint-private binding/exchange gate；revoked binding 与 scope mismatch 仍由 downstream credential exchange fail closed。
- workflow schedule fire 的调用者凭据只通过 typed `DurableCallerCredentialRef` 跨 scheduled-dispatch 可信通道传递。需要换取 refreshable bearer 的 credential source 先把 bearer 写入 durable vault，再只传新 handle；existing vault-backed `ScheduledInvocationAgentKey` 则 exact 复用其 `SecretReference.Ref/Purpose/OwnerScopeKey + ApiKeyId` 构造 borrowed handle，不 resolve、不复制 secret、也不 put 新 secret。`ChatRequestEvent` 只携带 `caller_durable_credential` handle；`WorkflowRunGAgent` 将 handle 保存到 `WorkflowCallerCredentialState`，不把 token 写入 `connector_http_authorization`、`llm_control` 或 24 小时 run-level runtime secret。LLM/tool/connector 每次外呼统一通过 `TryGetCallerCredentialAsync` late resolve，因此 rotation、expiry、revocation 与 identity mismatch 均以 vault 当前事实 fail closed；borrowed handle 的所有权仍属于原 credential owner，workflow run 终态不得 revoke。没有 handle 的旧 run 继续走 legacy fallback，不热替换。外部 API 请求若自带 `caller_durable_credential` 必须 fail closed；projection/readmodel/log 只暴露 credential source kind。
=======
...
- workflow schedule fire 的调用者凭据只通过 typed `DurableCallerCredentialRef` 跨 scheduled-dispatch 可信通道传递。fire 时换出的 NyxID bearer 先写入 durable vault，vault record 的 expiry 必须来自 credential exchange 的到期时间；legacy durable source 的每次投影也必须设置有限 TTL。`ChatRequestEvent` 只携带 `caller_durable_credential` handle；`WorkflowRunGAgent` 将 handle 保存到 `WorkflowCallerCredentialState`，并在 completed/stopped 终态按该 actor-owned reference best-effort revoke，vault expiry 是清理失败或终态提交后崩溃时的硬上界。token 不得写入 `connector_http_authorization`、`llm_control` 或 24 小时 run-level runtime secret。LLM/tool/connector 外呼统一通过 `TryGetCallerCredentialAsync` 在调用前解析 handle；没有 handle 的旧 run 继续走 legacy fallback，不热替换。外部 API 请求若自带 `caller_durable_credential` 必须 fail closed；projection/readmodel/log 只暴露 credential source kind。
