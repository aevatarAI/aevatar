...
- workflow schedule fire 的调用者凭据只通过 typed `DurableCallerCredentialRef` 跨 scheduled-dispatch 可信通道传递。fire 时新换出的 refreshable NyxID bearer 先写入 durable vault，vault record 的 expiry 必须来自 credential exchange 的到期时间；legacy durable source 的每次投影也必须设置有限 TTL。这类新建 handle 归 run actor 所有：`WorkflowRunGAgent` 将 handle 与 caller authority 保存到 `WorkflowCallerCredentialState`，并在 completed/stopped 终态 best-effort revoke，vault expiry 是清理失败或终态提交后崩溃时的硬上界。NyxID source 同时把原始 typed subject + capability scope 写入 handle 的 `scheduled_caller_nyx_id_authority`，service-dispatch boundary 再映射为 workflow caller authority；不得从 token、`subject_id` 字符串或 Aevatar `scopeId` 反推该事实。
...
- `ChatRequestEvent` 只携带 `caller_durable_credential` handle。token 不得写入 `connector_http_authorization`、`llm_control` 或 24 小时 run-level runtime secret；caller authority 也必须从 committed projection/readmodel payload 中移除。没有 handle 的旧 run 继续走 legacy fallback，不热替换。外部 API 请求若自带 `caller_durable_credential` 必须 fail closed；projection/readmodel/log 只暴露 credential source kind。
