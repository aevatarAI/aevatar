...
| GAgent service, registry, and Agent Profiles | Capability core | Registry/scope membership facts are owned by `GAgentRegistryGAgent`; `AgentProfileNamespaceGAgent` owns Profile namespace/provisioning facts and `AgentProfileGAgent` owns one Profile draft/published authority. | Command/admission/query surfaces live in `Aevatar.GAgentService.Application*` and related abstractions; Profile Application composes exact typed ports and returns accepted receipts. | Registry and the three Profile current-state readmodels are query replicas; Profile audit consumes the same committed projection input on namespace/owner contexts only. | Infrastructure owns actor dispatch adapters; exact Ornn Profile reads stay in `Aevatar.AI.ToolProviders.Ornn` and are selected by Host. | Hosting projects register service/Profile endpoints, system Profile bootstrap/readiness, unavailable defaults, and provider replacement. | Target actor ids are opaque; scope/Profile admission uses typed authority and readmodels, not actor id parsing, runtime side reads, or reference-derived identity. |
...
| Channels and IM adapters | Provider/adapter plus capability core where conversation facts exist | Conversation/session facts are owned by the conversation actor family; platform adapters do not own inbound runtime state. | Application command/admission surfaces normalize inbound activities and proactive sends into typed commands. | Chat history, user memory, and status readmodels come from committed conversation/run facts. | Channel-specific transport/outbound/rendering adapters live under channel/platform provider projects; NyxID relay use stays at adapter boundary. | Host endpoints verify signatures, normalize payloads, and register channel adapters; they acknowledge only honest acceptance. | Inbound delivery is committed or actor-owned before ack when required; heavy LLM/tool work runs in run-scoped actors. |
...
| Agent Profile identity, management, exact publish, read models, and bootstrap | [agent-profiles.md](agent-profiles.md) |
