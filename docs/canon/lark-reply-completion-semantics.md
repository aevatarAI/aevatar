...
Lark channel 发起的 workflow 从启动到终态回投统一使用 registration-owned 长效 NyxID Agent Key：direct workflow、published service workflow、嵌套 LLM/tool/connector NyxID proxy 调用以及 channel-relay `/reply` 都沿同一个 typed vault handle 传播。用户 inbound relay token 只属于原入站 turn，不能成为 workflow caller credential、不能进入 `WorkflowRunDeliveryGAgentState`，也不能作为 Agent Key 缺失、失效或 scope 不足时的 fallback。

### 10.1 Channel Agent Key：typed vault handle + fail closed（#2675）

- **凭据来源唯一**：Lark provisioning 在创建 relay api key 时捕获 NyxID 一次性 `full_key`，立即写入分布式 `ISecretVault`（历史兼容 purpose 字面仍为 `channel.workflow-result-delivery-agent-key`，owner scope = registration scope，subject = `nyx_agent_api_key_id`）。该 secret 的实际职责是整个 channel workflow 的 Agent Key，不再只限于 terminal delivery。raw key 不进入 Protobuf state、event、read model、result 或日志；mainnet 只读 secrets store 不参与。
- **强类型 handle 贯穿链路**：`ChannelBotRegistrationEntry.workflow_result_delivery_credential`（vault `SecretReference`）→ registration read model → `AgentToolChannelContext.WorkflowResultDeliveryCredential`（`SecretReference` + subject）→ `AgentToolExecutionContext.DurableNyxIdCredential` → direct/published service dispatch → `WorkflowRunExecutionContextState.CallerCredential` → LLM/tool/connector invocation；同一 handle 还进入 `WorkflowRunBackgroundDeliveryReservation` → `WorkflowRunDeliveryGAgentState` 完成终态回投。`WorkflowRunDeliveryStartRequested` 只绑定 accepted workflow identity，不再重复承载 credential。泛化字符串 `nyx_reply_credential_ref` / `durable_reply_credential_ref` 字段已 reserved 删除。
- **启动边界只认 Agent Key**：可信 `ChannelRegistration` tool context 必须解析出 exact registration vault handle；dispatcher 会清除 raw bearer、source-readable user bearer 与可刷新 user authority，只投影 `DurableCallerCredentialSourceKind.ChannelRegistration`。direct workflow 与 published service workflow 在 dispatch 前执行同一 readiness gate；handle 缺失、descriptor 不匹配、vault resolve 失败或 Agent Key 不可用时返回 `channel_agent_key_not_ready` 并停止启动，禁止回退 5 分钟用户 token。
- **scope 统一为 `read write proxy`**：新 provisioning 直接创建 `read write proxy` Agent Key。历史 `read write` key 在 workflow dispatch 前使用**该 Agent Key 自身**读取并原地追加 `proxy` scope，更新结果未确认则 fail closed；已有 `proxy` / `proxy:*` 不做写操作。repair 在 rotate 前也先确保源 key 带 `proxy`，使旋转后的 key 保留完整 workflow 权限。
...
- **channel 后台回投 + fail closed**：channel skill 触发 workflow 且无可用 handle 时，dispatcher 在 dispatch 前返回产品级配置错误（不泄漏 credential 细节），不创建注定无法执行或丢失终态结果的 run；reservation actor 对 typed handle、command identity 与 channel target 做第二道校验。pre-dispatch `ReserveAsync` 未 accepted 时禁止 dispatch workflow；workflow command 已 accepted 后，`RegisterAsync` 只补充 accepted workflow identity，失败不得把已运行的 workflow 伪装成 tool failure，也不得 abandon reservation，而应返回 reservation-backed accepted receipt 并记录 `binding_degraded`。内部日志用 typed reason 区分 `credential_handle_missing` / `channel_agent_key_not_ready` / `resolver_unavailable` / `workflow_background_delivery_reservation_failed` / `binding_degraded`。
