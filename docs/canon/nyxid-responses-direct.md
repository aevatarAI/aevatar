...
`use_skill` 和 `ornn_search_skills` 使用当前 `/v1/*` 请求的 bearer token，经 NyxID proxy 访问 Ornn API。Aevatar 会在 NyxID proxy service slug 后追加生产 `ornn-api` 当前暴露的 Ornn API 路由（`/api/v1/skill-search` 与 `/api/v1/skills/{idOrName}/json`）。也就是说，它们看到的是这个调用者在 NyxID / Ornn 权限下可见的 skill，而不是 Aevatar 服务端的全局技能库。使用受限 NyxID API key 时，`--allowed-services` 需要同时覆盖 `aevatar`、目标 LLM service，以及 Ornn API service（默认 slug 为 `ornn-api`，可通过 `Aevatar:Ornn:NyxIdSlug` 覆盖）。
