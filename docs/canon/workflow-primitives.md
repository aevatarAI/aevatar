...
- actor 必须按 `protect material -> persist pending EXECUTION_PENDING -> install and persist authored-deadline watchdog -> dispatch off-turn` 的顺序开始调用。provider completion 通过 typed self continuation 返回；actor 同时核对 self publisher、delivery operation id、run/step/call/execution identity、attempt 和 continuation token。provider I/O 不占用 actor turn，伪造、重复、stale 或 late completion 不推进状态。
