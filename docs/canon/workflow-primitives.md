...
两条路由都必须使用 live admission 固定的 exact UserService ID。interactive `code_execute` 以 source-readable caller credential 读取结构化 inventory 并在 runtime 复核；scheduled `code_execute` 的受限 Agent Key 不读取 inventory，只能对有效 admission proof 中的 exact ID 发出 `_nyxid_via` proxy 请求，由 NyxID 继续校验 slug 与 Agent Key service allowlist。`/execute` 不回退 `/run`，managed 路由不回退 `chrono-sandbox`，任一 identity 缺失、重复或 policy 不匹配都 fail closed。`codex_exec` 只接受强类型 target；workflow 不能选择镜像、provider、Codex flags 或 sandbox/isolation 配置。

`code_execute` 的 canonical identity 必须同时具有 exact slug 与非空 `catalog_service_id`；同 slug 的 custom route 不参与候选。`credential_source` 只表达 provenance 和 access：personal route 可用，org/member route 仅在 NyxID typed inventory 明确给出 `allowed=true` 时可用，禁止再把 `personal` 当作平台能力资格。该 resolver 只服务 `code_execute`，不得用于筛选普通 connected service、`nyxid_proxy` 或 managed `codex_exec`。Workflow 不 author route selector；新 admission 自动解析 exact route，并在 v5 proof 中冻结 UserService ID、slug snapshot、catalog identity 与 contract digest。readiness 的 missing、inactive、policy mismatch、ambiguous、access denied 必须分开报告；只有持有 source-readable credential 的 runtime 才复用 resolver 复核，scheduled runtime 必须沿 proof-bound exact route 执行。
