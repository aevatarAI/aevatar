...
- 通过 connector 名称调用 NyxID proxy 或 Aevatar-native payload builder。
...
- `workflows/aevatar-native/connectors/aevatar-native-s-workflows.connectors.json`：NyxID proxy connector 与 Aevatar-native payload builder connector 模板，只包含环境变量占位符，不包含真实 secret。
- `tools/workflows/validate_aevatar_native_s_workflows.py`：静态校验脚本，验证 registry 能找到两个 workflow，artifact 不依赖 n8n runtime，不含真实 secret，NyxID ingress/connector binding 存在，Ornn/tool binding 覆盖 payload 构造步骤。

## Bootstrap 方式

最小闭环是 artifact + registry + validator：

1. NyxID 侧创建或更新 endpoint、schedule、relay webhook 或 channel bot route，把目标配置为 Aevatar workflow callback URL。
2. Aevatar 启动时把 `workflows/aevatar-native/s-workflows/` 加入 workflow definition file source。
3. Aevatar connector catalog 合并 `workflows/aevatar-native/connectors/aevatar-native-s-workflows.connectors.json` 中的 connector 模板，并在部署环境提供相应环境变量。
4. Ornn 中发布或绑定 registry 声明的 skill，例如 `budget-monitoring.variance-payload-builder` 与 `lark-onboarding-email-approval.payload-builder`。
5. 运行校验脚本：
...
当前实现没有新增 C# runtime primitive，也没有实现 n8n 兼容层。若部署环境尚未提供真正的 payload builder connector，两个 workflow artifact 仍可被 registry 和 validator 审计，但完整业务执行需要补齐 Aevatar-native payload builder service 或把对应 payload 构造能力沉淀为已存在的 Aevatar connector/tool。
