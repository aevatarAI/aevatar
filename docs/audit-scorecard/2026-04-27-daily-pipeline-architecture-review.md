...
### B2. `UserConfigGAgent` 用 bot scope，丢了 Lark 用户身份——已由 [#438](https://github.com/aevatarAI/aevatar/pull/438) 修复（composite scope 路径）
...
**当前修复（#438）**
[PR #438](https://github.com/aevatarAI/aevatar/pull/438) `fix(daily): scope GitHub username binding per Lark user` 已合并到 dev：把 user-config 的 actor key 从 `scope_id` 改成 composite key `{registrationScopeId}:lark:{senderId}`，并把 `sender_id` 一路串过 `AgentToolRequestContext`。读写两端都对齐到 composite scope。issue #436 / #437 已 close。

**结构性后续（仍是开放方向）**
issue #436 当时给了两条路：composite scope（补丁路径）vs. 引入 `LarkUserGAgent`（结构路径）。#438 选了前者——能解 bug，但 user-config 依旧只是个键变长了的 dictionary，不是一个 first-class 的 Lark 用户实体。如果未来还要给 Lark 用户挂"跨 agent ACL / 跨 agent 历史查询 / per-user 配额 / per-user 通知偏好"，composite key 会很快不够用，每加一类信息都会再长一截。

**架构上正解仍然是 channel-user-binding actor**：`LarkUserGAgent`（id = `lark:{registrationScopeId}:{senderId}`）承载所有 per-Lark-user 状态。当前不阻塞，等下一类 per-user 偏好出现时一起重构。

> 当前不另开 issue 跟踪——#438 已收 bug；结构升级在有第二类 per-user 偏好登陆时再启动。
...
| 2 | `LarkUserGAgent`（per-platform-user 实体）+ 偏好挂它（B2） | bug 已由 [#438](https://github.com/aevatarAI/aevatar/pull/438) composite scope 路径修复；结构升级仍待办（有第二类 per-user 偏好时启动） | 中 | 大 + 长期复利 | bug 闭于 #436 / #437；结构性 follow-up 暂不另开 issue |
