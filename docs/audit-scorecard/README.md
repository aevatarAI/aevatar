...
1. 方案评分模式（Architecture Plan Scoring）：
仅评估架构方案与文档质量，不以实施状态扣分。
2. 实施评分模式（Implementation Scoring）：
评估已落地代码与门禁/测试结果，可按实现缺陷扣分。
3. 若用户未特别说明且审计对象是架构文档/需求文档，默认使用“方案评分模式”。
...
6. 方案评分模式下，实施状态相关问题（代码未实现、测试未落地、CI 未接入、脚本 skip）不得扣分。
...
4. 方案评分模式下的未实施事项（代码/测试/CI/性能实测缺失）一律不扣分，仅记录为“实施风险”。
...
1. 方案评分可执行最小一致性检查（如文档一致性守卫），但命令执行结果不作为实施扣分依据。
2. 方案评分必须以文档证据为主：术语一致性、边界定义、接口合同、验收矩阵、风险与回滚策略。
3. 若文档内存在互相矛盾或不可验证的架构断层，可按方案质量扣分。
