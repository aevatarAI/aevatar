...
2. Workflow 命令侧在此骨架上提供领域特化：
   ...
   `DefaultDetachedCommandDispatchService<WorkflowChatRunRequest, ...>`（accepted-only facade） +  
...
3. Scripting 命令侧已形成同一套 CQRS 接入模型：
   `RuntimeScriptEvolutionLifecycleService`（generic interaction facade） +  
   ...
   `ScriptActorCommandPortBase<TActor>`（definition/runtime 命令端口的范型骨架）。
