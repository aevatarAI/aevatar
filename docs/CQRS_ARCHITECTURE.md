...
3. `ActorCommandTargetDispatcher<TTarget>` 通过 `IActorDispatchPort` 落地 runtime-neutral envelope 投递；`IActorRuntime` 继续负责目标 actor 的获取/创建与拓扑语义；capability 侧只保留 `IWorkflowRunInteractionService` 这类领域交互 facade。
...
1. CQRS 命令侧已统一为：
   ...
   `ICommandTargetBinder<TCommand, TTarget, TReceipt, TError>`（target 绑定） +  
...
2. Workflow 命令侧在此骨架上提供领域特化：
   ...
   `WorkflowRunInteractionService`（SSE/WS 交互 facade） +  
   `WorkflowRunDetachedDispatchService`（accepted-only facade）。
3. Projection 端口实现已拆分为：
...
4. CI 增加编排类体量守卫：关键编排类的非空行数与直接依赖数有上限，避免“重新变胖”。
