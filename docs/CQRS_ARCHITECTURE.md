...
2. Workflow 命令侧在此骨架上提供领域特化：
   ...
   `WorkflowRunDetachedDispatchService`（accepted-only facade，detach live sink 后走 durable completion） +
...
4. Projection 端口实现已拆分为：
   ...
   `WorkflowProjectionQueryReader`（查询端口实现） +  
   `EventSinkProjectionLifecyclePortBase<>`（通用 lifecycle + live-sink 基类） +  
   `ContextProjectionActivationService<WorkflowExecutionRuntimeLease, WorkflowExecutionProjectionContext, IReadOnlyList<WorkflowExecutionTopologyEdge>>`（激活） +  
   `ContextProjectionReleaseService<WorkflowExecutionRuntimeLease, WorkflowExecutionProjectionContext, IReadOnlyList<WorkflowExecutionTopologyEdge>>`（释放） +  
   ...
   `WorkflowProjectionQueryReader`（查询映射；query 直接实现 read adapter，不再复用通用 query-port 基类）。
