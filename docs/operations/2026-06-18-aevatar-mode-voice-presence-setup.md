...
| WebSocket close `1008 voice_provider_credential_unavailable` | NyxID could not mint the per-session OpenAI Realtime credential | verify the caller can proxy its `openai-realtime` service and that the service holds a valid OpenAI credential |
