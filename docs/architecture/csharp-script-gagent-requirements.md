...
| R-SG-04 | 自定义 State | 支持脚本声明状态 schema 与默认值，定义/运行分别维护 `ScriptDefinitionState` 与 `ScriptRuntimeState` | In Progress | `src/Aevatar.Scripting.Core/script_host_messages.proto`、`ScriptProtoContractsTests` | 动态 schema 演进与迁移策略未实现 |
...
| R-SG-10 | 版本治理 | 支持脚本 revision 固化回放；升级默认“新实例生效”，在位升级需显式迁移 | In Progress | `RunScriptRequestedEvent.script_revision`、`ScriptRunDomainEventCommitted.script_revision` | revision 到编译句柄绑定未实现 |
...
| R-SG-15 | 生命周期边界 | GAgent 生命周期由 Actor Runtime 管理；IOC Scope 不承担 GAgent 生命周期 | In Progress | `RuntimeGAgentFactoryPort`、`RuntimeGAgentFactoryPortTests`、现有 Runtime 架构基线 | 需补 Scope 生命周期边界集成测试 |
...
| R-SG-17 | 双 GAgent + 自包含 | 脚本定义与运行拆分为两个 GAgent；定义态持久化源码字符串并支撑回放重编译 | In Progress | `ScriptDefinitionGAgent`、`ScriptRuntimeGAgent`、`ScriptDefinitionGAgentReplayContractTests` | 回放重编译契约测试待补 |
...
7. 通用创建端口已落地: 已提供 `IGAgentFactoryPort` 与 runtime 实现，脚本创建/销毁/链接能力通过 `IActorRuntime` 受控执行。
8. 缺生命周期护栏测试: 需补“Scope 仅依赖解析，不托管 GAgent 生命周期”的显式测试证据。
9. 缺复杂业务回归样例: 尚无“多智能体+AI 集成”的端到端 TDD 用例用于长期回归。
10. 缺自包含回放闭环: 已落地定义态 `source_text/source_hash/revision`，但“仅凭定义态完成重编译回放”契约测试尚未完成。
11. 运行态语义闭环仍受限: 编译器已支持执行脚本 `Decide` 并产出领域事件，但 `ScriptRuntimeGAgent` 仍未强制“定义快照加载 -> 脚本决策 -> 事件提交 -> 状态迁移”完整闭环。
...
2. 部分完成:
动态状态 schema 迁移、观测埋点、版本治理策略、自包含回放重编译闭环仍在进行中。
3. 阻塞项:
无硬阻塞。
