...
| R-SG-16 | 复杂业务场景 TDD | 提供多智能体业务测试用例，覆盖 RoleGAgent/AI、编排分支、回放同态、投影一致性与生命周期边界 | Planned | `docs/plans/2026-03-01-multi-agent-script-ai-tdd-testcase.md` | 用例测试尚未实现 |
...
9. `ScriptBusinessScenarioTestHarness`（Application/Test）:
...
5. Business Scenario Harness -> SubmitClaimRequestedEvent -> ScriptHost -> Role/Fraud/Compliance/HumanReview Agents -> ClaimCaseReadModel
...
2. 部分完成:
状态 schema 迁移、通用创建端口、观测埋点、版本治理策略与复杂业务场景回归测试仍在进行中。
3. 阻塞项:
无硬阻塞。
