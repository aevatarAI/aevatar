# AI Agent Script Container Runtime Blueprint（Docker 语义对齐版）
...
| 你原先的直觉名词 | 推荐标准名词 | Docker 对齐语义 | 实现要求 |
|---|---|---|---|
...
| 版本号 | Tag + Digest | tag 可变引用，digest 不可变内容地址 | 生产运行必须 pin digest |

结论：后续文档与代码统一使用 `Image / Registry / Container / Exec Session / Runtime`，不再使用“定义/实例”作为主术语。
...
5. 动态 IoC 子容器与依赖隔离。
6. 与 `IRoleAgent`/`AIGAgentBase` 能力对齐。
7. 独立 API 面与读模型。
8. 安全、审计、观测、CI 门禁。
...
4. 中间层禁止 `container_id/run_id/session_id -> context` 事实态字典。
...
flowchart LR
    ...
    CP --> IMG["Image Actor"]
    CP --> CTR["Container Actor"]
    CP --> RUN["Exec Session Actor"]
    IMG --> REG["Registry"]
...
5. 审计日志：记录 image digest、container_id、run_id、caller。
...
### WP-5：API + Query + Projection（P1）
- 交付：独立 API、read model、实时输出。
- DoD：可查询 image/container/run 全状态。
...
- [ ] WP-2 Container/Exec
- [ ] WP-3 IRoleAgent 兼容层
- [ ] WP-4 Script Runtime + IOC
- [ ] WP-5 API + Query + Projection
- [ ] WP-6 治理与门禁

当前状态：文档已完成 Docker 语义重构，代码尚未开始。
