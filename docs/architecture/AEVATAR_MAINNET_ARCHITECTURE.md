...
| Domain / Core | GAgent、RoleGAgent、WorkflowGAgent、WorkflowRunGAgent、无状态原语模块系统 | `Aevatar.Foundation.Core`、`Aevatar.AI.Core`、`Aevatar.Workflow.Core` |
...
sequenceDiagram
    ...
    AppSvc->>Def: 创建 / 激活 WorkflowGAgent
    Def->>Def: BindWorkflowDefinition(workflow_yaml) → 编译/校验
    Def->>Run: 创建并绑定 WorkflowRunGAgent
    Run->>Role1: 创建子 Actor (analyst)
    Run->>Role2: 创建子 Actor (reviewer)
    Run->>Run: 发布 StartWorkflowEvent
    Run->>Role1: StepRequestEvent (llm_call)
    Role1->>LLM: 调用大模型
    LLM-->>Role1: 返回结果
    Role1->>Run: StepCompletedEvent
    Run->>Role2: StepRequestEvent (llm_call)
    Role2->>LLM: 调用大模型
    LLM-->>Role2: 返回结果
    Role2->>Run: StepCompletedEvent
    Run->>Run: WorkflowCompletedEvent
    Run-->>API: 事件流（via AGUI Projection）
    API-->>App: SSE / WebSocket 实时推送
    App-->>User: 展示结果
```

Actor 层级关系：每个 workflow definition 对应一个 `WorkflowGAgent`，每次 accepted run 对应一个 `WorkflowRunGAgent`。角色子树挂在 run actor 下，而不是 definition actor 下。在 Orleans 模式下，Actor 自动分布到集群中的不同 Silo。

```mermaid
%%{init: {"maxTextSize": 100000, "flowchart": {"useMaxWidth": false, "nodeSpacing": 10, "rankSpacing": 50}, "themeVariables": {"fontSize": "10px"}}}%%
flowchart TB
    WF["WorkflowGAgent (definition)"]
    Run["WorkflowRunGAgent (run)"]
...
1. **创建 definition actor**：Application Service 可创建或命中 `WorkflowGAgent`，作为 workflow binding 源
2. **定义绑定**：优先使用请求内 `workflow_yaml` 调用 `WorkflowGAgent.BindWorkflowDefinition(yaml)`；兼容按 `workflow_name` 从注册表加载 YAML
3. **创建 run actor**：每次 accepted run 创建独立 `WorkflowRunGAgent`，并绑定 definition snapshot
4. **Profile 注入**：Mainnet 解析请求内 `agent_profile`（role agent + connector 配置），并与平台默认配置合并后做策略校验，生成 run 级执行上下文
5. **执行**：`WorkflowRunGAgent` 持久化 run facts，并通过内部事件驱动主链推进步骤
6. **完成**：发布 `WorkflowCompletedEvent`，更新 definition 级计数并由投影/流式输出对外收敛
...
AI Native App 的编排能力直接来自 SDK 对 Mainnet 的请求能力：将 `workflow_yaml`、`agent_profile` 与输入一起提交后，Mainnet 在运行时绑定 definition actor，并创建独立 `WorkflowRunGAgent` 执行该 run。  
...
`WorkflowRunGAgent` 负责主循环语义：接收 `ChatRequestEvent` 后写入 run state，发布 `StartWorkflowEvent`，随后基于持久化的 `StepRequestEvent / StepCompletedEvent / pending facts` 决定下一步（`Next` / `Branches`），直到发布 `WorkflowCompletedEvent`。
...
flowchart TB
    subgraph core_pack ["WorkflowCoreModulePack (stateless only)"]
        ...
        subgraph supported_modules ["Factory-Registered Primitives"]
            ...
            Emit["emit"]
            Validate["workflow_yaml_validate"]
            Dynamic["dynamic_workflow"]
        end
    end

    subgraph run_owned ["WorkflowRunGAgent Owned Facts"]
        RunOwned["llm_call / evaluate / reflect / delay / wait_signal / human_* / workflow_call / parallel / foreach / map_reduce / race / while / cache"]
    end
...
flowchart TB
    ...
    subgraph cluster ["Orleans Cluster"]
        subgraph silo1 ["Silo 1 (Pod)"]
            G1["WorkflowGAgent A"]
            G2["WorkflowRunGAgent A-Run1"]
        end
        subgraph silo2 ["Silo 2 (Pod)"]
            G3["WorkflowGAgent B"]
            G4["WorkflowRunGAgent B-Run1"]
        end
        subgraph silo3 ["Silo 3 (Pod)"]
            G5["RoleGAgent A-Run1"]
            G6["RoleGAgent B-Run1"]
        end
    end
...
flowchart LR
    ...
    API --> WF["WorkflowRunGAgent / RoleGAgent"]
...
- **共置优先策略（To-Be）**：若 Runtime 层后续暴露 Orleans Placement 配置，可评估将强耦合角色优先共置到父 `WorkflowRunGAgent` 所在 Silo，降低跨 Silo 往返。
...
- **串行调度优化评估（To-Be）**：`WorkflowRunGAgent` 当前按串行语义处理事件。若后续要缩短步骤间调度间隙，应优先在 Runtime 层评估“同 run 事件的本地连续处理策略”，并在并发安全审计后再落地。
...
- **Prompt 缓存**：对确定性步骤（`evaluate`、`reflect`、`guard`）的重复迭代，相同 prompt hash 可命中缓存，跳过 LLM 调用。缓存粒度为 `hash(system_prompt + user_message + model)`，TTL 按步骤类型配置。
- **Token 预算管理**：为每个 `llm_call` 步骤设置 `max_tokens` 上限，防止模型生成超长响应导致延迟和序列化开销不可控。在 YAML `parameters` 中声明，由 `WorkflowRunGAgent` 在步骤派发时透传给目标 `RoleGAgent`。
...
当 Orleans Silo 在工作流执行中途崩溃时，承载 definition actor 或 run actor 的 `RuntimeActorGrain` 都可能在其他 Silo 上重新激活。当前恢复能力取决于 definition facts 与 run facts 是否分别持久化。

**当前实现现状（Delivered）**

- `WorkflowState` 仅持久化 definition facts 与 execution counters。
- `WorkflowRunState` 持久化 run 事实，包括 `run_id/status/variables/active_step_id` 以及 timeout/retry/delay/wait/human gate/LLM/sub-workflow 等 pending facts。
- `WorkflowRunGAgent` reactivation 后会重建编译缓存、重装无状态原语处理器，并重新发布 suspended facts。
- 因此 accepted run 在 reactivation 后仍能继续收敛，而不是退化为“上层重试整个 run”。
...
要实现“步骤级断点恢复”，系统当前依赖的持久事实包括：

| 状态字段 | 说明 | 恢复时作用 |
|---|---|---|
| `WorkflowState.workflow_yaml/workflow_name` | definition 绑定事实 | 重新激活后重建 definition snapshot |
| `WorkflowRunState.active_step_id` | 当前执行步骤 | 从最近一个稳定步骤继续推进 |
| `WorkflowRunState.variables` | 运行时变量 | 恢复后后续步骤可继续引用前序输出 |
| `WorkflowRunState.step_executions/retry_attempts` | 运行历史与 attempt | 幂等判断与重试续跑 |
| `WorkflowRunState.pending_*` | 等待中的异步事实 | 恢复后重新对账 timeout/signal/human gate/external response |
...
sequenceDiagram
    ...
    Orleans->>Silo2: 重新激活 WorkflowRunGAgent
    Silo2->>Garnet: 读取 Grain 持久态
    Garnet-->>Silo2: WorkflowRunState + Workflow binding facts
    Silo2->>Silo2: 重建角色子 Agent 树
    Silo2->>Run: 重发 suspended facts / 恢复 pending 对账
    Run->>Run: 从 active step 继续推进
```

注意事项：

- 当前 `checkpoint` 模块仍不等价于独立快照系统；恢复正确性来自 `WorkflowRunState` 持久事实，而不是 `checkpoint` 自身。
...
Mainnet 的职责是将该 YAML 绑定为 workflow definition，并按 `agent_profile` 组装 RoleGAgent 与 connector 解析上下文后创建并驱动 `WorkflowRunGAgent`。
...
| `WorkflowRunGAgent` | 合约运行实例 | 在单次 run 生命周期内维护执行上下文 |
...
sequenceDiagram
    ...
    Resolver->>Def: BindWorkflowDefinition(workflow_yaml)
    Resolver->>Run: CreateRunActor + BindDefinition + StartRun
    Run->>Chrono: connector_call (storage/notification/...)
    Chrono-->>Run: capability response
    Run->>Projection: EmitDomainEvents
...
flowchart LR
    ...
    Api --> WfAgent["WorkflowRunGAgent Instance"]
