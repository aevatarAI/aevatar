...
| 运行主语义 | 一次 Run 对应一个 `WorkflowRunGAgent`；definition 由 `WorkflowGAgent` 提供，执行与输出由 run actor 事件流推进 | 由 Trigger 触发 workflow 执行，按节点依赖推进 |
...
- 统一入口，一对多分发，不使用双轨实现。
- 运行时输出通过 `workflow-run:{runActorId}:{commandId}` 会话事件流隔离。
