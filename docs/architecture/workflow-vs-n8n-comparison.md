...
| 扩展机制 | `IWorkflowPrimitivePack`、DI 组合、executor 注册 | 自定义节点与社区节点扩展 |
...
- workflow 步骤能力通过 primitive executor 扩展，不把流程逻辑硬编码在单一 Agent 方法内。
...
| Node Type | Workflow Primitive Executor（如 `conditional`/`connector_call`）或 run-owned primitive（如 `parallel`/`workflow_call`） | 无状态原语走 `IWorkflowPrimitiveExecutor`，有状态 run 语义落 `WorkflowRunState` |
| Execution Log | Projection ReadModel + Timeline | 读侧投影统一提供查询与审计视图 |
| Custom Node | 自定义 `IWorkflowPrimitiveExecutor` / primitive pack | 扩展走同一主链路，不另起执行体系 |
