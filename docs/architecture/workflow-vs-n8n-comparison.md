...
| 运行主语义 | 一次 Run 对应一个 `WorkflowRunGAgent`；definition 由 `WorkflowGAgent` 提供，执行与输出由 run actor envelope 流推进 | 由 Trigger 触发 workflow 执行，按节点依赖推进 |
| 实体边界 | workflow definition 与 workflow run 分离；definition actor 长生命周期，run actor 单次执行单独建模 | workflow 与执行实例为平台对象，不是 actor 绑定模型 |
| 状态与一致性 | 强调 `Application Command -> EventEnvelope -> Domain Event -> Projection -> ReadModel`，统一事实源 | 强调执行记录与任务运行管理 |
...
- 一次 run 的入口语义是创建或解析 definition binding，再驱动新的 `WorkflowRunGAgent`。
- `WorkflowGAgent` 仅持有 workflow definition，不直接承担执行推进。
- `/api/chat` 返回的是 run actor id；`resume/signal` 只发送到 run actor。

### 3.2 统一投影链路

- CQRS ReadModel 与 AGUI 实时输出共享同一投影输入管线。
- 统一入口，一对多分发，不使用双轨实现。
- 运行时输出通过 `workflow-run:{runActorId}:{commandId}` 会话 envelope 流隔离。
...
| Node Type | Workflow Module（如 `llm_call`/`parallel`） | 通过模块工厂与 DI 装配 |
| Execution Log | Projection ReadModel + Timeline | 读侧投影统一提供查询与审计视图 |
| Custom Node | 自定义 `IEventModule` / 模块包 | 扩展走同一主链路，不另起执行体系 |
