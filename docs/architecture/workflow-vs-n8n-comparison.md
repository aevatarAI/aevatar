...
| 运行主语义 | 一次 Run 等价于向 `WorkflowGAgent` 触发 `ChatRequestEvent`，后续由事件流推进 | 由 Trigger 触发 workflow 执行，按节点依赖推进 |
| 实体边界 | 一个 workflow 对应一个 actor，绑定后不可切换 | workflow 与执行实例为平台对象，不是 actor 绑定模型 |
...
- 一次 run 的入口语义是触发 `WorkflowGAgent` 的 `ChatRequestEvent`，执行与输出均由事件流驱动。
