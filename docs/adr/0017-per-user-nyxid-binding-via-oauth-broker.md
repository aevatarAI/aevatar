...
- end-to-end 测试 + 灰度
- 切到生产路径
- 同步:NyxID#549 OpenAPI/proto 契约冻结(scope name 用 `urn:nyxid:scope:broker_binding`,subject_token_type 用 `urn:nyxid:params:oauth:token-type:binding-id`,`invalid_grant` 错误码语义对齐 revoked binding)前 ADR 保持 `proposed`;契约冻结并 aevatar 接入后升 `accepted`
