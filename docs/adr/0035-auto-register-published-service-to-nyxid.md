...
11. **bind opt-in 语义**：bind request 的 typed `ExposureDesired=true` 只写 canonical opt-in intent；实际注册仍由 activation committed hook 统一 reconcile。`ExposureDesired=false` 若已有 opt-in intent，则复用 retire command 关闭外部暴露。
