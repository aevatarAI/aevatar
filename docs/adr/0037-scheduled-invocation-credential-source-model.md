...
2. **Durable 降级为引用、且 internal-only**：删除「raw bearer token 持久化进 state」；改为 `DurableCredentialReference(credential_id)`，id 是 NyxID agent key 的 handle（Studio minted 本就产出有 id 的 agent key，`ScheduledAgentKeyStudioRunCredentialIssuer.cs:44`）。fire 时由 id 经 broker 换短 token，与 ADR-0033 ephemeral broker 同范式。**通用 HTTP API 不再接受 durable**——durable reference 只能由 internal trusted provisioning（Studio）写入。
...
message ScheduledServiceInvocationAuthState {
  ...
  oneof source {
    NyxIdCredentialSourceState nyx_id = 4;
    DurableCredentialReferenceState durable = 5;
  }
...
message DurableCredentialReferenceState {
...
- 与 ADR-0018 / ADR-0033 / #375 不变量对齐：零长期 secret material，fire 时短 token 重签。
...
2. **Phase 0 契约先行**：proto `oneof` 收敛 + `NyxIdCredentialSource`/role/`DurableCredentialReference`；reducer 迁移期双读旧格式；proto 重生 + reducer/replay 测试。
...
4. **Phase 2 durable 收敛**：HTTP 关闭 durable；raw token → `DurableCredentialReference(id)`；Studio minted 走 reference；迁移相关测试。
