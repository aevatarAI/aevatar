...
| Profile `attempt` | one profiled turn authority/catalog attempt, inherited by successor steps and distinct from their operation generations |
| `historyDeletionOperationId` | one stable history-deletion operation, retained across activation redrives until committed completion |
...
Each admitted turn also stores a SHA-256 digest of deterministic Protobuf input:
prompt, sanitized durable input parts, per-part hashes of valid decoded inline
media bytes, and typed non-secret execution controls. Raw inline bytes, tracing
identities, raw credentials, credential references, external metadata, and
connected-services JSON are excluded. Reusing either the turn or client
request identity requires the exact actor/scope/turn/task/client tuple and a
fixed-time digest match. A mismatch commits a typed conflict; an exact match
commits a replay snapshot for the existing observation pipeline without
redispatching work.

Terminal admissions are retained beyond the current `activeTurn` in a bounded
Actor-owned Protobuf ledger keyed by both `turnId` and `clientRequestId`.
Current and retained records are checked before active-turn steering admission.
An exact old retry projects the canonical replay fact without model/tool work;
a collision on either identity with a different counterpart or digest fails
closed. The ledger holds at most 32 records and evicts oldest-first
deterministically. Each record contains the deterministic digest and safe
terminal turn/task snapshots, but no prompt body, input parts, inline bytes,
credentials, headers, or transport extension bag. Monotonic late evidence for a
terminal task refreshes the matching record atomically without changing its
insertion position, preserving FIFO eviction while making later replay expose
the latest committed safe effect and operation facts.
...
- A Profile attempt remains fixed across successor tool/LLM steps; only an explicit retry transition advances it, independently of per-step operation generation.
...
Conversation creation/deletion is also controlled by the canonical actor. Its
closed Protobuf lifecycle commits registration, the deterministic registry Actor
and stable registry-unregistration operation identity, a stable history deletion
operation identity, and cleanup phases before their external actions. Ordinary
unregister and typed unregister receipts acknowledge dispatch only. The registry
Actor commits removal or authoritative absence before sending a typed completion
from its own direct-envelope publisher identity. The target and completion Actor
must be the same lifecycle owner. Exact duplicates redeliver the immutable
completion; changed tuples fail closed. The registry's ordered Protobuf ledger is
bounded to 256 operations, evicts oldest callbacks only after target-inbox
admission, pins rejected callbacks for redrive, and rejects all-pending overflow
before commit.

After exact registry completion, the history adapter forwards one typed request
to both deterministic history owners without consulting the read model. The
canonical Actor commits a tombstone; the legacy Actor commits a matching
tombstone or owner-scoped authoritative absence without mutating a colliding
tuple. Each completion payload must match its actual direct-envelope publisher.
`Accepted` acknowledges dispatch admission only and leaves the lifecycle
pending. `history_deleted` requires acknowledgements from both owners, and the
canonical owner cannot claim absence. Query-oriented `NotFound`, an ambiguous
exception, or any other non-accepted dispatch remains pending and
activation-redrivable with the same operation ID, without registry compensation.
Mismatched and duplicate completions fail closed or are idempotent. Activation
redrives any outstanding phase through a typed self-message, and the handler
accepts it only when the full lifecycle snapshot still equals committed state.
When both history-owner bits are already durable but aggregate completion was
interrupted, that continuation commits `history_deleted` without another owner
deletion dispatch. Successful deletion commits the final cleanup-dispatching
waterline before the runtime-neutral Actor destruction call.
...
The registry response is bounded to 1 MiB of raw bytes. The adapter checks a
declared `Content-Length`, counts streamed bytes for chunked or unknown-length
responses before decoding, and uses strict UTF-8. Oversized multibyte or invalid
UTF-8 responses fail startup.
...
The conversation actor restores task, steps, control fences, pending actions,
operation generations, and the bounded redacted turn replay ledger from
protobuf state/event history. Activation may re-dispatch only a typed
self-continuation that the committed state proves is outstanding and safe.

The same rule covers conversation resource lifecycle: activation redrives
registration, creation compensation, deletion unregister/history,
re-registration compensation, or final cleanup from the exact committed phase.
Registry and history `Accepted` receipts cannot be interpreted as completion.
Stable operation IDs, the registry Actor ID, both history-owner acknowledgement
bits, and the history accepted-dispatch count are committed Actor state. Exact
committed completions are typed Actor messages whose direct-envelope publishers
must match their payload owners. External completion cannot be inferred from
process memory, and no event handler calls another handler as an inline
continuation.
...
- same-turn digest conflict for distinct durable input and typed controls, exact active replay, exact old-turn replay after later turns/restart, retained identity conflict before steering, deterministic bounded eviction, and in-place replay refresh after late terminal evidence;
- replay records exclude prompt/input/credential material while preserving safe terminal turn/task facts;
- creation/deletion restart at every external lifecycle boundary, duplicate self-signal idempotency, admission-only registry/history ACK, exact owner-sourced completion after restart, bounded pending-aware registry operation retention, same-operation ambiguous redrive without compensation, dual-owner history acknowledgement in either order, live/pristine/colliding legacy handling, projection-miss canonical tombstoning, weak `NotFound` rejection, and mismatched payload or publisher rejection;
...
- registry revision, supported tier, verb, typed parameter validation, raw-byte limit, and strict UTF-8 failure;
