...
3. A focused execution test runs each human-approval starter through the production parser, workflow kernel, assign module, and approval module for approve, reject, and timeout decisions. LLM completion is isolated at the provider boundary so the test remains deterministic.
4. Browser verification imports YAML through `/scopes/:scopeId/workflow-activity-vnext/workflows/new`, then applies and saves each template in the created `/scopes/:scopeId/workflow-activity-vnext/workflows/:workflowId` editor. Each pass requires zero diagnostics and a matching Canvas graph. The same workflow URL must be reloaded and resolve the same name and step graph before the draft is considered durably materialized. Approval and wait-signal templates require their expected suspended state and continuation controls before browser runtime verification can be claimed.
5. The remote template browser cannot discover un-deployed local assets. Catalog visibility is verified only after a backend deployment that publishes the `workflow-templates/` assets; local browser validation must not add a mock or fallback catalog.
