...
Workflow 的 `code_execute` 是一个明确的例外：它的 durable `/executions` submit、status、result 和 cancel 请求始终使用 `Aevatar:NyxId:ApiBaseUrl`，不读取 `InternalApiBaseUrl`，也不使用 internal-to-public mutation fallback。一次 workflow step 不再持有长时间 HTTP 请求，而是持久化 operation receipt 后由 actor durable callback 发起有界的状态查询。`timeout_secs` 默认 180 秒、允许 1 到 600 秒，并由 Aevatar 在 submit 时显式发送。Workflow stop 会持久化一个不会被重投递延长的绝对取消截止时间；截止后不再启动新的 admission-ledger/provider 操作，但仍以有界 backoff 完成冻结的 terminal audit。已经到期的 callback 使用至少 1 ms 的正延迟进入 durable scheduler，只有审计事实可靠落库后才释放 stop。当前 Chrono Sandbox 部署使用单副本 ephemeral operation store：可以解除单次 HTTP 的 125 秒限制，但进程重启会丢失 operation/result，尚不提供跨副本读取。完整状态机和上线门槛见 [Sandbox Execution](docs/canon/sandbox-execution.md#durable-workflow-execution)。

2026-08-17 UTC 的生产验收确认了这条边界：Ready 的 Aevatar 镜像 `9af2a379680cad37759e0628fae9712b12283e67` 通过 public NyxID route 使用异步 Chrono Sandbox；130 秒脚本在 `timeout_secs=180` 下以 `exit_code=0` 完成，provider 记录的执行时间为 `131124 ms`，所有 submit/status/result 交换的观测最大值不超过 1 秒，same-key submit 收敛且没有 `524`。独立的 HR-01 webhook canary 只有一次基线后 delivery，Aevatar committed read model 以 `102/102` steps、`success=true` 完成，Lark Approval API 回读的真实 `PENDING` 实例与合成记录一致。公开证据只保留 operation/run/approval/delivery 的 12 位 SHA-256 指纹：`2231f328b39d`、`9c6531877445`、`043342b4a7c0`、`35362f37942c`。这证明了 edge-timeout 解耦和已部署 adapter 路径，不把 ephemeral backend 描述成重启或跨副本 durable；运维约束见 [chrono-sandbox#11](https://github.com/ChronoAIProject/chrono-sandbox/issues/11)，持久化替换目标见 [chrono-sandbox#12](https://github.com/ChronoAIProject/chrono-sandbox/issues/12)。
