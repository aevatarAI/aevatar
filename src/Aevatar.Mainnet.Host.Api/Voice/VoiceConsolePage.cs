...
//     carried in the Sec-WebSocket-Protocol header ("aevatar-bearer.<token>"), not the URL.
//     Zero-config is kept CLIENT-side: /ws/voice itself fails closed (501) when the caller's chat route
//     policy resolves no voice_attach_target (PolicyAwareVoiceEndpoints, test-pinned). Before connecting,
//     the page pre-flights GET chat-route-policy with a resolver-mirroring check; when voice resolves no
//     target it provisions through the two existing audited endpoints — POST nyxid-chat/conversations
//     (creates the default conversation agent) + PUT chat-route-policy/rules/voice-default (binds voice to
//     that actor) — polls both readmodels until materialized, then connects. VoicePresenceCapabilityAuto-
//     EnablePort enables the voice capability on the routed actor at attach, exactly as before.
//     Resource semantics of that default path: ownership = a normal caller-scope nyxid-chat conversation
//     actor (listed and deletable like any other); reuse key = the voice-default rule (present → its target
//     is reused, nothing is re-created); cleanup = the existing DELETE rule / DELETE conversation endpoints,
//     plus best-effort compensation (delete the just-created conversation) when the rule write fails.
...
// Real HTTP surfaces called: GET /api/studio/context (scope chip), GET/PUT /api/scopes/{scopeId}/chat-route-
// policy[/rules/voice-default], POST/GET/DELETE /api/scopes/{scopeId}/nyxid-chat/conversations, the enable
// POST above, and the /ws/voice WebSocket. No new backend surface is introduced by this page.
...
/* ===========================================================================
   Backend surfaces used by this page:
     - GET  /api/studio/context → {scopeId, scopeResolved, scopeSource}  (scope chip + provisioning scope)
     - GET  /api/scopes/{scopeId}/chat-route-policy → protobuf-JSON policy view, 404 when absent
            (pre-flight: does a /ws/voice connect resolve a voice_attach_target?)
     - POST /api/scopes/{scopeId}/nyxid-chat/conversations → 202 {actorId,...}
            (zero-config step 1: create the default voice conversation agent)
     - PUT  /api/scopes/{scopeId}/chat-route-policy/rules/voice-default → 202
            body: protobuf-JSON UpsertChatRouteRuleRequested binding sourceKind=VOICE to that actor
            (zero-config step 2; owner_scope is server-stamped from the URL scope)
     - GET  /api/scopes/{scopeId}/nyxid-chat/conversations  (poll until the new actor is registry-visible)
     - DELETE /api/scopes/{scopeId}/nyxid-chat/conversations/{actorId}
            (compensation: remove the just-created conversation when the rule write fails)
   ...
   /ws/voice fails closed (501) when the caller's chat route policy resolves no voice attach target, so the
   Talk flow pre-flights the policy and self-provisions the default agent on first use (all endpoints above
   already exist and carry their own scope-access guards). The default agent has no config write API, so the
   Default-agent view shows real runtime defaults read-only.
   =========================================================================== */
...
// bearer write helper for the zero-config provisioning flow. Unlike apiGet it
// never throws on HTTP errors — callers branch on {status, body} because the
// provisioning steps map statuses to honest user-facing messages.
async function apiSend(method, path, body){
  ...
  if(res.status === 401){ signOutSilent(); throw new Error("unauthorized"); }
  let parsed = null;
  try { parsed = await res.json(); } catch(e){ /* tolerate empty/non-JSON bodies */ }
...
function renderAgents(){
  ...
  v.appendChild(pageHead(
    ...
    '首次在本页发起对话时自动开通：控制台创建默认会话 agent，并写入 <code class="inline">voice-default</code> 语音路由规则；之后连接复用该规则的目标，不再重复创建。以下为运行时默认值（只读）。'));
...
/* ===========================================================================
   Zero-config voice route provisioning.

   /ws/voice resolves its attach target from the caller's chat route policy and
   fails closed (501 "Voice ForwardToModel is not supported in v1.") when the
   resolution carries no voice_attach_target — a brand-new caller has no policy
   at all. To keep the first connect zero-config, the Talk flow pre-flights the
   policy and, when voice resolves no target, provisions one through the two
   existing audited endpoints, then waits for materialization before dialing.

   Resource semantics of the default path:
     ownership — a normal caller-scope nyxid-chat conversation actor (it shows
       up in the conversation list and is deletable like any other);
     reuse key — the voice-default route rule: when it (or any rule resolving a
       voice target) exists, its target is reused and nothing is created;
     cleanup  — the existing DELETE rule / DELETE conversation endpoints; when
       the rule write fails the just-created conversation is deleted best-effort
       so a retry does not accumulate orphans.
   =========================================================================== */
const VOICE_ROUTE_RULE_ID = "voice-default";

/* Mirrors ChatRouteResolver.Matches for THIS page's connect input:
   sourceKind=VOICE, channel/commandName/contentHint/model all empty (native
   channel), toolMode=NONE. The GET view is protobuf-JSON with default values
   omitted, so an absent match field means "match anything" — same as the
   resolver's unspecified/empty semantics. A rule pinned to a non-empty
   channel/command/model can never match this page's input, so it is ignored. */
function voiceRuleMatches(m){
  if(!m) return true;
  if(m.sourceKind && m.sourceKind !== "CHAT_SOURCE_KIND_VOICE") return false;
  if(m.channel || m.commandName || m.contentHint || m.model) return false;
  if(m.toolMode && m.toolMode !== "TOOL_MODE_NONE") return false;
  return true;
}
function voiceAttachActorId(action){
  const t = action && action.forwardToModel && action.forwardToModel.toolChoiceHint
    && action.forwardToModel.toolChoiceHint.voiceAttachTarget;
  return (t && t.actorId) ? t.actorId : null;
}
/* Mirrors ChatRouteResolver.Resolve over the GET view: rules are stored (and
   returned) priority-desc, first matching rule WITH an action wins, otherwise
   defaultTarget. Returns the resolved voice target plus enough context for
   provisioning: matchedPriority lets the new rule outrank a matching-but-not-
   voice-capable rule, and rejected honours an explicit Reject (never overridden). */
function resolveVoiceRoute(policy){
  if(!policy) return { actorId:null, matchedPriority:null, rejected:false, reason:"" };
  const rules = policy.rules || [];
  for(let i=0;i<rules.length;i++){
    const r = rules[i];
    if(!voiceRuleMatches(r.match)) continue;
    const a = r.action || null;
    if(!a || (!a.forwardToModel && !a.reject)) continue; // action-less rule: resolver skips it
    const p = (typeof r.priority === "number") ? r.priority : 0;
    if(a.reject) return { actorId:null, matchedPriority:p, rejected:true, reason:(a.reject.reason||"") };
    return { actorId:voiceAttachActorId(a), matchedPriority:p, rejected:false, reason:"" };
  }
  const d = policy.defaultTarget || null;
  if(d && d.reject) return { actorId:null, matchedPriority:null, rejected:true, reason:(d.reject.reason||"") };
  return { actorId:voiceAttachActorId(d), matchedPriority:null, rejected:false, reason:"" };
}

async function ensureVoiceRoute(onStep){
  if(!state.scopeId) await loadContext();
  const scopeId = state.scopeId;
  if(!scopeId) throw new Error("scope 未解析，无法检查语音路由");
  const policyPath = "/api/scopes/"+encodeURIComponent(scopeId)+"/chat-route-policy";
  const convsPath  = "/api/scopes/"+encodeURIComponent(scopeId)+"/nyxid-chat/conversations";

  const route = resolveVoiceRoute(await apiGet(policyPath));
  if(route.rejected) throw new Error("路由策略拒绝语音接入" + (route.reason ? "：" + route.reason : ""));
  if(route.actorId) return { actorId: route.actorId, provisioned:false };

  onStep("首次使用：正在创建默认语音会话 agent…");
  const created = await apiSend("POST", convsPath);
  const actorId = (created.status === 202 && created.body && created.body.actorId) ? created.body.actorId : null;
  if(!actorId){
    const why = created.body && (created.body.detail || created.body.error);
    throw new Error("创建默认会话失败（HTTP " + created.status + (why ? " · " + why : "") + "）");
  }

  onStep("正在写入 voice-default 语音路由规则…");
  // Outrank whatever non-voice-capable rule currently matches voice input so the
  // new rule actually resolves; rules are evaluated priority-desc server-side.
  const priority = route.matchedPriority == null ? 0 : route.matchedPriority + 1;
  const rule = await apiSend("PUT", policyPath + "/rules/" + VOICE_ROUTE_RULE_ID, {
    defaultTargetIfUninitialized: { forwardToModel: {} },
    rule: {
      priority: priority,
      match: { sourceKind: "CHAT_SOURCE_KIND_VOICE" },
      action: { forwardToModel: { toolChoiceHint: { voiceAttachTarget: { actorId: actorId } } } },
      description: "Console-provisioned default voice attach target"
    }
  });
  if(rule.status !== 202){
    try { await apiSend("DELETE", convsPath + "/" + encodeURIComponent(actorId)); }
    catch(e){ console.warn("voice: orphan conversation cleanup failed", e); }
    const why = rule.body && (rule.body.detail || rule.body.error);
    throw new Error("写入语音路由规则失败（HTTP " + rule.status + (why ? " · " + why : "") + "）");
  }

  // Both writes are 202-accepted commands (honest ACK), so wait until the rule
  // resolves to the new actor AND the conversation actor is registry-visible —
  // /ws/voice reads the policy readmodel and its auto-enable probes the live actor.
  onStep("等待路由与会话物化…");
  const deadline = Date.now() + 15000;
  let ruleSeen = false, convSeen = false;
  while(Date.now() < deadline && !(ruleSeen && convSeen)){
    await new Promise(r => setTimeout(r, 500));
    if(!ruleSeen){
      try { ruleSeen = resolveVoiceRoute(await apiGet(policyPath)).actorId === actorId; }
      catch(e){ console.warn("voice: policy materialization poll failed", e); }
    }
    if(!convSeen){
      try {
        const list = await apiGet(convsPath);
        convSeen = !!(list && list.conversations && list.conversations.some(c => c.actorId === actorId));
      } catch(e){ console.warn("voice: conversation materialization poll failed", e); }
    }
  }
  if(!ruleSeen) throw new Error("语音路由规则尚未物化，请稍后重试");
  if(!convSeen) throw new Error("默认会话 agent 尚未注册完成，请稍后重试");
  return { actorId: actorId, provisioned:true };
}
...
async function startVoice(){
  if(voice.ws || voice.provisioning) return;
  const token=getToken(); if(!token){ signOutSilent(); return; }
  voice.transcript=""; voice.responseId=0; voice.playoutSeq=0;

  // Zero-config pre-flight: /ws/voice fails closed (501) when the caller's chat
  // route policy resolves no voice attach target, so resolve — and on first use
  // provision — the target BEFORE asking for the mic or dialing.
  voice.provisioning = true;
  vStatus("connecting","正在检查语音路由…");
  let routeTarget;
  try { routeTarget = await ensureVoiceRoute(d => vStatus("connecting", d)); }
  catch(e){ vStatus("error", String(e && e.message || e)); return; }
  finally { voice.provisioning = false; }
  if(voice.status !== "connecting") return; // user hit stop while provisioning
  vStatus("connecting", routeTarget.provisioned ? "已自动开通默认语音 agent · 正在请求麦克风…" : "正在请求麦克风…");
...
function renderTest(){
  ...
  if(voice.status==="error"){
    v.appendChild(banner("warn",ICON.alert,"连不上？",
      '首次使用时本页会自动开通默认语音 agent（创建默认会话并写入 <code class="inline">voice-default</code> 路由规则）；开通失败会在上方给出原因，可直接重试。'+
      '<code class="inline">/ws/voice</code> 在未配置 provider 时返回 503，路由未解析出语音目标时返回 501，会话内部异常返回 5xx。'+
