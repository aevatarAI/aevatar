...
4. interaction 路径由 `WorkflowRunCommandTargetBinder` 为 run actor 建立 projection lease 与 live sink；accepted-only 路径使用 `NoOpCommandTargetBinder`，只生成 accepted receipt，不 drain session event stream。
