...
- `WorkflowChatRunApplicationService`
  ...
  - inline `workflowYaml` 请求默认不参与自动回退，避免掩盖编排配置错误。
...
- `WorkflowRunActorResolver`
  - 解析优先级：`workflowYaml`（inline） > `workflow`（registry 名称） > 默认 workflow（`WorkflowRunBehaviorOptions.DefaultWorkflowName`，默认 `direct`）。
