...
- `fork` 入口同样收敛为标准 CQRS 命令：Host 与自动 fork coordinator 只构造 `WorkflowForkRunCommand`，由 target resolver 查询 seed、创建新 run，并由 envelope factory 把 `WorkflowChatRequestEvent.ResumeSeed` 投递给新 run actor
...
│   ├── WorkflowForkRunEnvelopeFactory.cs
