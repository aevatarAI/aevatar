...
export function validateActionRequest(payload) {
  ...
  if (value.schemaVersion !== 4 ||
      !["service.connect", "service.access_review", "key.create", "key.rotate"].includes(value.action)) {
...
export function validateActionContinuation(input, { expectedAction = null } = {}) {
  ...
  const actions = value.actions.map((inputReport) => {
    ...
    if (report.disposition === "completed") {
      const expectedVariant = ["service.connect", "service.access_review"].includes(expectedAction)
        ? "userService"
