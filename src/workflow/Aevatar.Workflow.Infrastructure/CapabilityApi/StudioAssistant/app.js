...
} from "./transport.js?v=20260817-p0-key-actions-integrated";
import {
  consumeSse,
  mergeUsage,
...
} from "./protocol.js?v=20260817-p0-key-actions-integrated";
import {
...
} from "./blocks.js?v=20260817-p0-key-actions-integrated";
import {
  actorCan,
  ...
  reduceActorEvent,
  restoreCachedAction,
} from "./actor-state.js?v=20260817-p0-key-actions-integrated";
import { describeReadinessFailure } from "./readiness.js?v=20260817-p0-key-actions-integrated";
...
function createKeyActionCard(action, { conversation = null, projection = null } = {}) {
  const request = action?.request || null;
  ...
  card.root.dataset.taskId = request.taskId;
  card.root.dataset.stepId = request.stepId;
  if (!action.conflicted) applyActorActionProof(card, action, projection);
...
function renderActionCards(entry = conversationContext || state.activeConversation) {
  ...
  for (const projection of projections) {
    ...
    for (const action of projection.actions?.values?.() || []) {
      if (!["service.connect", "service.access_review", ...KEY_ACTION_CARD_ACTIONS]
        .includes(action.action) || !action.request) {
        continue;
      }
      const key = actionEntryKey(action.request.actorId, action.actionRequestId);
      let card = entry.run.cardElements.get(key);
      if (!card) {
        card = KEY_ACTION_CARD_ACTIONS.includes(action.action)
          ? createKeyActionCard(action, { conversation: entry, projection })
          : createConnectCard(action, { conversation: entry, projection });
        if (!card) continue;
        ...
        if (action.conflicted) {
          ...
          if (KEY_ACTION_CARD_ACTIONS.includes(action.action)) {
            const report = projectedActionReport(action);
            if (report) {
              card.report = report;
...
async function reconcileActionContinuation(card, conversation) {
  ...
  return withConversationState(conversation, () => {
    ...
    if (verified) renderActionCard(card);
