...
internal sealed class WorkflowRunInternalSignalRouter
{
    ...
    public WorkflowRunInternalSignalRouter(
    ...
    public async Task<bool> TryHandleAsync(EventEnvelope envelope, CancellationToken ct)
    {
        ...
        var handlers = _registry.GetInternalSignalCandidates(envelope.Payload)
            .Where(x => x.CanHandleInternalSignal(envelope, _read))
            .ToArray();
        if (handlers.Length == 0)
            return false;
        if (handlers.Length > 1)
            throw new InvalidOperationException(
                $"Multiple workflow run capabilities matched internal signal '{envelope.Payload?.TypeUrl ?? "(none)"}'.");

        await handlers[0].HandleInternalSignalAsync(envelope, _read, _write, _effects, ct);
        return true;
    }
}
