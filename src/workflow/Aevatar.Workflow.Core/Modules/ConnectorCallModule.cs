...
public sealed class ConnectorCallModule : IEventModule<IWorkflowExecutionContext>
{
    ...
    public async Task HandleAsync(EventEnvelope envelope, IWorkflowExecutionContext ctx, CancellationToken ct)
    {
        ...
        if (response is { Success: true })
        {
            ...
            await ctx.PublishAsync(ok, BroadcastDirection.Self, ct);
            return;
        }
        ...
        if (string.Equals(onError, "continue", StringComparison.OrdinalIgnoreCase))
        {
            ...
            await ctx.PublishAsync(continued, BroadcastDirection.Self, ct);
            return;
        }
        ...
        await ctx.PublishAsync(failed, BroadcastDirection.Self, ct);
    }
        ...
        CancellationToken ct)
    {
        ...
        await ctx.PublishAsync(skipped, BroadcastDirection.Self, ct);
    }
