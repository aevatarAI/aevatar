...
public sealed class LLMCallModule : IEventModule<IWorkflowExecutionContext>
{
        ...
        ChatRequestEvent chatRequest)
    {
        ...
        var runtimeControl = BuildScrubbedLlmControlFromToolContext(toolContext);
        if (HasNarrowLlmControl(runtimeControl))
            chatRequest.LlmControl = runtimeControl.ToPayload();

        var childToolContext = ScrubCredentialContext(toolContext);
        if (HasNonControlToolContext(childToolContext))
            chatRequest.ToolContext = childToolContext.ToPayload();
    }
    ...
    private static LLMControlContext BuildScrubbedLlmControlFromToolContext(AgentToolExecutionContext toolContext) =>
        new(
            NyxIdAccessToken: null,
            NyxIdOrgToken: null,
            SenderNyxIdAccessToken: null,
    ...
    private static AgentToolExecutionContext ScrubCredentialContext(AgentToolExecutionContext toolContext) =>
        toolContext with
        {
