...
- `document_extract` 读取 artifact store 中的 typed file ref 并返回 bounded text；支持 UTF-8/PDF/DOCX 文档文本，以及最多 5 MiB 的 `image/png` / `image/jpeg` 图片文字抽取。图片抽取需要已配置且支持 image input 的 LLM provider；缺失或不支持时返回 `image_provider_unavailable`，`image/webp` 仍 fail closed。`workflow_file_submit` 只提交到固定 Lark Drive media 或 Lark approval file upload 目标，结果只包含 `file_token`/`file_code` 等 typed facts，不回显 bytes/base64。
