...
- 防重放依赖 `IWorkflowWebhookReplayStore`，生产启用但缺少 durable store 时 fail closed。当前 admission 尚未与 run terminal state 形成 lease/completed 状态机，因此 `202` 只表示 accepted，不承诺 committed、completed 或 crash-safe exactly-once；HMAC 也不提供下游写工具 credential/approval。
