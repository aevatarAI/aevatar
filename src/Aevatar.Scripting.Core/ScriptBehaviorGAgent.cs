...
public sealed class ScriptBehaviorGAgent : GAgentBase<ScriptBehaviorState>
{
        ...
        CancellationToken ct)
    {
        ...
        await EventPublisher.SendToAsync(evt.ReplyStreamId, new ScriptBehaviorBindingRespondedEvent
        {
        ...
        CancellationToken ct)
    {
        ...
        var capabilities = _capabilityFactory.Create(
            new ScriptBehaviorRuntimeCapabilityContext(
                ...
                CorrelationId: envelope.Propagation?.CorrelationId ?? ResolveRunId(envelope)),
