...
- **传递事件**：通过统一 Stream/Router 抽象把事件送到目标节点。
- **存储与去重**：在内存（或可替换的存储）里保存事件记录、路由关系与 Manifest，并做事件去重。
...
| **持久化** | `Persistence/` | 事件存储（EventStore）与 Agent 配置（Manifest）的默认实现；可替换为持久化后端。 |
...
├── Routing/             # 事件路由与层级存储
├── Persistence/         # EventStore、Manifest 与去重
...
在宿主程序中注册 Local 实现后，再向容器请求 `IActorRuntime`：
