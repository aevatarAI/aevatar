...
你不需要直接写 .NET 代码也能用 Aevatar；使用 **Aevatar.Host.Api** 的 HTTP 接口即可。Runtime 主要面向「想理解系统结构」或「要二次开发、替换实现」的读者。
...
- **只打算用 Aevatar 跑工作流、对话**：直接使用 **Aevatar.Host.Api** 的 HTTP 接口即可，无需关心 Runtime 内部。
...
services.AddAevatarRuntime(options =>
{
...
Runtime 依赖 Aevatar.Foundation.Core（Agent 基类与事件管道）、以及 .NET 的依赖注入与缓存、可观测性库。具体依赖见项目文件；部署时与 Aevatar.Host.Api 一起使用即可。
