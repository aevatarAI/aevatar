        ...
        CancellationToken ct)
    {
        ...
        if (!string.IsNullOrWhiteSpace(workflowNameForReport))
        {
            try
            {
                ...
                loggerFactory?.CreateLogger("Aevatar.Hosts.Api.Chat").LogInformation("Chat run report saved: json={JsonPath}, html={HtmlPath}", jsonPath, htmlPath);
            }
            catch (Exception ex)
            {
                var loggerFactory = http.RequestServices.GetService(typeof(ILoggerFactory)) as ILoggerFactory;
                loggerFactory?.CreateLogger("Aevatar.Hosts.Api.Chat").LogWarning(ex, "Failed to write chat run report");
            }
        }
