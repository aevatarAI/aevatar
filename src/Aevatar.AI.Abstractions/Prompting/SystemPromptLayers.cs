...
public sealed class ProfileRoutingPromptLayer
{
    public ProfileRoutingPromptLayer(
        ...
        PromptLayerBounds bounds,
        IReadOnlyList<PromptLayerDiagnostic>? diagnostics = null)
        : this(content, [], provenance, bounds, diagnostics)
    {
    }
    ...
    {
        Content = content ?? string.Empty;
        AlwaysSkillProcedures = alwaysSkillProcedures?
            .Select(static procedure => procedure ?? string.Empty)
            .ToArray() ?? [];
        var rendering = ProfilePromptLayerRenderer.Render(Content, AlwaysSkillProcedures);
        RenderedContent = rendering.Content;
        Provenance = provenance ?? throw new ArgumentNullException(nameof(provenance));
        Bounds = bounds ?? throw new ArgumentNullException(nameof(bounds));
        Diagnostics = PromptLayerValues.CopyDiagnostics(diagnostics);
        ActualUtf8Bytes = rendering.ActualUtf8Bytes;
        EstimatedTokens = rendering.EstimatedTokens;
    }

    public string Content { get; }
    public IReadOnlyList<string> AlwaysSkillProcedures { get; }
