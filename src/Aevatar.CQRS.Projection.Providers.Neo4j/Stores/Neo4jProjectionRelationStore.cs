      ...
      IAsyncDisposable
{
    ...
    private readonly ILogger<Neo4jProjectionRelationStore> _logger;
    ...
    public Neo4jProjectionRelationStore(
        Neo4jProjectionRelationStoreOptions options,
        string scope,
        string providerName = ProjectionReadModelProviderNames.Neo4j,
        ILogger<Neo4jProjectionRelationStore>? logger = null)
    {
        ...
        _nodeLabel = NormalizeLabel(options.NodeLabel, "ProjectionRelationNode");
        ...
        _logger = logger ?? NullLogger<Neo4jProjectionRelationStore>.Instance;
    ...
    public async Task UpsertNodeAsync(ProjectionRelationNode node, CancellationToken ct = default)
    {
    ...
    public async Task UpsertEdgeAsync(ProjectionRelationEdge edge, CancellationToken ct = default)
    {
    ...
    public async Task<IReadOnlyList<ProjectionRelationEdge>> GetNeighborsAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        var edges = new List<ProjectionRelationEdge>(rows.Count);
    ...
    public async Task<ProjectionRelationSubgraph> GetSubgraphAsync(
        ProjectionRelationQuery query,
        CancellationToken ct = default)
    {
        ArgumentNullException.ThrowIfNull(query);
        ...
        var rootNodeId = NormalizeToken(query.RootNodeId);
        if (scope.Length == 0 || rootNodeId.Length == 0)
            return new ProjectionRelationSubgraph();
        ...
        {
            nodes.Add(new ProjectionRelationNode
            {
                Scope = scope,
                NodeId = rootNodeId,
                NodeType = "Unknown",
    ...
    private async Task<List<ProjectionRelationNode>> GetNodesByIdsAsync(
        string scope,
        ...
        CancellationToken ct)
    {
        ...
        var rows = await ExecuteReadAsync(cypher, parameters, ct);
        var nodes = new List<ProjectionRelationNode>(rows.Count);
        foreach (var row in rows)
        {
    ...
    private ProjectionRelationEdge? BuildEdgeFromRow(string scope, IReadOnlyDictionary<string, object> row)
    {
    ...
    private string BuildNeighborCypher(ProjectionRelationDirection direction, int take)
    {
        var filter = "WHERE size($relationTypes) = 0 OR r.relationType IN $relationTypes ";
    ...
    private async Task EnsureSchemaAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            var nodeConstraintName = NormalizeConstraintName($"projection_relation_node_scope_id_{_nodeLabel}");
    ...
    private static string[] NormalizeRelationTypes(IReadOnlyList<string> relationTypes)
    {
    ...
    private Dictionary<string, string> DeserializeProperties(string payload)
    {
        ...
        catch (Exception ex)
        {
            _logger.LogWarning(
                ex,
                "Failed to deserialize relation properties payload. provider={Provider} scope={Scope}",
    ...
    private static string NormalizeConstraintName(string rawName)
    {
        ...
        var normalized = new string(chars);
        if (normalized.Length == 0)
            return "projection_relation_constraint";
        if (char.IsDigit(normalized[0]))
