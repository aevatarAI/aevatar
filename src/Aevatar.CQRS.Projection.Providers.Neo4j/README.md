...
- 不依赖任何业务域 read model。
- 通过 `IProjectionStoreRegistration<IProjectionReadModelStore<TReadModel, TKey>>` 与上层模块解耦集成（Document）。
- 通过 `IProjectionStoreRegistration<IProjectionRelationStore>` 与上层模块解耦集成（Graph）。
