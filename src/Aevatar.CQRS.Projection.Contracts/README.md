...
`Aevatar.CQRS.Projection.Contracts` 只包含 CQRS 投影通用契约。

## 包含内容

- 生命周期与编排：`IProjectionLifecycleService<,>`、`IProjectionCoordinator<,>`、`IProjectionSubscriptionRegistry<>`
- 扩展契约：`IProjectionProjector<,>`、`IProjectionEventReducer<,>`
...
3. 任何领域（WorkflowExecution、CaseDemo 等）都应依赖这些泛型契约，而不是再造别名接口层。
