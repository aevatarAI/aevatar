...
public static class SystemPromptLayerComposer
{
        ...
        ConversationContextPromptLayer? conversation)
    {
        ...
        var globalReport = AppendOptional(
            ...
            global?.Bounds,
            global?.Diagnostics);
        var profileReport = AppendProfile(builder, profile);
    ...
    private static PromptLayerCompositionReport AppendProfile(
        StringBuilder builder,
        ProfileRoutingPromptLayer? profile)
    {
        if (profile is null)
        ...
        if (string.IsNullOrWhiteSpace(profile.RenderedContent))
        {
            return new PromptLayerCompositionReport(
                included: false,
                profile.ActualUtf8Bytes,
                profile.EstimatedTokens,
                profile.Bounds,
                BoundDiagnostics(composerDiagnostic: null, profile.Diagnostics));
        }

        if (IsOverBudget(profile.ActualUtf8Bytes, profile.EstimatedTokens, profile.Bounds))
        {
            var rejected = new PromptLayerDiagnostic(
                PromptLayerDiagnosticCode.OptionalLayerRejectedOverBudget,
                $"actual_utf8_bytes={profile.ActualUtf8Bytes};max_utf8_bytes={profile.Bounds.MaxUtf8Bytes};" +
                $"estimated_tokens={profile.EstimatedTokens};max_estimated_tokens={profile.Bounds.MaxEstimatedTokens}");
            return new PromptLayerCompositionReport(
                included: false,
                profile.ActualUtf8Bytes,
                profile.EstimatedTokens,
                profile.Bounds,
                BoundDiagnostics(rejected, profile.Diagnostics));
        }

        Append(builder, profile.RenderedContent);
