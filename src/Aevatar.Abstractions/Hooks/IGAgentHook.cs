// ─────────────────────────────────────────────────────────────
// IGAgentHook - foundation-level GAgent hook contract.
//
...
public interface IGAgentHook
{
    ...
    Task OnEventHandlerStartAsync(GAgentHookContext ctx, CancellationToken ct) => Task.CompletedTask;

    /// <summary>Called after event handler execution.</summary>
    Task OnEventHandlerEndAsync(GAgentHookContext ctx, CancellationToken ct) => Task.CompletedTask;

    /// <summary>Called when event handler execution fails.</summary>
    Task OnErrorAsync(GAgentHookContext ctx, Exception ex, CancellationToken ct) => Task.CompletedTask;
}
