...
internal sealed class ScopeWorkflowCatalogueBackfillHostedService : IHostedService
{
    ...
    private async Task RunBackfillAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var serviceCatalog in serviceCatalogs)
        {
            ...
            if (!deploymentCatalogsByServiceKey.TryGetValue(serviceCatalog.Id, out var deploymentCatalog) ||
                ResolveCatalogueVisibleDeployment(deploymentCatalog) is not { } catalogueVisibleDeployment ||
    ...
    private static ServiceDeploymentReadModel? ResolveCatalogueVisibleDeployment(ServiceDeploymentCatalogReadModel deploymentCatalog)
    {
