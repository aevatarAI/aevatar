...
internal static class UserWorkflowCapabilityConventions
{
    public static string NormalizeWorkflowId(string workflowId)
    {
        var normalized = UserWorkflowCapabilityOptions.NormalizeRequired(workflowId, nameof(workflowId));
    ...
    public static ServiceIdentity BuildIdentity(
        UserWorkflowCapabilityOptions options,
        string userId,
        string workflowId) =>
        new()
        {
            ...
            Namespace = options.BuildNamespace(userId),
