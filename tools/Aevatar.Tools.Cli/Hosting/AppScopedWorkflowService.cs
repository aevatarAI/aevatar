...
public sealed class AppScopedWorkflowService
{
        ...
        CancellationToken ct = default)
    {
        ...
        var workflowId = string.IsNullOrWhiteSpace(request.WorkflowId)
            ? AppStudioEndpoints.NormalizeStudioDocumentId(workflowName, "workflow")
    ...
    private static string BuildLayoutCachePath(string scopeId, string workflowId) =>
        Path.Combine(
            ...
            $"{AppStudioEndpoints.NormalizeStudioDocumentId(scopeId, "scope")}--{AppStudioEndpoints.NormalizeStudioDocumentId(workflowId, "workflow")}.json");
