...
NEO4J_HTTP_ENDPOINT="${AEVATAR_DISTRIBUTED_SMOKE_NEO4J_HTTP_ENDPOINT:-http://127.0.0.1:${NEO4J_HTTP_PORT}}"
...
if [[ "${BOOTSTRAP_DOCKER_INFRA}" == "true" ]]; then
  ...
  wait_neo4j_query_auth "${NEO4J_HTTP_ENDPOINT}" "${NEO4J_USERNAME}" "${NEO4J_PASSWORD}"
else
  ensure_local_tcp_ports_free \
    "${LOCK_OWNER}" \
    "${HTTP_PORTS[@]}" \
