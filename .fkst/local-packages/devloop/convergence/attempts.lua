...
function C.timeout_attempt_marker(proposal_id, issue_version, state_name, round, source_ref)
  ...
  return '<!-- fkst:github-devloop:timeout-attempt:v1 proposal="' .. safe_attr(proposal_id, devloop_base._max_key_len)
    .. '" version="' .. safe_attr(lineage_version, devloop_base._max_dedup_len)
    .. '" state="' .. safe_attr(state_name, max_attr_len)
    .. '" round="' .. tostring(n)
    .. '" dedup="' .. safe_attr("timeout-attempt:" .. tostring(lineage_version) .. "/" .. tostring(state_name) .. "/" .. tostring(n), devloop_base._max_dedup_len)
    .. '" source_ref_kind="' .. safe_attr(normalized.kind or "", max_attr_len)
    .. '" source_ref="' .. safe_attr(normalized.ref or "", devloop_base._max_key_len)
    .. '" -->'
end
...
function C.timeout_attempt_v2_marker(proposal_id, state_name, liveness_class_id, generation_key, round, source_ref)
  ...
  return '<!-- fkst:github-devloop:timeout-attempt:v2 proposal="' .. safe_attr(proposal_id, devloop_base._max_key_len)
    ...
    .. '" generation_key="' .. safe_attr(generation_key, devloop_base._max_dedup_len)
    .. '" round="' .. tostring(n)
    .. '" dedup="' .. safe_attr("timeout-attempt:v2:" .. tostring(state_name) .. "/" .. tostring(liveness_class_id) .. "/" .. tostring(generation_key) .. "/" .. tostring(n), devloop_base._max_dedup_len)
    .. '" source_ref_kind="' .. safe_attr(normalized.kind or "", max_attr_len)
    .. '" source_ref="' .. safe_attr(normalized.ref or "", devloop_base._max_key_len)
    .. '" -->'
end
...
function C.timeout_attempt_latest_marker(proposal_id, state_name, liveness_class_id, generation_key)
  return '<!-- fkst:github-devloop:timeout-attempt:latest:v1 proposal="' .. safe_attr(proposal_id, devloop_base._max_key_len)
    ...
    .. '" generation_key="' .. safe_attr(generation_key or "", devloop_base._max_dedup_len)
    .. '" -->'
end
...
function C.decompose_exhausted_marker(proposal_id, issue_version, round, source_ref)
  ...
  local normalized = base_ids.normalize_source_ref(source_ref)
  local lineage_version = transition_version.strip_suffixes(issue_version)
  return '<!-- fkst:github-devloop:decompose-exhausted:v1 proposal="' .. safe_attr(proposal_id, devloop_base._max_key_len)
    .. '" version="' .. safe_attr(lineage_version, devloop_base._max_dedup_len)
    .. '" round="' .. tostring(n)
