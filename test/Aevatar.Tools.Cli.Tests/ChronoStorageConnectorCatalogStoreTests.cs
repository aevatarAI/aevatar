...
public sealed class ChronoStorageConnectorCatalogStoreTests
{
    ...
    [Fact]
    public async Task GetConnectorCatalogAsync_WhenRemoteCatalogMissing_ShouldImportLocalCatalog()
    {
        using var workspaceRoot = new TemporaryDirectory();
        var localStore = new InMemoryStudioWorkspaceStore
        {
        ...
        catalog.Connectors.Should().BeEquivalentTo(localStore.ConnectorCatalog.Connectors);
        storageServer.Objects.Should().ContainSingle();
    }
    ...
    private static ChronoStorageConnectorCatalogStore CreateStore(
        ...
        new(
            ...
            {
                Enabled = true,
                BaseUrl = "https://chrono-storage.test",
            ...
            NullLogger<ChronoStorageConnectorCatalogStore>.Instance);
