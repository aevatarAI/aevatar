...
""";
    ...
    [Fact]
    public async Task IssueBridgeCallbackTokenAsync_ShouldSerializeRequestAndParseResponse()
    {
        var client = CreateClient(async (request, ct) =>
        {
            request.Method.Should().Be(HttpMethod.Post);
            request.RequestUri?.AbsolutePath.Should().Be("/api/bridge/callback-token");
            ...
            doc.RootElement.GetProperty("runId").GetString().Should().Be("run-1");
            doc.RootElement.GetProperty("stepId").GetString().Should().Be("wait-1");
            doc.RootElement.GetProperty("signalName").GetString().Should().Be("openclaw_reply");

            return new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new StringContent(
                    """{"token":"tok","tokenId":"tid","bridgeActorId":"bridge:default","actorId":"actor-1","runId":"run-1","stepId":"wait-1","signalName":"openclaw_reply","issuedAtUnixTimeMs":10,"expiresAtUnixTimeMs":20}""",
                    Encoding.UTF8,
                    "application/json"),
            };
        });

        var result = await client.IssueBridgeCallbackTokenAsync(new BridgeCallbackTokenIssueRequest
        {
            ActorId = "actor-1",
            RunId = "run-1",
            StepId = "wait-1",
        ...
        result.TokenId.Should().Be("tid");
        result.BridgeActorId.Should().Be("bridge:default");
    }

    [Fact]
    public async Task PostBridgeCallbackAsync_ShouldSerializeRequestAndParseResponse()
    {
        var client = CreateClient(async (request, ct) =>
        {
            request.Method.Should().Be(HttpMethod.Post);
            request.RequestUri?.AbsolutePath.Should().Be("/api/bridge/callbacks");

            var body = await request.Content!.ReadAsStringAsync(ct);
            using var doc = JsonDocument.Parse(body);
            doc.RootElement.GetProperty("callbackToken").GetString().Should().Be("tok");
            doc.RootElement.GetProperty("source").GetString().Should().Be("telegram.openclaw");
            doc.RootElement.GetProperty("payload").GetString().Should().Be("ok");

            return new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new StringContent(
                    """{"accepted":true,"commandId":"cmd-1","bridgeActorId":"bridge:default"}""",
                    Encoding.UTF8,
                    "application/json"),
            };
        });

        var result = await client.PostBridgeCallbackAsync(new BridgeIngressRequest
        {
            CallbackToken = "tok",
