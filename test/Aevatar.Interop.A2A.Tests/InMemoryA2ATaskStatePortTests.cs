...
public class InMemoryA2ATaskStatePortTests
{
    ...
    {
        var task = await _taskState.CreateTaskAsync("t1", "s1", MakeMessage("hello"));
    ...
    {
        await _taskState.CreateTaskAsync("t1", null, MakeMessage("a"));

        var act = () => _taskState.CreateTaskAsync("t1", null, MakeMessage("b"));
    ...
    {
        await _taskState.CreateTaskAsync("t1", null, MakeMessage("hello"));
        var task = await _taskState.GetTaskAsync("t1");
    ...
    {
        var task = await _taskState.GetTaskAsync("missing");
        task.Should().BeNull();
    }
    ...
    public async Task UpdateTaskState_ChangesState()
    {
        ...
        var updated = await _taskState.UpdateTaskStateAsync("t1", TaskState.Working);
        updated.Status.State.Should().Be(TaskState.Working);

        var agentMsg = MakeMessage("Done!");
        var completed = await _taskState.UpdateTaskStateAsync("t1", TaskState.Completed, new Message
        {
            Role = "agent",
    ...
    {
        var act = () => _taskState.UpdateTaskStateAsync("missing", TaskState.Working);
    ...
    public async Task AddArtifact_AppendsToList()
    {
        ...
        var task = await _taskState.AddArtifactAsync("t1", artifact);
    ...
    {
        await _taskState.CreateTaskAsync("t1", null, MakeMessage("hello"));
        var deleted = await _taskState.DeleteTaskAsync("t1");
    ...
    {
        var deleted = await _taskState.DeleteTaskAsync("missing");
    ...
    {
        await _taskState.CreateTaskAsync("t1", null, MakeMessage("hello"));
        var reader = _taskState.SubscribeAsync("t1");

        await _taskState.UpdateTaskStateAsync("t1", TaskState.Working);
    ...
    public async Task Subscribe_FinalUpdate_CompletesChannel()
    {
        ...
        await _taskState.UpdateTaskStateAsync("t1", TaskState.Completed);
    ...
    {
        await _taskState.CreateTaskAsync("t1", null, MakeMessage("hello"));
        var reader1 = _taskState.SubscribeAsync("t1");
        var reader2 = _taskState.SubscribeAsync("t1");
    ...
    public async Task Subscribe_ArtifactUpdate_IncludesArtifact()
    {
        ...
        await _taskState.AddArtifactAsync("t1", artifact);
    ...
    public async Task UpdateTaskState_WithMessage_AppendsToHistory()
    {
        ...
        await _taskState.UpdateTaskStateAsync("t1", TaskState.Working, agentMsg);

        var agentMsg2 = new Message { Role = "agent", Parts = [new TextPart { Text = "Reply 2" }] };
        await _taskState.UpdateTaskStateAsync("t1", TaskState.Completed, agentMsg2);
    ...
    {
        await _taskState.CreateTaskAsync("t1", null, MakeMessage("hello"));
        await _taskState.AddArtifactAsync("t1", new Artifact { Parts = [new TextPart { Text = "a" }], Index = 0 });
        await _taskState.AddArtifactAsync("t1", new Artifact { Parts = [new TextPart { Text = "b" }], Index = 1 });
    ...
    {
        var act = () => _taskState.AddArtifactAsync("missing", new Artifact
        {
