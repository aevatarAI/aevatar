...
{
    [Fact]
    public async Task HandleListWorkflowCapabilitiesAsync_ShouldReturnExactSanitizedDescriptors()
    {
        const string bearer = "transient-capability-bearer";
        var http = CreateHttpContext();
        http.Request.Headers.Authorization = $"Bearer {bearer}";
        var listPort = new RecordingExternalWorkflowCapabilityListPort
        {
            Result = new ExternalWorkflowCapabilityDiscoveryResult
            {
                CandidateCount = 2,
                RejectedCount = 1,
                Capabilities =
                {
                    new ExternalWorkflowCapabilityDescriptor
                    {
                        ...
                        Source = new ExternalCapabilitySourceStamp
                        {
                            ...
                            FreshUntil = Timestamp.FromDateTimeOffset(
                                DateTimeOffset.Parse("2026-09-02T08:05:00Z")),
                        },
                    },
                },
                Diagnostics =
                {
                    new ExternalCapabilityDiscoveryDiagnostic
                    {
                        Code = ExternalCapabilityDiscoveryDiagnosticCode.UnsupportedSchema,
                        SafeMessage = "One operation was omitted because its schema is unsupported.",
                        Count = 1,
                    },
                },
            },
        ...
        var result = await ScopeWorkflowEndpoints.HandleListWorkflowCapabilitiesAsync(
            http,
            "user-1",
            listPort,
        ...
        body.Should().Contain("\"kind\":\"nyxid_operation\"");
        body.Should().Contain("\"safeMessage\":\"One operation was omitted because its schema is unsupported.\"");
        body.Should().NotContain(bearer);
        body.ToLowerInvariant().Should().NotContain("credential");
        body.ToLowerInvariant().Should().NotContain("deprecated");
    }

    [Fact]
    public async Task HandleListWorkflowCapabilitiesAsync_ShouldRejectMismatchedScopeBeforeListing()
    {
    ...
    [Fact]
    public async Task HandleWorkflowCapabilityReadinessAsync_ShouldReturnTypedOperationContract()
    {
        var http = CreateHttpContext();
        var readinessPort = new RecordingExternalWorkflowCapabilityReadinessPort
        {
            Result = BuildReadyExternalCapability(),
        };

        var result = await ScopeWorkflowEndpoints.HandleWorkflowCapabilityReadinessAsync(
            http,
            "user-1",
        ...
        readinessPort.Request.Selector.NyxIdOperation.UserServiceId.Should().Be("us-posthog-alpha");
        readinessPort.Request.Selector.NyxIdOperation.EndpointId.Should().Be("update-dashboard");
        body.Should().Contain("\"status\":\"ready\"");
        body.Should().Contain("\"httpMethod\":\"PATCH\"");
        body.Should().Contain("\"location\":\"path\"");
        body.Should().Contain("\"valueKind\":\"string\"");
        body.Should().Contain("\"allowedValues\":[\"draft\",\"live\"]");
        body.Should().Contain("\"risk\":\"write\"");
        body.Should().Contain("\"approval\":\"required\"");
        body.Should().Contain("\"safeMessage\":\"Reconnect the PostHog service.\"");
        body.Should().Contain("\"label\":\"Reconnect PostHog\"");
    }

    [Theory]
    [InlineData("", "us-posthog-alpha", "update-dashboard", "interactive")]
    [InlineData("nyxid_request", "us-posthog-alpha", "update-dashboard", "interactive")]
    [InlineData("nyxid_operation", "", "update-dashboard", "interactive")]
    [InlineData("nyxid_operation", "us-posthog-alpha", "", "interactive")]
    [InlineData("nyxid_operation", "us-posthog-alpha", "update-dashboard", "background")]
    public async Task HandleWorkflowCapabilityReadinessAsync_ShouldRejectInvalidSelectorBeforeInspection(
        string kind,
        ...
        string executionMode)
    {
        ...
        var result = await ScopeWorkflowEndpoints.HandleWorkflowCapabilityReadinessAsync(
            ...
            "user-1",
            new WorkflowCapabilityReadinessHttpRequest(
                new WorkflowCapabilitySelectorHttpRequest(kind, userServiceId, endpointId),
                executionMode),
            readinessPort,
            CancellationToken.None);
        ...
        readinessPort.Request.Should().BeNull();
    }
    ...
    private static ExternalCapabilityReadiness BuildReadyExternalCapability()
    {
        var stringSchema = new NyxIdOperationSchema
        {
            ValueKind = NyxIdOperationValueKind.String,
        ...
        return new ExternalCapabilityReadiness
        {
            ...
            SelectedCapability = new ExternalWorkflowCapabilityRef
            {
                NyxIdUserService = new NyxIdUserServiceCapabilityRef
                {
                    UserServiceId = "us-posthog-alpha",
                    EndpointId = "update-dashboard",
                    ServiceSlugSnapshot = "posthog",
                    HttpMethod = "PATCH",
                    PathTemplate = "/api/projects/{project_id}/dashboards/{dashboard_id}",
                    Parameters =
                    {
                        new NyxIdOperationParameterContract
                        {
                            Name = "dashboard_id",
                            Location = NyxIdOperationParameterLocation.Path,
                            Required = true,
                            Schema = stringSchema,
                        },
                    },
                    RequestBody = new NyxIdOperationRequestBodyContract
                    {
                        Required = true,
                        MediaType = "application/json",
                        Schema = new NyxIdOperationSchema
                        {
                            ValueKind = NyxIdOperationValueKind.Object,
                            AdditionalPropertiesAllowed = false,
                            RequiredProperties = { "name" },
                            Properties =
                            {
                                new NyxIdOperationSchemaProperty
                                {
                                    Name = "name",
                                    Schema = new NyxIdOperationSchema
                                    {
    ...
    private sealed class RecordingExternalWorkflowCapabilityListPort :
        IExternalWorkflowCapabilityListPort
    {
        public ListExternalWorkflowCapabilitiesRequest? Request { get; private set; }

        public ExternalWorkflowCapabilityDiscoveryResult Result { get; init; } = new();
    ...
    private sealed class RecordingExternalWorkflowCapabilityReadinessPort :
        IExternalWorkflowCapabilityReadinessPort
    {
        public InspectExternalWorkflowCapabilityReadinessRequest? Request { get; private set; }

        public ExternalCapabilityReadiness Result { get; init; } = new();
