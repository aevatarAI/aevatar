...
public sealed class UserWorkflowApplicationServicesTests
{
    [Fact]
    public async Task UpsertAsync_ShouldCreateWorkflowServiceRevisionAndActivation()
    {
        var options = new UserWorkflowCapabilityOptions
        {
            TenantId = "tenant-a",
            AppId = "workflow-app",
        ...
        var queryService = new UserWorkflowQueryApplicationService(
            queryPort,
            new FakeWorkflowActorBindingReader(),
            Options.Create(options));
        var service = new UserWorkflowCommandApplicationService(
            commandPort,
            queryPort,
            queryService,
            Options.Create(options));

        var result = await service.UpsertAsync(new UserWorkflowUpsertRequest(
            "external-user-1",
            "approval-flow",
            "name: approval",
        ...
        result.Workflow.UserId.Should().Be("external-user-1");
    ...
    public async Task ListAsync_ShouldQueryUserScopeAndEnrichWorkflowNameFromBinding()
    {
        ...
        var service = new UserWorkflowQueryApplicationService(
            queryPort,
            bindingReader,
            Options.Create(options));
