...
public sealed class MainnetAgentProfileRolloutSelectorTests
{
    ...
    [Fact]
    public void Enabled_selector_should_define_the_host_owned_composite_route_tool_set()
    {
        using var artifact = ProfileArtifactScope.Create(BuildValidProfileJson());
        var selector = CreateSelector(new Dictionary<string, string?>
        {
        ...
        var registryOptions = new ToolSetRegistryOptions();

        selector.AddReviewedRouteToolSet(registryOptions, ToolSetNames.WorkspaceDefault);

        registryOptions.Registrations.Should().ContainSingle().Which.Should().BeEquivalentTo(
            new
            {
                Name = "profile.route.v1",
                IncludeToolSetNames = new[]
                {
    ...
    internal static string BuildValidProfileJson(byte policyHashByte = 1)
    {
        ...
        profile.MaximumToolPolicy.ToolSetRefs.Add(ToolSetNames.NyxIdConnectedServices);
