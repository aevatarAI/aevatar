...
public class ScriptEvolutionSessionGAgentTests
{
    ...
    [Fact]
    public async Task QueryDecision_ShouldRespondFromSessionState_WhenCompleted()
    {
        var publisher = new RecordingEventPublisher();
        var agent = CreateAgent(
            publisher,
        ...
        response.ProposalId.Should().Be("proposal-query");
        response.Status.Should().Be(ScriptEvolutionStatuses.Promoted);
        response.DefinitionActorId.Should().Be("definition-2");
        response.Diagnostics.Should().ContainSingle(x => x == "compile-ok");
    }

    [Fact]
    public async Task QueryDecision_ShouldReturnNotFound_WhenSessionNotCompleted()
    {
        var publisher = new RecordingEventPublisher();
        var agent = CreateAgent(
            publisher,
    ...
    [Fact]
    public async Task QueryDecision_ShouldReturnNotFound_WhenProposalIdIsMissing()
    {
        var publisher = new RecordingEventPublisher();
        var agent = CreateAgent(
            publisher,
        ...
        var response = publisher.Sent[0].Payload.Should().BeOfType<ScriptEvolutionDecisionRespondedEvent>().Subject;
        response.Found.Should().BeFalse();
        response.FailureReason.Should().Be("ProposalId is required.");
    }

    [Fact]
    public async Task QueryDecision_ShouldIgnore_WhenReplyRoutingIsMissing()
    {
        var publisher = new RecordingEventPublisher();
        var agent = CreateAgent(
            publisher,
