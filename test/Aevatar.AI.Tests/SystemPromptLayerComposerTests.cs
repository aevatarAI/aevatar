...
public sealed class SystemPromptLayerComposerTests
{
    ...
    [Fact]
    public void ProfileLayer_ShouldMeasureExactCanonicalRenderingAndDefensivelyCopyProcedures()
    {
        var boundary = AgentProfileExecutionBindingCodecTests.BuildRenderedProfileLayerBoundary(0);
        var procedures = boundary.AlwaysProcedures.ToArray();
        var layer = new ProfileRoutingPromptLayer(
            boundary.ProfileInstructions,
            procedures,
        ...
        procedures[0] = "mutated after construction";

        layer.AlwaysSkillProcedures.Should().Equal(boundary.AlwaysProcedures);
        layer.ActualUtf8Bytes.Should()
            .Be(AgentProfileExecutionBindingLimits.MaterializedProfileLayerMaxUtf8Bytes);
        layer.ActualUtf8Bytes.Should().Be(Encoding.UTF8.GetByteCount(boundary.Rendered));
        layer.EstimatedTokens.Should().Be((layer.ActualUtf8Bytes + 3) / 4);
    }

    [Fact]
    public void Compose_ProfileAtExactRenderedBound_ShouldIncludeCanonicalOutputAndAccurateReport()
    {
        var boundary = AgentProfileExecutionBindingCodecTests.BuildRenderedProfileLayerBoundary(0);
        var profile = new ProfileRoutingPromptLayer(
            boundary.ProfileInstructions,
            boundary.AlwaysProcedures,
            new ProfileRoutingPromptProvenance("profile"),
            new PromptLayerBounds(
                AgentProfileExecutionBindingLimits.MaterializedProfileLayerMaxUtf8Bytes,
                AgentProfileExecutionBindingLimits.MaterializedProfileLayerMaxEstimatedTokens));
        ...
        result.Profile.Included.Should().BeTrue();
        result.Profile.ActualUtf8Bytes.Should()
            .Be(AgentProfileExecutionBindingLimits.MaterializedProfileLayerMaxUtf8Bytes);
        result.Prompt.Should().Be($"kernel\n\nfloor\n\n{boundary.Rendered}");
    }

    [Fact]
    public void Compose_ProfileOneRenderedByteOverBound_ShouldRejectTheWholeProfileLayer()
    {
        var boundary = AgentProfileExecutionBindingCodecTests.BuildRenderedProfileLayerBoundary(1);
        var profile = new ProfileRoutingPromptLayer(
            boundary.ProfileInstructions,
            boundary.AlwaysProcedures,
            new ProfileRoutingPromptProvenance("profile"),
        ...
        result.Profile.Included.Should().BeFalse();
        result.Profile.ActualUtf8Bytes.Should()
            .Be(AgentProfileExecutionBindingLimits.MaterializedProfileLayerMaxUtf8Bytes + 1);
        result.Profile.Diagnostics.Should().ContainSingle(diagnostic =>
