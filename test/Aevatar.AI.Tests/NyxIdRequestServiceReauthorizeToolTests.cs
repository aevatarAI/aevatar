...
public sealed class NyxIdRequestServiceReauthorizeToolTests
{
    ...
    [Theory]
    [InlineData("""{"blocked":true,"action":1,"user_service_id":"service-alpha","requested_scopes":["repo"],"reason_code":"NYXID_SERVICE_REAUTHORIZATION_REQUIRED","safe_message":"Re-authorize the exact connected NyxID service in the secure browser action."}""")]
    [InlineData("""{"blocked":true,"action":"service.reauthorize","user_service_id":1,"requested_scopes":["repo"],"reason_code":"NYXID_SERVICE_REAUTHORIZATION_REQUIRED","safe_message":"Re-authorize the exact connected NyxID service in the secure browser action."}""")]
    [InlineData("""{"blocked":true,"action":"service.reauthorize","user_service_id":"service-alpha","requested_scopes":[1],"reason_code":"NYXID_SERVICE_REAUTHORIZATION_REQUIRED","safe_message":"Re-authorize the exact connected NyxID service in the secure browser action."}""")]
    [InlineData("""{"blocked":true,"action":"service.reauthorize","user_service_id":"service-alpha","requested_scopes":["repo"],"reason_code":1,"safe_message":"Re-authorize the exact connected NyxID service in the secure browser action."}""")]
    [InlineData("""{"blocked":true,"action":"service.reauthorize","user_service_id":"service-alpha","requested_scopes":["repo"],"reason_code":"NYXID_SERVICE_REAUTHORIZATION_REQUIRED","safe_message":1}""")]
    public void CreateResultReceipt_ShouldRejectWrongTypedResultFields(string result)
    {
        var tool = CreateTool(new StubHandler(ActiveServiceAlpha));
        const string arguments =
            """{"user_service_id":"service-alpha","requested_scopes":["repo"]}""";
