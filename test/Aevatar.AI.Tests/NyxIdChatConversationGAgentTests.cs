...
public sealed class NyxIdChatConversationGAgentTests
{
    ...
    [Fact]
    public async Task StartTurn_SameIdentityWithDifferentDurableInputPart_ShouldCommitConflictWithoutRedispatch()
    {
        const string conversationActorId = "conversation-alpha";
        var operations = new List<string>();
        var eventStore = new InMemoryEventStoreForTests();
        ...
        await agent.ActivateAsync();
        var first = CreateStartTurnCommand();
        first.InputParts.Add(CreateDurableImagePart("image-alpha", "hash-alpha"));
        ...
        conflicting.InputParts.Add(CreateDurableImagePart("image-beta", "hash-beta"));

        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, conflicting));

        dispatch.Calls.Should().ContainSingle("a conflicting replay must not execute again");
        var committed = await eventStore.GetEventsAsync(conversationActorId);
        committed.Should().HaveCount(3);
        committed[^1].EventData.Is(NyxIdChatTurnAdmissionRejectedEvent.Descriptor).Should().BeTrue();
        committed[^1].EventData.Unpack<NyxIdChatTurnAdmissionRejectedEvent>()
            .ReasonCode.Should().Be("NYXID_CHAT_TURN_IDEMPOTENCY_CONFLICT");
    }

    [Fact]
    public async Task StartTurn_SameIdentityWithDifferentInlineImageBytes_ShouldCommitConflictWithoutRedispatch()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        ...
        await agent.ActivateAsync();
        var first = CreateStartTurnCommand();
        first.InputParts.Add(CreateInlineImagePart("aW1hZ2UtYWxwaGE="));
        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, first));
        var conflicting = first.Clone();
        conflicting.InputParts[0] = CreateInlineImagePart("aW1hZ2UtYmV0YQ==");

        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, conflicting));

        dispatch.Calls.Should().ContainSingle("different inline bytes must not replay or execute");
        var committed = await eventStore.GetEventsAsync(conversationActorId);
        committed.Should().HaveCount(3);
        committed[^1].EventData.Unpack<NyxIdChatTurnAdmissionRejectedEvent>()
            .ReasonCode.Should().Be("NYXID_CHAT_TURN_IDEMPOTENCY_CONFLICT");
    ...
    [Fact]
    public async Task StartTurn_SameInlineImageBytesWithEquivalentBase64_ShouldCommitReplay()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        var dispatch = new RecordingActorDispatchPort([], static (_, _) => Task.CompletedTask);
        ...
        replay.InputParts[0] = CreateInlineImagePart("\naW1hZ2UtYWxwaGE=\r\n");

        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, replay));

        dispatch.Calls.Should().ContainSingle("equivalent decoded bytes must execute once");
        var committed = await eventStore.GetEventsAsync(conversationActorId);
        committed.Should().HaveCount(3);
        committed[^1].EventData.Is(NyxIdChatTurnReplayCommittedEvent.Descriptor).Should().BeTrue();
    }

    [Fact]
    public async Task StartTurn_ExactDuplicateWhileActive_ShouldCommitObservableReplayWithoutRedispatch()
    {
        const string conversationActorId = "conversation-alpha";
        var operations = new List<string>();
        var eventStore = new InMemoryEventStoreForTests();
        ...
        await agent.ActivateAsync();
        var command = CreateStartTurnCommand();
        command.InputParts.Add(CreateDurableImagePart("image-alpha", "hash-alpha"));
        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, command));

        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, command.Clone()));

        dispatch.Calls.Should().ContainSingle("an exact active replay must not execute again");
        var committed = await eventStore.GetEventsAsync(conversationActorId);
        committed.Should().HaveCount(3);
        committed[^1].EventData.TypeUrl.Should().EndWith(
            "/aevatar.gagents.nyxid_chat.NyxIdChatTurnReplayCommittedEvent");
    }

    [Fact]
    public async Task StartTurn_ExactDuplicateAfterTerminalRestart_ShouldCommitObservableTerminalReplay()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        var firstOperations = new List<string>();
        ...
        var command = CreateStartTurnCommand();
        command.InputParts.Add(CreateDurableImagePart("image-alpha", "hash-alpha"));
        var first = CreateController(services, conversationActorId, firstDispatch);
        await first.ActivateAsync();
        await first.HandleEventAsync(CreateEnvelope(conversationActorId, command));
        var operationKey = first.State.ActiveTask.Steps.Single().Operation.Key.Clone();
        await first.HandleEventAsync(CreateEnvelope(conversationActorId, new NyxIdChatOperationResultSignal
        {
            Key = operationKey,
        ...
        first.State.ActiveTurn.Status.Should().Be(NyxIdChatTurnStatus.Succeeded);
        var beforeReplay = (await eventStore.GetEventsAsync(conversationActorId)).Count;
        var replayOperations = new List<string>();
        var replayDispatch = new RecordingActorDispatchPort(
            replayOperations,
            static (_, _) => Task.CompletedTask);
        var replayed = CreateController(services, conversationActorId, replayDispatch);
        await replayed.ActivateAsync();

        await replayed.HandleEventAsync(CreateEnvelope(conversationActorId, command.Clone()));

        replayDispatch.Calls.Should().BeEmpty("terminal replay must not repeat model or tool work");
        var committed = await eventStore.GetEventsAsync(conversationActorId);
        committed.Should().HaveCount(beforeReplay + 1);
        committed[^1].EventData.TypeUrl.Should().EndWith(
            "/aevatar.gagents.nyxid_chat.NyxIdChatTurnReplayCommittedEvent");
    }

    [Fact]
    public async Task StartTurn_ExactOldTurnAfterLaterTerminalTurnAndRestart_ShouldReplayWithoutRedispatch()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        ...
        var turnAlpha = CreateStartTurnCommand("alpha");
        turnAlpha.InputParts.Add(CreateDurableImagePart("image-alpha", "hash-alpha"));
        await CompleteTurnAsync(initial, conversationActorId, turnAlpha, "answer-alpha");
        var turnBeta = CreateStartTurnCommand("beta");
        await CompleteTurnAsync(initial, conversationActorId, turnBeta, "answer-beta");
        var beforeReplay = (await eventStore.GetEventsAsync(conversationActorId)).Count;
        var replayDispatch = new RecordingActorDispatchPort([], static (_, _) => Task.CompletedTask);
        var restarted = CreateController(services, conversationActorId, replayDispatch);
        await restarted.ActivateAsync();

        await restarted.HandleEventAsync(CreateEnvelope(conversationActorId, turnAlpha.Clone()));

        replayDispatch.Calls.Should().BeEmpty("an old exact retry must not repeat model or tool work");
        var committed = await eventStore.GetEventsAsync(conversationActorId);
        committed.Should().HaveCount(beforeReplay + 1);
        committed[^1].EventData.Is(NyxIdChatTurnReplayCommittedEvent.Descriptor).Should().BeTrue();
        var replay = committed[^1].EventData.Unpack<NyxIdChatTurnReplayCommittedEvent>();
        replay.TurnId.Should().Be("turn-alpha");
        replay.TaskId.Should().Be("task-alpha");
        replay.Turn.Prompt.Should().BeEmpty();
        replay.Turn.InputParts.Should().BeEmpty();
        restarted.State.ActiveTurn.TurnId.Should().Be("turn-beta");
    }

    [Fact]
    public async Task StartTurn_OldIdentityWithChangedDigest_ShouldCommitConflictWithoutRedispatch()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        var dispatch = new RecordingActorDispatchPort([], static (_, _) => Task.CompletedTask);
        ...
        await agent.ActivateAsync();
        var turnAlpha = CreateStartTurnCommand("alpha");
        await CompleteTurnAsync(agent, conversationActorId, turnAlpha, "answer-alpha");
        await CompleteTurnAsync(
            agent,
            conversationActorId,
            CreateStartTurnCommand("beta"),
            "answer-beta");
        var dispatchCount = dispatch.Calls.Count;
        var conflicting = turnAlpha.Clone();
        conflicting.Prompt = "changed old request";

        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, conflicting));

        dispatch.Calls.Should().HaveCount(dispatchCount);
        var committed = await eventStore.GetEventsAsync(conversationActorId);
        committed[^1].EventData.Is(NyxIdChatTurnAdmissionRejectedEvent.Descriptor).Should().BeTrue();
        committed[^1].EventData.Unpack<NyxIdChatTurnAdmissionRejectedEvent>()
            .ReasonCode.Should().Be(NyxIdChatControlCommands.TurnIdempotencyConflict);
    }

    [Fact]
    public async Task StartTurn_ExactOldTurnWhileLaterTurnActive_ShouldReplayBeforeSteeringRejection()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        var dispatch = new RecordingActorDispatchPort([], static (_, _) => Task.CompletedTask);
        ...
        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, turnAlpha.Clone()));

        dispatch.Calls.Should().HaveCount(dispatchCount);
        var committed = await eventStore.GetEventsAsync(conversationActorId);
        committed[^1].EventData.Is(NyxIdChatTurnReplayCommittedEvent.Descriptor).Should().BeTrue();
        committed[^1].EventData.Unpack<NyxIdChatTurnReplayCommittedEvent>()
            .TurnId.Should().Be("turn-alpha");
    }

    [Fact]
    public async Task TurnReplayRecords_ShouldEvictOldestDeterministicallyAndExcludePrompt()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        using var services = BuildEventSourcingServices(eventStore);
        ...
        await agent.ActivateAsync();
        var total = NyxIdChatTaskLifecycle.MaxTurnReplayRecords + 2;

        for (var index = 0; index < total; index++)
        {
            var suffix = $"replay-{index:00}";
            await CompleteTurnAsync(
                agent,
                conversationActorId,
                CreateStartTurnCommand(suffix),
                $"answer-{index:00}");
        }

        agent.State.RecentTurnReplayRecords.Should()
            .HaveCount(NyxIdChatTaskLifecycle.MaxTurnReplayRecords);
        agent.State.RecentTurnReplayRecords.Select(static record => record.TurnId).Should().Equal(
            Enumerable.Range(2, NyxIdChatTaskLifecycle.MaxTurnReplayRecords)
                .Select(static index => $"turn-replay-{index:00}"));
        agent.State.RecentTurnReplayRecords.Should().OnlyContain(record =>
            record.AdmissionRequestSha256.Length == 32 &&
            record.Turn.Prompt.Length == 0 &&
            record.Turn.InputParts.Count == 0 &&
            record.Turn.Status != NyxIdChatTurnStatus.Active &&
            record.Task.Status != NyxIdChatTaskStatus.Active);

        var reconstructed = CreateController(services, conversationActorId);
        await reconstructed.ActivateAsync();
        reconstructed.State.RecentTurnReplayRecords.Should()
            .BeEquivalentTo(agent.State.RecentTurnReplayRecords, options => options.WithStrictOrdering());
    }

    [Fact]
    public async Task StartTurn_SameIdentityWithDifferentTypedModelControl_ShouldCommitConflict()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        var dispatch = new RecordingActorDispatchPort([], static (_, _) => Task.CompletedTask);
        ...
        await agent.ActivateAsync();
        var first = CreateStartTurnCommand();
        first.LlmControl.ModelOverride = "model-alpha";
        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, first));
        var conflicting = first.Clone();
        conflicting.LlmControl.ModelOverride = "model-beta";
    ...
    [Fact]
    public async Task StartTurn_ExactDuplicateWithRefreshedCredentials_ShouldCommitReplay()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        var dispatch = new RecordingActorDispatchPort([], static (_, _) => Task.CompletedTask);
        ...
        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, first));
        var replay = first.Clone();
        replay.LlmControl.NyxIdAccessToken = "runtime-token-beta";
        replay.LlmControl.NyxIdOrgToken = "org-token-beta";
        replay.ToolContext.Credentials.NyxIdAccessToken = "runtime-token-beta";
        replay.ToolContext.Credentials.NyxIdOrgToken = "org-token-beta";
    ...
    [Fact]
    public async Task LateToolReceiptAfterStop_ShouldRefreshOldTurnReplayAcrossLaterTurnAndRestart()
    {
        const string conversationActorId = "conversation-alpha";
        var eventStore = new InMemoryEventStoreForTests();
        using var services = BuildEventSourcingServices(eventStore);
        ...
        await agent.ActivateAsync();
        var turnAlpha = CreateStartTurnCommand("alpha");
        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, turnAlpha));
        var llmKey = agent.State.ActiveTask.Steps.Single().Operation.Key.Clone();
        await agent.HandleEventAsync(CreateEnvelope(conversationActorId, new NyxIdChatOperationResultSignal
        {
            Key = llmKey,
        ...
        await agent.HandleEventAsync(CreateEnvelope(
            conversationActorId,
            CreateStopCommand(expectedStateVersion: 4)));
        agent.State.RecentTurnReplayRecords.Should().ContainSingle();
        agent.State.RecentTurnReplayRecords[0].Task.Steps[^1].ExternalEffect.Should().Be(
            NyxIdChatEffectEvidence.MayHaveChanged);
        ...
        agent.State.ActiveTask.Steps[^1].ExternalEffect.Should().Be(
            NyxIdChatEffectEvidence.Confirmed);
        agent.State.ActiveTask.Steps[^1].Operation.Phase.Should().Be(
        ...
        await CompleteTurnAsync(
            ...
            "answer-beta");
        var replayDispatch = new RecordingActorDispatchPort([], static (_, _) => Task.CompletedTask);
        var reconstructed = CreateController(
            services,
            conversationActorId,
            replayDispatch);
        await reconstructed.ActivateAsync();
        reconstructed.State.RecentTurnReplayRecords.Select(static record => record.TurnId)
            .Should().Equal("turn-alpha", "turn-beta");
        var beforeReplay = (await eventStore.GetEventsAsync(conversationActorId)).Count;

        await reconstructed.HandleEventAsync(CreateEnvelope(
            conversationActorId,
            turnAlpha.Clone()));

        replayDispatch.Calls.Should().BeEmpty(
            "reconstruction and exact old-turn replay must not redispatch work");
        var committed = await eventStore.GetEventsAsync(conversationActorId);
        committed.Should().HaveCount(beforeReplay + 1);
        committed[^1].EventData.Is(NyxIdChatTurnReplayCommittedEvent.Descriptor).Should().BeTrue();
        var replay = committed[^1].EventData.Unpack<NyxIdChatTurnReplayCommittedEvent>();
        replay.Turn.Status.Should().Be(NyxIdChatTurnStatus.Stopped);
        replay.Task.Status.Should().Be(NyxIdChatTaskStatus.Stopped);
        var replayedTool = replay.Task.Steps[^1];
        replayedTool.Status.Should().Be(NyxIdChatStepStatus.Uncertain);
        replayedTool.ExternalEffect.Should().Be(NyxIdChatEffectEvidence.Confirmed);
        replayedTool.Operation.Phase.Should().Be(NyxIdChatOperationPhase.Succeeded);
        reconstructed.State.ActiveTurn.TurnId.Should().Be("turn-beta");
    }
    ...
    private static NyxIdChatStartTurnCommand CreateStartTurnCommand(string identitySuffix)
    {
        var command = CreateStartTurnCommand();
        command.TurnId = $"turn-{identitySuffix}";
        command.TaskId = $"task-{identitySuffix}";
        command.ClientRequestId = $"client-{identitySuffix}";
        command.CommandId = $"command-{identitySuffix}";
        command.CorrelationId = $"correlation-{identitySuffix}";
        command.Prompt = $"prompt-{identitySuffix}";
        return command;
    }
    ...
    private static ChatContentPart CreateDurableImagePart(string name, string sha256) => new()
    {
    ...
    private static ChatContentPart CreateInlineImagePart(string dataBase64) => new()
    {
        Kind = ChatContentPartKind.Image,
        Name = "inline.png",
        MediaType = "image/png",
