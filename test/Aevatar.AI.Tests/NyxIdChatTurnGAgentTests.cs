...
public sealed class NyxIdChatTurnGAgentTests
{
    ...
    [Fact]
    public async Task OperationExecutor_MismatchedProfileAuthority_ShouldNotInvokeGeneration()
    {
        var generationExecutor = new StreamingCapabilityReplyExecutor();
        var executor = new NyxIdChatTurnOperationExecutor(generationExecutor);
        var binding = AgentProfileExecutionBindingCodec.Seal(
            AgentProfileExecutionBindingCodecTests.BuildExecutionBinding());
        var command = new NyxIdChatOperationDispatchCommand
        {
            Key = CreateKey(),
        ...
        generationExecutor.InitialStateBuilds.Should().Be(0);
        generationExecutor.LlmStepStates.Should().BeEmpty();
    ...
    [Fact]
    public async Task OperationExecutor_CandidateLessAuthorityFromDifferentBinding_ShouldFailBeforeGeneration()
    {
        var tools = new IAgentTool[]
        {
            new CatalogTool("recovery-tool"),
        };
        var materializer = new AgentProfileTurnCatalogMaterializer(
            new CatalogToolSetRegistry("nyxid.profile.route", tools),
            new CatalogClassifier());
        var bindingAInput = AgentProfileExecutionBindingCodecTests.BuildExecutionBinding(
            instructionBody: "Always authority A instructions.");
        bindingAInput.Source.ProfileId = "profile-candidate-less-a";
        bindingAInput.Source.PublishedSnapshotSha256 = ByteString.CopyFrom(
            Enumerable.Repeat((byte)0xA1, 32).ToArray());
        bindingAInput.ProfileInstructions = "Profile A instructions.";
        AgentProfileExecutionBindingCodecTests.ConfigureAlwaysMember(
            bindingAInput.Members[0],
            "Always authority A instructions.");
        var bindingA = AgentProfileExecutionBindingCodec.Seal(bindingAInput);
        var bindingBInput = AgentProfileExecutionBindingCodecTests.BuildExecutionBinding(
            instructionBody: "Always substituted B instructions.");
        bindingBInput.Source.ProfileId = "profile-candidate-less-b";
        bindingBInput.Source.PublishedSnapshotSha256 = ByteString.CopyFrom(
            Enumerable.Repeat((byte)0xB2, 32).ToArray());
        bindingBInput.ProfileInstructions = "Profile substituted B instructions.";
        AgentProfileExecutionBindingCodecTests.ConfigureAlwaysMember(
            bindingBInput.Members[0],
            "Always substituted B instructions.");
        var bindingB = AgentProfileExecutionBindingCodec.Seal(bindingBInput);
        ...
        var generationExecutor = new CatalogObservingReplyExecutor();
        var executor = new NyxIdChatTurnOperationExecutor(
            generationExecutor,
            new RecordingActionPostconditionPort(),
            materializer);
        var command = new NyxIdChatOperationDispatchCommand
        {
            Key = CreateKey(),
            AgentProfileBinding = bindingB,
            AgentProfileTurnAuthority = preparation.Authority,
            Llm = new NyxIdChatLLMOperationInput
            {
                Request = new ChatRequestEvent
                {
                    Prompt = "must not use substituted profile content",
                    SessionId = "turn-alpha",
                },
            },
        };
        ...
        generationExecutor.InitialStateBuilds.Should().Be(0);
        generationExecutor.LlmStepBuilds.Should().Be(0);
        execution.Result.ResultCase.Should().Be(
            NyxIdChatOperationResultSignal.ResultOneofCase.Failure);
        execution.Result.Failure.FailureCode.Should().Be(
            "NYXID_CHAT_AGENT_PROFILE_AUTHORITY_INVALID");
    ...
    [Fact]
    public async Task OperationExecutor_ProfiledLlm_ShouldPassMaterializedCatalogToGeneration()
    {
        var tools = new IAgentTool[]
        {
            new CatalogTool("recovery-tool"),
            new CatalogTool("task-tool"),
        };
        var materializer = new AgentProfileTurnCatalogMaterializer(
            new CatalogToolSetRegistry("nyxid.profile.route", tools),
            new CatalogClassifier());
        var binding = AgentProfileExecutionBindingCodec.Seal(
            AgentProfileExecutionBindingCodecTests.BuildExecutionBinding());
        var toolContext = AgentToolExecutionContext.Empty;
        var preparation = await materializer.PrepareAsync(
            binding,
            "turn-alpha",
            "alpha run",
            tools,
            toolContext);
        var committed = await materializer.MaterializeCommittedAsync(
            binding,
        ...
        using var services = new ServiceCollection()
            .AddSingleton<IAgentRunReplyGenerationExecutorPort>(generationExecutor)
            .AddSingleton<INyxIdActionPostconditionPort, RecordingActionPostconditionPort>()
            .AddSingleton(materializer)
            .AddSingleton<NyxIdChatTurnOperationExecutor>()
            .BuildServiceProvider();
        var executor = services.GetRequiredService<NyxIdChatTurnOperationExecutor>();
        var command = new NyxIdChatOperationDispatchCommand
        {
            Key = CreateKey(),
            AgentProfileBinding = binding,
            AgentProfileTurnAuthority = committed.ReconcileProposal,
            AgentProfileAttemptSource = committed.ReconcileProposal.ReconciliationKey.Clone(),
            Llm = new NyxIdChatLLMOperationInput
            {
                Request = new ChatRequestEvent
                {
        ...
        generationExecutor.InitialCatalog.Should().NotBeNull();
        var catalog = generationExecutor.InitialCatalog!;
        generationExecutor.StepCatalog.Should().BeSameAs(catalog);
        catalog.ProfilePromptLayer!.Content.Should().Contain(binding.ProfileInstructions);
        catalog.SelectedSkillPromptLayer!.Content.Should().Be(binding.Members[0].InstructionBody);
        catalog.RouteOwnedTools["recovery-tool"].Should().BeSameAs(tools[0]);
        catalog.RouteOwnedTools["task-tool"].Should().BeSameAs(tools[1]);
    }

    [Fact]
    public async Task OperationExecutor_ProfiledFirstOutput_ShouldRecordOnceAcrossContinuationRounds()
    {
        const long sourceStateVersion = 910_002;
        var targetActivities = new List<IReadOnlyDictionary<string, object?>>();
        var firstOutputSeams = new List<string>();
        using var listener = new ActivityListener
        {
            ...
            {
                if (activity.DisplayName != "agent_profile execution round" ||
        ...
        meterListener.InstrumentPublished = (instrument, currentListener) =>
        {
        ...
        {
            if (Activity.Current?.GetTagItem("aevatar.agent_profile.source_state_version")?.ToString() !=
                sourceStateVersion.ToString(System.Globalization.CultureInfo.InvariantCulture))
            {
                return;
            }
            ...
            {
                if (tag.Key == "aevatar.agent_profile.seam" &&
                    string.Equals(tag.Value?.ToString(), "first_stream_output", StringComparison.Ordinal))
                {
                    firstOutputSeams.Add("first_stream_output");
                }
            }
        });
        meterListener.Start();
        var harness = await CreateProfiledExecutorHarnessAsync(sourceStateVersion);
        harness.GenerationExecutor.StreamChunks.Add(new LLMStreamChunk
        {
            DeltaContent = "profiled output",
        });
        harness.GenerationExecutor.StreamChunks.Add(new LLMStreamChunk
        {
            DeltaReasoningContent = "profiled reasoning",
        });
        var session = new NyxIdChatTransientExecutionSession();

        await harness.Executor.ExecuteAsync(
            harness.InitialCommand,
            session,
            static (_, _) => Task.CompletedTask,
            CancellationToken.None);
        await harness.Executor.ExecuteAsync(
            ...
            {
                Key = CreateKey("step-continuation-alpha", "operation-continuation-alpha"),
                AgentProfileAttemptSource = harness.CommittedAuthority.ReconciliationKey.Clone(),
        ...
        targetActivities.Should().HaveCount(2);
        targetActivities.Count(tags =>
                tags.ContainsKey("aevatar.agent_profile.first_stream_output.outcome"))
            .Should().Be(1);
        firstOutputSeams.Should().ContainSingle().Which.Should().Be("first_stream_output");
        targetActivities.SelectMany(static tags => tags)
            .Where(static tag => tag.Key == "aevatar.agent_profile.activation_mode")
            .Select(static tag => tag.Value)
            .Should().OnlyContain(value =>
                value != null &&
                string.Equals(value.ToString(), "enforced", StringComparison.Ordinal));
        harness.GenerationExecutor.StepCatalogs.Should().HaveCount(2);
        harness.GenerationExecutor.StepCatalogs[1]
            .Should().BeSameAs(harness.GenerationExecutor.StepCatalogs[0]);
    }

    [Fact]
    public async Task OperationExecutor_ProfiledRetryWithoutFreshAuthority_ShouldFailBeforeGeneration()
    {
        var harness = await CreateProfiledExecutorHarnessAsync();
        var session = new NyxIdChatTransientExecutionSession();
        await harness.Executor.ExecuteAsync(
            harness.InitialCommand,
            session,
        ...
        {
            Key = CreateKey("step-retry-alpha", "operation-retry-alpha", 2),
            Llm = new NyxIdChatLLMOperationInput
            {
                Request = new ChatRequestEvent
                {
                    Prompt = "retry without committed authority",
                    SessionId = "turn-alpha",
                },
            },
        };
        ...
        execution.Result.Failure.FailureCode.Should().Be(
            "NYXID_CHAT_AGENT_PROFILE_AUTHORITY_INVALID");
        harness.GenerationExecutor.InitialStateBuilds.Should().Be(1);
        harness.GenerationExecutor.LlmStepBuilds.Should().Be(1);
    }

    [Fact]
    public async Task OperationExecutor_ProfileAttemptSourceMismatch_ShouldFailBeforeGeneration()
    {
        var harness = await CreateProfiledExecutorHarnessAsync();
        var staleAttempt = harness.InitialCommand.Clone();
        staleAttempt.Key = CreateKey("step-stale-attempt", "operation-stale-attempt", 1);
        staleAttempt.AgentProfileTurnAuthority.ReconciliationKey.Attempt.Should().Be(1);
        staleAttempt.AgentProfileAttemptSource.Attempt = 2;
        ...
        execution.Result.Failure.FailureCode.Should().Be(
            "NYXID_CHAT_AGENT_PROFILE_AUTHORITY_INVALID");
        harness.GenerationExecutor.InitialStateBuilds.Should().Be(0);
        harness.GenerationExecutor.LlmStepBuilds.Should().Be(0);
    }

    [Fact]
    public async Task OperationExecutor_ProfiledContinuationForDifferentTurn_ShouldFailBeforeGeneration()
    {
        var harness = await CreateProfiledExecutorHarnessAsync();
        var session = new NyxIdChatTransientExecutionSession();
        await harness.Executor.ExecuteAsync(
            ...
            static (_, _) => Task.CompletedTask,
            CancellationToken.None);
        var continuationKey = CreateKey("step-continuation-beta", "operation-continuation-beta", 1);
        continuationKey.TurnId = "turn-beta";
    ...
    [Fact]
    public async Task OperationExecutor_ProfiledRetryWithFreshAuthority_ShouldRematerializeCatalog()
    {
        var harness = await CreateProfiledExecutorHarnessAsync();
        var session = new NyxIdChatTransientExecutionSession();
        await harness.Executor.ExecuteAsync(
            ...
            static (_, _) => Task.CompletedTask,
            CancellationToken.None);
        var retryAuthority = harness.CommittedAuthority.Clone();
        retryAuthority.ReconciliationKey.Attempt = 2;
        var retry = new NyxIdChatOperationDispatchCommand
        {
            Key = CreateKey("step-retry-alpha", "operation-retry-alpha", 2),
            AgentProfileBinding = harness.Binding.Clone(),
            AgentProfileTurnAuthority = retryAuthority,
            AgentProfileAttemptSource = retryAuthority.ReconciliationKey.Clone(),
            Llm = new NyxIdChatLLMOperationInput
            {
                Request = new ChatRequestEvent
                {
                    Prompt = "retry with committed authority",
                    SessionId = "turn-alpha",
                },
            },
        };
        ...
        execution.Result.ResultCase.Should().Be(
            NyxIdChatOperationResultSignal.ResultOneofCase.Llm);
        harness.GenerationExecutor.InitialCatalogs.Should().HaveCount(2);
        harness.GenerationExecutor.StepCatalogs.Should().HaveCount(2);
        harness.GenerationExecutor.InitialCatalogs[1]
            .Should().NotBeSameAs(harness.GenerationExecutor.InitialCatalogs[0]);
        harness.GenerationExecutor.StepCatalogs[1]
            .Should().BeSameAs(harness.GenerationExecutor.InitialCatalogs[1]);
    }

    [Theory]
    [InlineData(AgentProfileActivationMode.Enforced, "alpha run", AgentProfileTurnAuthorityKind.Selected)]
    [InlineData(AgentProfileActivationMode.Shadow, "unmatched request", AgentProfileTurnAuthorityKind.Recovery)]
    public async Task OperationExecutor_ProfileAttemptTwoToolContinuation_ShouldKeepExactCatalog(
        AgentProfileActivationMode activationMode,
        string prompt,
        AgentProfileTurnAuthorityKind expectedAuthorityKind)
    {
        var exactTool = new CountingCatalogTool("recovery-tool");
        var sameNameSubstitute = new CountingCatalogTool("recovery-tool");
        var routeTools = new IAgentTool[]
        {
            exactTool,
            new CatalogTool("task-tool"),
        };
        var materializer = new AgentProfileTurnCatalogMaterializer(
            new CatalogToolSetRegistry("nyxid.profile.route", routeTools),
            new CatalogClassifier());
        var binding = AgentProfileExecutionBindingCodec.Seal(
            AgentProfileExecutionBindingCodecTests.BuildExecutionBinding(activationMode));
        var preparation = await materializer.PrepareAsync(
            binding,
            "turn-alpha",
            prompt,
            routeTools,
            AgentToolExecutionContext.Empty);
        preparation.Authority.AuthorityKind.Should().Be(expectedAuthorityKind);
        if (expectedAuthorityKind == AgentProfileTurnAuthorityKind.Selected)
            preparation.Authority.CandidateRoute.Should().NotBeNull();
        else
            preparation.Authority.CandidateRoute.Should().BeNull();
        var committed = await materializer.MaterializeCommittedAsync(
            binding,
            ...
            routeTools,
            AgentToolExecutionContext.Empty);
        var retryAuthority = committed.ReconcileProposal.Clone();
        retryAuthority.ReconciliationKey.Attempt = 2;
        var generationExecutor = new ProfileAttemptSequenceReplyExecutor(exactTool);
        var executor = new NyxIdChatTurnOperationExecutor(
            generationExecutor,
            new RecordingActionPostconditionPort(),
            materializer);
        var session = new NyxIdChatTransientExecutionSession();
        var retryCommand = new NyxIdChatOperationDispatchCommand
        {
            Key = CreateKey("step-retry-alpha", "operation-retry-alpha", 2),
        ...
        var state = CreateRunningConversationState(retryCommand.Key);
        state.AgentProfileBinding = binding.Clone();
        state.AgentProfileTurnAuthority = retryAuthority.Clone();
        ...
        var llm = await executor.ExecuteAsync(
            ...
            static (_, _) => Task.CompletedTask,
            CancellationToken.None);
        var toolDecision = NyxIdChatTaskLifecycle.ApplyOperationResult(
            state,
            llm.Result,
            Timestamp.FromDateTimeOffset(DateTimeOffset.UtcNow));
        toolDecision.NextCommand.Should().NotBeNull();
        var toolCommand = toolDecision.NextCommand!;

        toolCommand.Key.OperationGeneration.Should().Be(1);
        var tool = await executor.ExecuteAsync(
            toolCommand,
            session,
            static (_, _) => Task.CompletedTask,
            CancellationToken.None);
        var continuationDecision = NyxIdChatTaskLifecycle.ApplyOperationResult(
            toolDecision.State,
            tool.Result,
            Timestamp.FromDateTimeOffset(DateTimeOffset.UtcNow));
        continuationDecision.NextCommand.Should().NotBeNull();
        var continuationCommand = continuationDecision.NextCommand!;

        continuationCommand.Key.OperationGeneration.Should().Be(1);
        continuationCommand.Llm.ContinueSession.Should().BeTrue();
        ...
        continuation.Result.ResultCase.Should().Be(
            NyxIdChatOperationResultSignal.ResultOneofCase.Llm);
        continuation.Result.Llm.Content.Should().Be("final profiled response");
        exactTool.ExecutionCount.Should().Be(1);
        sameNameSubstitute.ExecutionCount.Should().Be(0);
        generationExecutor.ObservedCatalogs.Should().HaveCount(2);
        generationExecutor.ObservedCatalogs[1]
            .Should().BeSameAs(generationExecutor.ObservedCatalogs[0]);
        generationExecutor.ObservedTools.Should().OnlyContain(tool => ReferenceEquals(tool, exactTool));
    }
    ...
    private sealed class CatalogObservingReplyExecutor : IAgentRunReplyGenerationExecutorPort
    {
        public List<AgentProfileTurnCatalog?> InitialCatalogs { get; } = [];
        public List<AgentProfileTurnCatalog?> StepCatalogs { get; } = [];
        public List<LLMStreamChunk> StreamChunks { get; } = [];
        public AgentProfileTurnCatalog? InitialCatalog => InitialCatalogs.LastOrDefault();
        public AgentProfileTurnCatalog? StepCatalog => StepCatalogs.LastOrDefault();
        public int InitialStateBuilds => InitialCatalogs.Count;
        public int LlmStepBuilds => StepCatalogs.Count;
        ...
        {
            ct.ThrowIfCancellationRequested();
            InitialCatalogs.Add(ReadCatalog(request));
        ...
        {
            ct.ThrowIfCancellationRequested();
            StepCatalogs.Add(ReadCatalog(request));
            if (request.ReportChunkAsync is not null)
            {
                foreach (var chunk in StreamChunks)
                    await request.ReportChunkAsync(chunk, ct);
            }
            ...
            return new AgentRunLlmStepExecution(
                new AgentRunNextLlmStepRequestedEvent
                {
                    ...
                    LlmStepResult = new AgentRunLlmStepResult
                    {
                        ...
                        AccumulatedText = "profiled response",
        ...
        private static AgentProfileTurnCatalog? ReadCatalog(object request) =>
            request.GetType().GetProperty("TurnCatalog")?.GetValue(request) as AgentProfileTurnCatalog;
    }

    private sealed class CatalogTool(string name) : IAgentTool
    {
    ...
    private sealed class CountingCatalogTool(string name) : IAgentTool
    {
        public string Name => name;
        public string Description => name;
        public string ParametersSchema => "{}";
    ...
    private sealed class CatalogToolSource(IReadOnlyList<IAgentTool> tools) : IAgentToolSource
    {
        public Task<IReadOnlyList<IAgentTool>> DiscoverToolsAsync(CancellationToken ct = default) =>
            Task.FromResult(tools);
    }

    private sealed class CatalogToolSetRegistry(
        string routeName,
        IReadOnlyList<IAgentTool> tools) : IToolSetRegistry
    {
        public IReadOnlyList<string> GetRegisteredNames() => [routeName];

        public ToolSetResolveResult Resolve(ChatRouteToolSetRef? toolSetRef) =>
            string.Equals(toolSetRef?.Name, routeName, StringComparison.Ordinal)
                ? ToolSetResolveResult.Success(routeName, [new CatalogToolSource(tools)])
    ...
    private sealed class CatalogClassifier : IAgentProfileTurnClassifier
    {
    ...
    private static async Task<ProfiledExecutorHarness> CreateProfiledExecutorHarnessAsync(
        long sourceStateVersion = 17)
    {
        ...
        return new ProfiledExecutorHarness(
            executor,
            generationExecutor,
            binding,
            committed.ReconcileProposal,
            command);
    }

    private sealed record ProfiledExecutorHarness(
        NyxIdChatTurnOperationExecutor Executor,
        CatalogObservingReplyExecutor GenerationExecutor,
        AgentProfileExecutionBinding Binding,
        AgentProfileTurnAuthorityState CommittedAuthority,
        NyxIdChatOperationDispatchCommand InitialCommand);

    private sealed class ProfileAttemptSequenceReplyExecutor(IAgentTool expectedTool)
        : IAgentRunReplyGenerationExecutorPort
    {
        private static AgentRunToolCall ToolCall { get; } = new()
        {
            Id = "call-profile-alpha",
            Name = "recovery-tool",
            ArgumentsJson = "{}",
        };

        public List<AgentProfileTurnCatalog> ObservedCatalogs { get; } = [];
        public List<IAgentTool> ObservedTools { get; } = [];
        ...
        {
            ct.ThrowIfCancellationRequested();
            var catalog = request.TurnCatalog ?? throw new InvalidOperationException(
                "The Profile sequence requires the committed transient catalog.");
            var catalogTool = catalog.RouteOwnedTools[ToolCall.Name];
            ObservedCatalogs.Add(catalog);
            ObservedTools.Add(catalogTool);
            if (!ReferenceEquals(catalogTool, expectedTool))
                throw new InvalidOperationException("The exact Profile tool capability was widened.");

            var result = new AgentRunLlmStepResult
            {
                Content = request.StepState.Round > 0
                    ? "final profiled response"
                    : "authorized Profile tool required",
                AccumulatedText = request.StepState.Round > 0
                    ? "final profiled response"
                    : "authorized Profile tool required",
                FinishReason = request.StepState.Round > 0 ? "stop" : "tool_calls",
                HasStreamedTextContent = true,
            };
            if (request.StepState.Round > 0)
            {
                return Task.FromResult(new AgentRunLlmStepExecution(
                    BuildLlmContinuation(request, result),
                    AuthorizedToolStep: null));
            }

            result.ToolCalls.Add(ToolCall.Clone());
            var continuation = BuildLlmContinuation(request, result);
            var authorized = new AgentRunAuthorizedToolStep(
                ...
                async token =>
                {
                    var output = await catalogTool.ExecuteAsync(ToolCall.ArgumentsJson, token);
                    return new AgentRunToolStepResult
                    {
                        AdvanceRound = true,
                        ResultMessages =
                        {
            ...
            CancellationToken ct)
        {
            var result = authorizedToolStep?.Matches(request) == true
                ...
                : throw new InvalidOperationException("The exact authorized tool step did not match.");
        ...
        private static AgentRunNextLlmStepRequestedEvent BuildLlmContinuation(
            AgentRunReplyStepExecutionRequest request,
    ...
    private static NyxIdChatConversationGAgentState CreateRunningConversationState(
