...
public sealed class NyxIdAssistantActionRegistryTests
{
    ...
    [Fact]
    public async Task HttpSource_ChunkedMultibytePayloadOverRawByteLimit_ShouldRejectBeforeDecoding()
    {
        var rawPayload = Encoding.UTF8.GetBytes(new string('\u754c', 400_000));
        rawPayload.Should().HaveCountGreaterThan(1024 * 1024);
        var handler = new RawRegistryHandler(rawPayload);
        var source = new NyxIdAssistantActionRegistryHttpSource(
    ...
    [Fact]
    public async Task HttpSource_InvalidUtf8WithinByteLimit_ShouldFailStrictly()
    {
        var handler = new RawRegistryHandler([0xC3, 0x28]);
        var source = new NyxIdAssistantActionRegistryHttpSource(
            new StubHttpClientFactory(new HttpClient(handler)),
            new NyxIdToolOptions { BaseUrl = "https://nyxid.example.test/" });
    ...
    private sealed class RawRegistryHandler(byte[] responseBody) : HttpMessageHandler
    {
        public List<HttpRequestMessage> Requests { get; } = [];
            ...
            CancellationToken cancellationToken)
        {
            ...
            return Task.FromResult(new HttpResponseMessage(HttpStatusCode.OK)
            {
                Content = new UnknownLengthContent(responseBody),
            });
        }
    }
