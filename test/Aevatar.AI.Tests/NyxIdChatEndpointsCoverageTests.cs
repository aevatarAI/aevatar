...
public class NyxIdChatEndpointsCoverageTests
{
    ...
    {
        var relay = CreateRelayInvocationDependencies(relayApiKeyId: "scope-invoice");
        var payload = """
            {
              "message_id":"msg-invoice-1",
              "correlation_id":"corr-invoice-1",
              "platform":"lark",
              "reply_token":"reply-token-invoice-1",
              "agent":{"api_key_id":"scope-invoice"},
              "conversation":{"id":"oc_private_1","type":"private"},
              "sender":{"platform_id":"ou_user_1","display_name":"Alice"},
              "content":{"type":"text","text":"/invoice alice"},
              "raw_platform_data":{
                "event":{
        ...
        context.Request.ContentType = "application/json";
        context.Request.Body = new MemoryStream(Encoding.UTF8.GetBytes(payload));
        AttachRelayHeaders(context, relay, payload, "msg-invoice-1");
        ...
        response.StatusCode.Should().Be(StatusCodes.Status202Accepted);
        response.Body.Should().Contain("accepted");
        var expectedActorId = BuildScopedRelayConversationActorId("scope-invoice", "lark:dm:ou_user_1");
        runtime.CreateCalls.Should().ContainSingle(call =>
            call.Type == typeof(ConversationGAgent) &&
            call.Id == expectedActorId);
        runtime.Actors.Should().ContainKey(expectedActorId);
        ...
        actor.HandledEnvelopes.Should().ContainSingle(envelope =>
            ...
            envelope.Payload.Is(NyxRelayInboundActivity.Descriptor));
        var relayInbound = actor.HandledEnvelopes.Single().Payload.Unpack<NyxRelayInboundActivity>();
        relayInbound.ReplyToken.Should().Be("reply-token-invoice-1");
        relayInbound.CorrelationId.Should().Be("corr-invoice-1");
        relayInbound.RelayApiKeyId.Should().Be(relay.RelayApiKeyId);
        relayInbound.CallbackJti.Should().Be("corr-invoice-1");
        relayInbound.CallbackObservedAtUnixMs.Should().BeGreaterThan(0);
        relayInbound.CallbackReplayExpiresAtUnixMs.Should().BeGreaterThan(relayInbound.CallbackObservedAtUnixMs);
        var activity = relayInbound.Activity;
        activity.Id.Should().Be("msg-invoice-1");
        activity.Content.Text.Should().Be("/invoice alice");
        ...
        activity.OutboundDelivery.ReplyMessageId.Should().Be("msg-invoice-1");
        activity.OutboundDelivery.CorrelationId.Should().Be("corr-invoice-1");
    ...
    public async Task HandleRelayWebhookAsync_ShouldStashSenderNyxUserIdOnTransportExtras_ForChatRoutePolicyLookup()
    {
        ...
        var response = await ExecuteResultAsync(result);
        response.StatusCode.Should().Be(StatusCodes.Status202Accepted);
        var expectedActorId = BuildScopedRelayConversationActorId("scope-invoice", "lark:dm:ou_user_2");
        var actor = (StubActor)runtime.Actors[expectedActorId];
        var relayInbound = actor.HandledEnvelopes.Single().Payload.Unpack<NyxRelayInboundActivity>();
    ...
    public async Task HandleRelayWebhookAsync_ShouldLeaveSenderNyxUserIdEmpty_WhenResolverFails()
    {
        ...
        var expectedActorId = BuildScopedRelayConversationActorId("scope-invoice", "lark:dm:ou_user_3");
        var actor = (StubActor)runtime.Actors[expectedActorId];
        var relayInbound = actor.HandledEnvelopes.Single().Payload.Unpack<NyxRelayInboundActivity>();
