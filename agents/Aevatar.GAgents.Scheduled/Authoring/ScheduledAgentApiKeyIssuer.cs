...
internal sealed class ScheduledAgentApiKeyIssuer : IScheduledAgentApiKeyIssuer
{
        ...
        CancellationToken ct)
    {
        ...
        // The planner seals how admitted NyxID operations are authorized for
        // unattended execution. Only a per-call-policy attestation can be
        // satisfied by an ordinary scoped key; an unattested plan predates the
        // durable operation authority contract and must be rebound.
        if (plan.DurableOperationAuthorityMode !=
            ScheduledDurableOperationAuthorityMode.PerCallPolicy)
        {
            return ScheduledAgentApiKeyIssueResult.Failed(
                "scheduled_durable_operation_authority_unavailable",
                "The scheduled authorization plan does not attest how its NyxID operations are authorized for unattended execution.",
                "Rebind so the authorization planner seals the durable operation authority mode into the plan.");
        }
        ...
        long keyExpiresAtUnixMs)
    {
        ...
        try
        {
            ...
            // Per-call-policy authority issues an ordinary scoped proxy key.
            // A scheduled_invocation class or durable grant receipts on the
            // response mean NyxID provisioned more authority than the plan
            // attested, so the key is preserved only for cleanup.
                ...
                !securityClass.IsGeneralProxyCredential)
            {
                return ScheduledAgentApiKeyIssueResult.FailedAfterIssue(
                    ...
                    "NyxID returned a credential class that does not match the authorized ordinary scoped proxy credential.",
                    "Revoke this Agent Key; per-call-policy authority never provisions durable operation grants.");
            }
