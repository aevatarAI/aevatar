...
public sealed class NyxIdChatGAgent : RoleGAgent
{
        ...
        NyxIdChatConversationCreateCommand command)
    {
        ...
        await BindAgentProfileAsync(command.AgentProfileBinding);
        ...
        AgentProfileTurnDegradationReason reason)
    {
        ...
        if (proposal.SelectedExactSkillRef is not null)
        {
        ...
        return AgentProfileTurnCatalogMaterialization.Create(
            new AgentProfileTurnCatalog(
                ...
                candidateIntentId: proposal.CandidateRoute?.IntentId),
            proposal);
    }
    ...
    public override async Task HandleChatRequest(ChatRequestEvent request)
    {
        ...
        var telemetryContext = State.AgentProfileBinding is { } binding
            ? CreateTelemetryContext(binding)
            : null;
    ...
    {
        var boundBinding = State.AgentProfileBinding;
        if (binding is null)
        {
            if (boundBinding is not null)
                throw new InvalidOperationException("A bound agent profile execution binding cannot be removed from a conversation.");
            return;
        }
        ...
        {
            await PersistDomainEventAsync(new AgentProfileBoundEvent { Binding = binding.Clone() });
            return;
        }

        if (!AgentProfileExecutionBindingCodec.ByteEquivalent(boundBinding, binding))
            throw new InvalidOperationException("A conversation cannot replace its bound agent profile execution binding.");
    }
    ...
    protected override RoleGAgentState TransitionState(RoleGAgentState current, IMessage evt)
    {
        ...
        if (profileBound.Binding is null)
            throw new InvalidOperationException("Agent profile binding events require a complete binding.");

        if (!AgentProfileExecutionBindingCodec.Verify(profileBound.Binding))
        ...
        {
            if (!AgentProfileExecutionBindingCodec.ByteEquivalent(current.AgentProfileBinding, profileBound.Binding))
                throw new InvalidOperationException("Committed agent profile bindings cannot be replaced.");
            return current;
        }

        var next = current.Clone();
        next.AgentProfileBinding = profileBound.Binding.Clone();
        return next;
    }
}
