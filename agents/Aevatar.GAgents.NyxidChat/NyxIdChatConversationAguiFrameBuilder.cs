...
internal static class NyxIdChatConversationAguiFrameBuilder
{
    ...
    public static IReadOnlyList<AGUIEvent> BuildTurnReplay(
        NyxIdChatTurnReplayCommittedEvent replay,
        long sequence)
    {
        ArgumentNullException.ThrowIfNull(replay);
        if (replay.Turn is null ||
            replay.Task is null ||
            sequence <= 0 ||
            !string.Equals(replay.TurnId, replay.Turn.TurnId, StringComparison.Ordinal) ||
            !string.Equals(replay.TaskId, replay.Turn.TaskId, StringComparison.Ordinal) ||
            !string.Equals(replay.TaskId, replay.Task.TaskId, StringComparison.Ordinal) ||
            !string.Equals(replay.TurnId, replay.Task.TurnId, StringComparison.Ordinal) ||
            !string.Equals(
                replay.ClientRequestId,
                replay.Turn.ClientRequestId,
                StringComparison.Ordinal))
        {
            return [];
        }

        var frames = BuildTaskFrames(replay.Task, replay.Task.Steps, sequence);
        AppendTerminalIfNeeded(
            frames,
            replay.ConversationActorId,
            replay.TurnId,
            replay.Task,
            replay.Turn,
            sequence);
