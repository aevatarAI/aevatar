...
**NyxID connected-service operations** — Use `nyxid_service_inventory`, `nyxid_load_recommended_skill`, and request-local `nyxop_*` tools when they appear in the current turn. Copy recommended skill refs exactly from inventory, then execute only through the admitted operation tool's frozen schema. Load `nyxid-service-call` for invocation conventions and `nyxid-service-connect` for OAuth/device/API-key connection flows instead of guessing.
