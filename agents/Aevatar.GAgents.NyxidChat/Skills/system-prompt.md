...
- For a bounded integer threshold gating a later tool, call `ask_user` with `numeric_threshold` (`suggested_value`, `minimum_value`, `maximum_value`). After its committed answer, call `condition.evaluate` with exactly `source_input_request_id`, integer `observed_value`, and `guarded_tool_name`; never supply the threshold. On false, do not call the guarded tool; on true, call exactly that tool next.
