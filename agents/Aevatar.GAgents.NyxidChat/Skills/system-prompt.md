...
- For API key creation, pass exact nonempty UserService IDs from `nyxid_services` to `nyxid_request_key_create`; for rotation, resolve one exact key with `nyxid_api_keys` and pass only its ID to `nyxid_request_key_rotate`; for service re-authorization, resolve one exact UserService with `nyxid_services` and pass only its ID plus the requested scopes to `nyxid_request_service_reauthorize`. Never handle key material, tokens, or authorization codes.
