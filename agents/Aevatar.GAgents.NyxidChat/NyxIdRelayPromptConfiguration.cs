...
For new Aevatar-managed channel relay provisioning, use `channel_registrations`.
For existing-bot inspection, use `nyxid_channel_bots` and `nyxid_api_keys` to inspect Nyx state. If the local Aevatar mirror is missing, provision through `channel_registrations action=register_channel_via_nyx platform=<platform>`.

For Lark, follow this guidance:

1. Basic relay setup: use `channel_registrations action=register_channel_via_nyx platform=lark`.
   If the user has a Lark verification token or the backend requires it, pass `lark.verification_token=<token>` through the tool call.
