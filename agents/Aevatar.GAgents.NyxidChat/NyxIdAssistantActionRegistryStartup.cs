    ...
    : INyxIdAssistantActionRegistrySource
{
    ...
    public async Task<string> FetchAsync(CancellationToken ct)
    {
        ...
        using var registryBytes = new MemoryStream();
        ...
        while (true)
        {
            ...
            if (read == 0)
                break;
            if (registryBytes.Length > MaximumRegistryBytes - read)
            {
                throw new InvalidOperationException(
                    "The NyxID Assistant action registry exceeds the maximum size.");
            }
            registryBytes.Write(buffer, 0, read);
        }

        return StrictUtf8.GetString(registryBytes.GetBuffer(), 0, checked((int)registryBytes.Length));
    }
}
