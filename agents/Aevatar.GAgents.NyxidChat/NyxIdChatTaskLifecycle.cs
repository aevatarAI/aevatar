...
{
    public const int MaxTurnReplayRecords = NyxIdChatTurnReplayRetention.MaxRecords;
        ...
        Timestamp now)
    {
        ...
        AttachProfileAttemptSource(next, command);
        FinalizeDerivedState(next, now);
        return new NyxIdChatTaskLifecycleDecision(
            transition.Outcome,
            transition.ReasonCode,
        ...
        Timestamp now)
    {
        ...
        AttachProfileAttemptSource(state, command);
        return command;
    }

    private static void AttachProfileAttemptSource(
        NyxIdChatConversationGAgentState state,
        NyxIdChatOperationDispatchCommand command)
    {
        if (state.AgentProfileBinding is null ||
            state.AgentProfileTurnAuthority?.ReconciliationKey is not { } source)
        {
            return;
        }

        command.AgentProfileAttemptSource = source.Clone();
    }
        ...
        NyxIdChatTurnState turn)
    {
        ...
        NyxIdChatTurnReplayRetention.RetainTerminalTurn(state, turn);
    }
