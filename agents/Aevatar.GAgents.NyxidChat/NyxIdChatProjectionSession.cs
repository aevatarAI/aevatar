    ...
    : ProjectionSessionEventProjectorBase<NyxIdChatSessionProjectionContext, AGUIEvent>
{
        ...
        EventEnvelope envelope)
    {
        ...
        if (payload.Is(NyxIdChatTurnReplayCommittedEvent.Descriptor))
        {
            var replay = payload.Unpack<NyxIdChatTurnReplayCommittedEvent>();
            if (!string.Equals(
                    replay.ConversationActorId,
                    context.RootActorId,
                    StringComparison.Ordinal) ||
                !string.Equals(replay.TurnId, context.SessionId, StringComparison.Ordinal))
            {
                return EmptyEntries;
            }
            return Entries(
                context,
                NyxIdChatConversationAguiFrameBuilder.BuildTurnReplay(replay, stateVersion));
        }
