...
public static class NyxIdChatBrowserActions
{
        ...
        Timestamp now)
    {
        ...
        if (isStateChangeWake)
        {
            ...
            for (var index = 0; index < pending.Length; index++)
            {
                ...
                if (RequiresCompletedResourceHint(pending[index].Action) && report is null)
                    return RejectContinuation(state, ActionContinuationInvalid);
    ...
    private static bool RequiresCompletedResourceHint(NyxIdAssistantActionKind action) =>
        action is NyxIdAssistantActionKind.KeyCreate or NyxIdAssistantActionKind.KeyRotate;
