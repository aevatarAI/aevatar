    ...
    IReplyOperationActorContext
{
        ...
        CancellationToken ct)
    {
        ...
        var runtimeRegistration = string.IsNullOrWhiteSpace(registration.Id)
            ? registration.Clone()
            : registration;
        if (string.IsNullOrWhiteSpace(runtimeRegistration.Id))
            runtimeRegistration.Id = registrationId;

        return ChannelRuntimeConfigProofBuilder.Build(runtimeRegistration, configRevision);
    }
