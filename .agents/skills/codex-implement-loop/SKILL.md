...
- A Codex subagent — not codex — as the reviewer
...
4. **Dispatch**:
   ```bash
   .Codex/skills/codex-implement-loop/scripts/spawn-codex.sh \
     --cd "$WORKTREE_PATH" \
     --add-dir "$REPO_ROOT" \
...
🤖 Generated by codex-implement-loop. Reviewer is a Codex subagent (see PR comments for round-N review reports).
EOF
)
...
## Phase 3 — Review (Codex subagent, synchronous)
...
2. **Dispatch the subagent** (one call, synchronous, blocks the controller turn):
   ...
   - Read `AGENTS.md` + any cited `docs/canon/` references.
...
3. On task notification, check log for `FIX_DONE:${pr_number}:round-${R}:<status>` where `status ∈ {ok, blocked}`.
   ...
   - `blocked` → read the fix summary's "Blocked" section. If blocker is "review demand contradicts AGENTS.md / would scope-creep" → escalate (`issues_failed` reason `fix-blocked:<short>`, PushNotification, stop). Do NOT auto-loop on `blocked`.
...
5. **No external repo changes**: codex prompts forbid touching NyxID / chrono-* (per AGENTS.md "外部仓库无改动权").
...
8. **Subagent reviewer is independent**: the review subagent must not see the controller's own conclusions; pass it only the materialized review prompt + tool access (Read / Bash / Grep). It must reach a verdict from PR diff + issue body + AGENTS.md, nothing else.
