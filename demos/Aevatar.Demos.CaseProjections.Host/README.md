...
- `Aevatar.Demos.CaseProjections` 复用通用 CQRS 内核 (`ProjectionLifecycleService` / `ProjectionSubscriptionRegistry`)。
- `Aevatar.Demos.CaseProjections.Extensions.Sla` 通过外部程序集新增 `CaseEscalatedEventReducer`，不修改核心投影项目。
- Host 仅通过 DI 组合能力：
