 ...
 */
export async function resolveReferences(text, authorization) {
    ...
    for (const match of matches) {
        ...
        try {
            ...
            if (isSkill) {
                ...
                if (content)
                    logger.info({ ref: fullRef, contentLength: content.length }, "Resolved skill reference");
            }
            else {
                content = await fetchSchemaJson(name);
                if (content)
                    logger.info({ ref: fullRef, contentLength: content.length }, "Resolved schema reference");
            }
            if (content) {
            ...
            logger.warn({ ref: fullRef, err: err.message }, "Failed to resolve reference, leaving as-is");
        }
    }
 ...
 */
async function fetchSkillMarkdown(name, _authorization) {
 ...
 */
async function fetchSchemaJson(name) {
    try {
        ...
        logger.error({ name, err: err.message }, "Failed to fetch schema");
        return null;
    }
}
