...
import { getSessionCollection } from "./models/session.js";
import { getEventLogCollection } from "./models/event-log.js";
import { fetchCompiledWorkflow, startExecution, startDeployedExecution, streamEvents, terminateExecution, } from "./aevatar-client.js";
...
const VERIFY_CRON_INTERVAL_HOURS = parseInt(process.env["VERIFY_CRON_INTERVAL_HOURS"] ?? "6", 10);
// Active sessions keyed by sessionId
const activeSessions = new Map();
let verifyCronTask = null;
/**
 * Start a workflow session.
 * @param options.runMode "deployed" (default) enforces workflow must be deployed; "draft" compiles on-the-fly
 */
export async function startSession(workflowType, triggeredBy, options) {
...
export async function stopSession(workflowType) {
    ...
    const db = getDb();
    await getSessionCollection(db).updateOne({ id: session.id }, {
        $set: {
            status: "stopped",
            stoppedAt: now,
            duration,
 ...
 */
export function getSessionStatus(workflowType) {
    for (const [, val] of activeSessions) {
        if (val.session.workflowType === workflowType)
            return { running: true, session: val.session };
    }
 ...
 */
export function getAllSessionStatus() {
    const types = ["research", "translate", "purify", "verify"];
    const result = {};
...
export async function recoverStaleSessions() {
    ...
    for (const session of staleSessions) {
        try {
            ...
            logger.warn({ sessionId: session.id, err: err.message }, "Failed to terminate stale execution");
        }
        await col.updateOne({ id: session.id }, {
            $set: {
                status: "failed",
...
export function startVerifyCron() {
    ...
    verifyCronTask = cron.schedule(cronExpr, async () => {
        ...
        try {
            ...
            logger.error({ err: err.message }, "Verify cron trigger failed");
        }
    });
}
/**
 ...
 */
export function stopVerifyCron() {
    if (verifyCronTask) {
        verifyCronTask.stop();
        verifyCronTask = null;
 ...
 */
async function checkUnverifiedBlueNodes() {
    try {
        ...
        logger.warn({ err: err.message }, "Error checking blue nodes, assuming unverified exist");
 ...
 */
async function consumeEventsFromStream(session, sseStream, authorization, signal) {
    const db = getDb();
    ...
    if (!stream) {
        ...
        try {
            ...
            logger.error({ sessionId: session.id, err: err.message }, "Failed to connect to event stream");
            return;
        }
    }
    if (!stream)
    ...
    try {
        for await (const evt of parseSseStream(stream, signal)) {
            ...
            if (evt.runError) {
                await markFailed(session, evt.runError?.message ?? "workflow error");
            }
        }
    }
    catch (err) {
        if (!signal?.aborted) {
            logger.error({ sessionId: session.id, err: err.message }, "Event stream error");
        }
    }
}
function detectEventType(evt) {
    ...
    if (evt.stepStarted)
        return "step_started";
    if (evt.stepCompleted)
        return "step_completed";
    if (evt.textMessageStart)
        return "text_message_start";
    if (evt.textMessageContent)
        return "text_message_content";
    if (evt.textMessageEnd)
...
async function markCompleted(session) {
    ...
    await getSessionCollection(db).updateOne({ id: session.id }, { $set: { status: "completed", stoppedAt: now, duration } });
...
async function markFailed(session, error) {
    ...
    const now = new Date().toISOString();
    const duration = Date.now() - new Date(session.startedAt).getTime();
    await getSessionCollection(db).updateOne({ id: session.id }, { $set: { status: "failed", stoppedAt: now, duration, error } });
    session.status = "failed";
    session.error = error;
    activeSessions.delete(session.id);
    logger.error({ sessionId: session.id, error, duration }, "Workflow session failed");
}
function buildPrompt(workflowType, options) {
