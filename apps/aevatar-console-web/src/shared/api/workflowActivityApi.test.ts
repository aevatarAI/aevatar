...
describe("workflowActivityApi", () => {
    ...
    const fetchMock = jest
      ...
      .mockResolvedValueOnce(
        jsonResponse({
          ...
          steps: [
            {
              ...
              toolApproval: null,
              usage: { promptTokens: 1, completionTokens: 0, totalTokens: 1, cost: 0 },
            },
          ],
          timeline: [],
          statistics: {
          ...
          usageTotals: { promptTokens: 1, completionTokens: 0, totalTokens: 1, cost: 0 },
        }),
      )
      .mockResolvedValueOnce(
        jsonResponse({
          ...
          nodes: [
            { nodeId: "node-step", nodeType: "WorkflowStep", stepId: "step-failed" },
          ],
          edges: [],
        }),
      );
  ...
  it("preserves authorization status and rejects malformed authoritative identity", async () => {
    const fetchMock = jest
      .fn()
      .mockResolvedValueOnce(
        jsonResponse({ code: "SCOPE_ACCESS_DENIED", message: "Forbidden" }, 403),
      )
      .mockResolvedValueOnce(
        jsonResponse([
          {
