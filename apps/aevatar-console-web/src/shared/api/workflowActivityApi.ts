...
function readNullableStringValue(value: unknown, label: string): string | null {
  return value === null || value === undefined ? null : expectString(value, label);
}

function readNullableBooleanValue(value: unknown, label: string): boolean | null {
...
function decodeUsage(value: unknown, label: string): WorkflowActivityUsageTotals {
...
function decodeDiagnostic(
  ...
  const record = expectRecord(value, label);
  return {
    timestampUtc: readNullableStringValue(record.timestampUtc, `${label}.timestampUtc`),
...
function decodeToolApproval(value: unknown, label: string): WorkflowActivityToolApproval {
  const record = expectRecord(value, label);
  return {
...
function decodeStep(
  ...
  return {
    ...
    success: readNullableBooleanValue(record.success, `${label}.success`),
    durationMs: readNullableNumberValue(record.durationMs, `${label}.durationMs`),
    outputPreview: readString(record, "outputPreview", `${label}.outputPreview`),
...
function decodeToolCall(value: unknown, label: string): WorkflowActivityToolCall {
  const record = expectRecord(value, label);
  return {
    toolName: readString(record, "toolName", `${label}.toolName`),
    callId: readString(record, "callId", `${label}.callId`),
    argumentsJson: readString(record, "argumentsJson", `${label}.argumentsJson`),
...
function decodeDetail(value: unknown): WorkflowActivityRunDetail {
  ...
  return {
    ...
    steps: expectArray(record.steps, "WorkflowActivityRunDetail.steps", decodeStep),
...
function decodeForkReceipt(value: unknown): WorkflowRunForkAcceptedReceipt {
  ...
  if (readBoolean(record, "accepted", "WorkflowRunForkAcceptedReceipt.accepted") !== true) {
...
function compactForkRequest(input: WorkflowRunForkRequest): Record<string, unknown> {
...
export const workflowActivityApi = {
  ...
  forkRun(input: WorkflowRunForkRequest): Promise<WorkflowRunForkAcceptedReceipt> {
    return requestActivityJson(
