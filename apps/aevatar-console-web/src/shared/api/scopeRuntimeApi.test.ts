...
type ServiceBindingCatalogSnapshotReader = typeof scopeRuntimeApi & {
  getServiceBindingCatalogSnapshot: (
    scopeId: string,
    serviceId: string,
  ) => Promise<
    | {
...
const scopeRuntimeApiWithBindingSnapshot =
  scopeRuntimeApi as ServiceBindingCatalogSnapshotReader;
...
describe('scopeRuntimeApi', () => {
  ...
  it('reports a missing binding catalog as not materialized', async () => {
    const fetchMock = jest.fn().mockResolvedValue({
      ok: false,
      status: 404,
    ...
    await expect(
      scopeRuntimeApiWithBindingSnapshot.getServiceBindingCatalogSnapshot(
        'scope-a',
        'svc-alpha',
      ),
    ).resolves.toEqual({ kind: 'not_materialized' });

    expect(fetchMock).toHaveBeenCalledWith(
      '/api/scopes/scope-a/services/svc-alpha/bindings',
      expect.objectContaining({
  ...
  it('loads endpoint invoke contract details for Bind surfaces', async () => {
    const fetchMock = jest.fn().mockResolvedValue({
      ...
      json: async () => ({
        ...
        requestTypeUrl:
          'type.googleapis.com/Aevatar.AI.Abstractions.ChatRequestEvent',
        responseTypeUrl:
          'type.googleapis.com/Aevatar.AI.Abstractions.ChatResponseEvent',
        supportsSse: true,
