...
describe("teamAutomationApi", () => {
  ...
  it("fails closed on an unknown node role", () => {
    const result = authorizationResult();
    result.plan.nyxIdNodeGrants[0].role = "NYX_ID_NODE_ROLE_UNSPECIFIED";

    expect(() => teamAutomationApiDecoders.permissionReview(result)).toThrow(
      "Unknown NyxID node role",
    );
  });

  it("preserves duplicate grants and their order in the review", () => {
    const result = authorizationResult();
    result.plan.nyxIdNodeGrants.push({ ...result.plan.nyxIdNodeGrants[0] });
