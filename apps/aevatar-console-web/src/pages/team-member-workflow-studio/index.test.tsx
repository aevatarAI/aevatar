    ...
    return React.createElement(
      ...
      props.nodes?.map((node) =>
        React.createElement(
          ...
          {
            key: node.id,
            'data-execution-focused': node.data?.executionFocused
              ? 'true'
              : 'false',
...
function mockParseYaml() {
  (studioApi.parseYaml as jest.Mock).mockImplementation(
    async ({ yaml }: { yaml: string }) => {
      const name =
        /(?:^|\n)name:\s*([^\n]+)/.exec(yaml)?.[1]?.trim() || 'Workflow Alpha';
...
function clickMoreAction(
  name: 'Delete selected connection' | 'Delete selected node',
) {
  openMoreActionsMenu();
...
describe('TeamMemberWorkflowStudioPage', () => {
  ...
  it('renders a blank new workflow member editor before backend creation', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Recurring work' }),
    ).toBeDisabled();
    expect(
      screen.getByRole('button', { name: 'Recurring work' }),
    ).toHaveAttribute(
  ...
  it('runs the current unsaved new workflow draft without creating a member first', async () => {
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: 'Add first step' }),
    );
    fireEvent.click(
  ...
  it('opens the node library, inserts a first step, and saves a new linked workflow member', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'node:step:llm_step' }),
    ).toBeTruthy();
    expect(screen.getByText('Unsaved changes')).toBeTruthy();
    const saveButton = screen.getByRole('button', { name: 'Save' });
  ...
  it('does not toast while an accepted workflow draft is still becoming readable', async () => {
    window.history.replaceState(
      {},
      '',
      '/scopes/scope-1/teams/t-alpha/members/new/workflow',
    );
    const { createdWorkflow } = mockNewWorkflowMemberCreateFixtures();
    (studioApi.saveWorkflow as jest.Mock).mockResolvedValue({
      kind: 'accepted',
      receipt: {
        accepted: true,
        ackStage: 'accepted',
        ackedAtUtc: '2026-08-06T00:00:00Z',
        actorId: 'actor-workflow-draft',
        commandId: 'command-workflow-draft',
        expectedVersion: null,
        readiness: {
          message: 'Draft projection is pending.',
          readable: false,
    ...
    let resolveMaterializedDraft:
      | ((value: typeof createdWorkflow) => void)
      | undefined;
    (studioApi.getWorkflowDraftFile as jest.Mock).mockReturnValue(
      new Promise<typeof createdWorkflow>((resolve) => {
    ...
    expect(mockConsoleToast.info).not.toHaveBeenCalled();
    expect(mockConsoleToast.success).not.toHaveBeenCalled();

    await act(async () => {
      resolveMaterializedDraft?.(createdWorkflow);
    });
  ...
  it('uses a generic toast when saving a workflow draft fails', async () => {
    window.history.replaceState(
      {},
      '',
      '/scopes/scope-1/teams/t-alpha/members/new/workflow',
    );
    (studioApi.saveWorkflow as jest.Mock).mockRejectedValue(
      new Error('The backend returned an internal implementation detail.'),
    );
    ...
    expect(mockConsoleToast.error).not.toHaveBeenCalledWith(
      'The backend returned an internal implementation detail.',
    );
  ...
  it('renders an existing workflow member graph from a stable workflow ref', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'node:step:triage' }),
    ).toBeTruthy();
  ...
  it('does not reload runtime display names or fall back to the member id', async () => {
    ...
    (studioApi.getWorkflow as jest.Mock).mockImplementation(
      async (workflowId: string) => {
        throw Object.assign(new Error(`Not found: ${workflowId}`), {
          status: 404,
        });
      },
    );
  ...
  it('reuses the route draft id after refresh even before the member read model exposes the link', async () => {
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: 'Insert Guard node' }),
    );
    await waitFor(() => {
      expect(saveButton).toBeEnabled();
    });
  ...
  it('opens the floating node inspector and applies guided configuration edits into the workflow document', async () => {
    ...
    expect(within(inspector).getAllByText('LLM call').length).toBeGreaterThan(
      0,
    );
  ...
  it('shows a raw configuration error for an unknown node', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Apply raw JSON' }),
    ).toBeDisabled();
    expect(screen.getByRole('button', { name: 'Save' })).toBeDisabled();
  });
  ...
  it('shows backend child step ids as product labels in guided cache node configuration', async () => {
    ...
    fireEvent.click(
      screen.getByRole('button', { name: 'node:step:cache_response' }),
    );
  ...
  it('runs the current workflow draft and shows returned logs', async () => {
    ...
    expect(headerPrimaryActions).toHaveAttribute(
    ...
    expect(headerResponsiveRules).toContain('max-width: min(360px, 100%);');
    ...
    expect(
      within(headerIdentity).getByRole('link', { name: 'Team' }),
    ...
    expect(globalBackButton).toHaveAttribute(
      'data-aevatar-back-button',
      'true',
    );
    expect(within(headerIdentity).getByText('Draft')).toBeTruthy();
    expect(
      within(headerIdentity).getByRole('button', {
    ...
    const publishedRunsButton = within(headerPrimaryActions).getByRole(
      'button',
      {
        name: 'Published runs',
      },
    ...
    expect(
      screen.queryByRole('menuitem', { name: 'Publish member' }),
    ).toBeNull();
    expect(
      screen.queryByRole('menuitem', { name: 'Refresh status' }),
    ).toBeNull();
    expect(
    ...
    expect(
      within(consolePanel).getByRole('radio', { name: 'Nodes' }),
    ).toBeTruthy();
    expect(
      within(consolePanel).getByRole('radio', { name: 'Events' }),
    ).toBeTruthy();
    ...
    expect(logDetails).not.toHaveTextContent('aevatar.step.completed');
    ...
    expect(
      screen.getByRole('button', { name: 'node:step:triage' }),
    ).toHaveAttribute('data-execution-focused', 'true');
    ...
    expect(
      screen.getByRole('button', { name: 'node:step:triage' }),
    ).toHaveAttribute('data-execution-focused', 'false');
    expect(
    ...
    expect(approvalLogRow).not.toHaveTextContent(
      'Need approval before deployment',
    );
    fireEvent.click(approvalLogRow);
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).toHaveTextContent('Need approval before deployment');
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ...
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).not.toHaveTextContent('raw-observation-1');
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).toHaveTextContent('runtime-observer');
    expect(screen.queryByTestId('member-run-summary')).toBeNull();
  ...
  it('renders draft run log cards as SSE frames arrive', async () => {
    ...
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).toHaveTextContent('Run the workflow');
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).toHaveTextContent('No output captured for this node.');
    fireEvent.keyDown(within(consolePanel).getByLabelText('Logs overview'), {
      key: 'ArrowDown',
    });
    ...
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).toHaveTextContent('Workflow complete');
    ...
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).toHaveTextContent('Second pass input');
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).toHaveTextContent('No output captured for this node.');
    ...
    const settledDraftRunPanel =
      await screen.findByLabelText('Draft run panel');
    await waitFor(() => {
      expect(
    ...
    const startSecondRunButton = within(settledDraftRunPanel).getByRole(
      'button',
      {
        name: /Start draft run/,
      },
  ...
  it('runs draft workflow members before they are published', async () => {
    ...
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).toHaveTextContent('No input captured for this node.');
    expect(
      within(consolePanel).getByLabelText('Log details'),
    ).not.toHaveTextContent('Run Workflow Alpha');
    expect(runtimeRunsApi.streamChat).not.toHaveBeenCalled();
  });
  ...
  it('keeps invalid YAML in the buffer and preserves the current graph', async () => {
    ...
    expect(await screen.findByLabelText('Workflow YAML editor')).toHaveValue(
      'not: valid',
    );
    expect(
      screen.getByRole('button', { name: 'Apply to draft' }),
  ...
  it('blocks applying YAML with error-level findings', async () => {
    ...
    expect(
      await screen.findByText(/Unknown field 'unknown_field'\./),
    ).toBeTruthy();
    expect(
      screen.getByRole('button', { name: 'Apply to draft' }),
    ).toBeDisabled();
  ...
  it('requires a fresh explicit-request confirmation before publishing an existing workflow member', async () => {
    ...
    const secondPreviewInput = (studioApi.previewExplicitRequests as jest.Mock)
      .mock.calls[1]?.[0];
    const reconfirmationConfig = (Modal.confirm as jest.Mock).mock
      .calls[1]?.[0];
    await act(async () => {
    ...
    await waitFor(() => {
      ...
      expect(studioApi.bindMemberWorkflow).toHaveBeenCalledWith({
        ...
        workflowYamls: [
          expect.stringContaining('name: Workflow Alpha Published'),
        ],
    ...
    await waitFor(() => {
      expect(screen.getByText('Published')).toBeTruthy();
      expect(
        screen.getByTitle(/Published member workflow is serviceable/),
      ).toBeTruthy();
    });
  ...
  it('reports rejected publish binding without introducing activation language', async () => {
    ...
    await waitFor(() => {
      ...
      expect(
        screen.getByTitle(/Binding run was rejected by the member authority/),
      ).toBeTruthy();
    });
  ...
  it('saves and rebinds a changed published workflow member through save-and-bind', async () => {
    ...
    const previewInput = (studioApi.previewExplicitRequests as jest.Mock).mock
      .calls[0]?.[0];
    const saveAndBindInput = (studioApi.saveAndBindWorkflow as jest.Mock).mock
      .calls[0]?.[0];
