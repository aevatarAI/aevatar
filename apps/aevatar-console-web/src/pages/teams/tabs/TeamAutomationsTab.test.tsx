...
function renderTab(routeMemberId = '', members = [member]) {
...
describe('TeamAutomationsTab canonical member authority', () => {
  ...
  it('keeps member selection inside the create form when multiple members are eligible', async () => {
    ...
    expect(
      screen.queryByRole('combobox', { name: 'Automation member' }),
    ...
    expect(
      within(form).getByRole('combobox', { name: 'Automation member' }),
    ).toBeEnabled();
  ...
  it('stays on the Team collection after creating for an explicitly selected member', async () => {
    ...
    await waitFor(() =>
      expect(teamAutomationApi.create).toHaveBeenCalledTimes(1),
    );
  ...
  it('attributes Team collection rows to each automation member', async () => {
    ...
    const plannerRow = await screen.findByRole('article', {
      name: 'Planner review',
    });
  ...
  it('uses the row member authority for Team collection actions', async () => {
    ...
    fireEvent.click(
      within(reviewerRow).getByRole('button', { name: 'Run now' }),
    );
  ...
  it('opens the complete dev form directly from the Team shell', async () => {
    ...
    expect(
      screen.getByRole('button', { name: 'Preview next runs' }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: 'Create automation' }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole('button', { name: 'Review authorization' }),
    ).not.toBeInTheDocument();
    expect(history.push).not.toHaveBeenCalled();
    expect(teamAutomationApi.listAll).toHaveBeenCalledWith(
      { scopeId: 'scope-alpha', teamId: 'team-alpha' },
  ...
  it('uses a generic schedule preview failure toast without exposing API details', async () => {
    const rawBackendError =
      'POST /internal/schedule-preview returned trace-correlation-secret';
    (previewScheduledDispatch as jest.Mock).mockRejectedValue(
      new Error(rawBackendError),
    );
    renderTab('m-alpha');
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Preview next runs' }));
    ...
    expect(mockConsoleToast.error).not.toHaveBeenCalledWith(rawBackendError);
    expect(screen.queryByText(rawBackendError)).not.toBeInTheDocument();
  });
  ...
  it('loads only the exact canonical member collection', async () => {
    ...
    expect(
      screen.getByRole('heading', { name: 'Give a member recurring work' }),
    ).toBeInTheDocument();
    expect(
  ...
  it('derives Upcoming from the exact member read model', async () => {
    ...
    expect(
      await screen.findByText('Planner recurring work'),
    ).toBeInTheDocument();
    expect(teamAutomationApi.listAll).toHaveBeenCalledWith(
      { scopeId: 'scope-alpha', teamId: 'team-alpha', memberId: 'm-alpha' },
      { take: 200 },
  ...
  it('does not query when the path member is outside the current Team', async () => {
    ...
    expect(
      await screen.findByText('Member unavailable for automation'),
    ).toBeInTheDocument();
  ...
  it('shows a no-service authorization review and requires confirmation before creating', async () => {
    ...
    expect(
      within(reviewDialog).queryByText(/gateway\s*\//),
    ).not.toBeInTheDocument();
  ...
  it('reports authorization preflight failures through the shared toast without changing the modal layout', async () => {
    ...
    expect(mockConsoleToast.error).not.toHaveBeenCalledWith(rawCatalogFailure);
  ...
  it('keeps typed preflight failure details out of the shared toast', async () => {
    const rawBackendError =
      'POST /internal/automations returned trace-correlation-secret';
    (teamAutomationApi.preflightCreate as jest.Mock).mockRejectedValue(
  ...
  it('keeps typed preflight plan changes in the review flow', async () => {
    ...
    expect(
      await screen.findByText('Authorization plan changed'),
    ).toBeInTheDocument();
    expect(
  ...
  it('shows an accepted create while the authoritative row is unavailable', async () => {
    ...
    const acceptedRow = await screen.findByRole('article', {
      name: 'Daily review',
    });
    expect(
  ...
  it('does not poll for an existing pending automation', async () => {
    ...
    try {
      (teamAutomationApi.listAll as jest.Mock).mockResolvedValue({
        items: [
          automationView({ authorizationStatus: 'provisioning_pending' }),
        ],
        nextCursor: null,
        totalCount: 1,
      });
  ...
  it('stops create polling after six seconds and manual refresh stays one-shot', async () => {
    ...
    try {
      ...
      expect(
        screen.getByRole('article', { name: 'Daily review' }),
      ).toBeInTheDocument();

      const callsAtAcceptance = (teamAutomationApi.listAll as jest.Mock).mock
        .calls.length;
      await act(async () => {
        await jest.advanceTimersByTimeAsync(6_000);
      });
      const callsAtDeadline = (teamAutomationApi.listAll as jest.Mock).mock
        .calls.length;
      ...
      expect(teamAutomationApi.listAll).toHaveBeenCalledTimes(
        callsAtDeadline + 1,
      );
  ...
  it('recovers a confirmed create when the fresh binding is missing', async () => {
    ...
    const loginWithRedirect = (NyxIDAuthClient as jest.Mock).mock.results[0]
      .value.loginWithRedirect as jest.Mock;
  ...
  it('uses a generic update failure toast without exposing API details', async () => {
    const rawBackendError =
      'PATCH /internal/automations/sch-alpha returned trace-correlation-secret';
    (teamAutomationApi.listAll as jest.Mock).mockResolvedValue({
      items: [automationView()],
      nextCursor: null,
      totalCount: 1,
  ...
  it('allows run now while an active automation is paused', async () => {
    ...
    expect(mockConsoleToast.info).toHaveBeenCalledWith('Run request accepted');
    expect(mockConsoleToast.success).not.toHaveBeenCalled();
    expect(screen.getByRole('button', { name: 'Resume' })).toBeInTheDocument();
  });

  it('uses a generic automation action failure toast without exposing API details', async () => {
    const rawBackendError =
      'POST /internal/automations/sch-alpha/run returned trace-correlation-secret';
    (teamAutomationApi.listAll as jest.Mock).mockResolvedValue({
      items: [automationView()],
      nextCursor: null,
      totalCount: 1,
    });
    (teamAutomationApi.runNow as jest.Mock).mockRejectedValue(
      new Error(rawBackendError),
    );
    renderTab('m-alpha');
  ...
  it('reconnects NyxID for revocation retry without persisting an action ledger', async () => {
    ...
    fireEvent.click(
      await screen.findByRole('button', { name: 'Retry revocation' }),
    );
  ...
  it('keeps delete unavailable while replacement or revocation is pending', async () => {
    ...
    expect(
      await screen.findByText('Replacing authorization'),
    ).toBeInTheDocument();
