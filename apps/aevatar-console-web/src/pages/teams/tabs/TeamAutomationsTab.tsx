...
import { buildTeamMemberAutomationsHref } from '@/shared/navigation/teamRoutes';
import {
...
import { DetailPill, FactLine } from '../components/TeamDetailPrimitives';
import {
...
function describeDraftCadence(
  ...
  > = {
    '0 9 * * 1-5': {
      ...
      summary: copy('teams.automations.cron.weekdays', 'Weekdays · {time}', {
        time: '09:00',
      }),
    },
    '0 9 * * *': {
      ...
      summary: copy('teams.automations.cron.daily', 'Daily · {time}', {
        time: '09:00',
      }),
    },
    '0 9 * * 1': {
      ...
      summary: copy('teams.automations.cron.weekly', '{weekday} · {time}', {
        weekday: copy('teams.automations.weekdays.monday', 'Monday'),
        time: '09:00',
      }),
    },
    '0 * * * *': {
      ...
      summary: copy('teams.automations.cron.hourly', 'Hourly · :{minute}', {
        minute: '00',
      }),
    },
  };
...
function requiresBindingRecovery(
  error: unknown,
): error is TeamAutomationApiError {
  return (
...
async function retryTypedPreflight<T>(operation: () => Promise<T>): Promise<T> {
  ...
  for (let attempt = 0; ; attempt += 1) {
    try {
      ...
      const code =
        error instanceof TeamAutomationApiError ? error.code : undefined;
      if (
        !code ||
        !retryablePreflightCodes.has(code) ||
...
const TeamAutomationsTab: React.FC<Props> = ({
  ...
  const [authorizationFlow, setAuthorizationFlow] =
    React.useState<AuthorizationFlow>({ state: 'idle' });
  ...
  const flowBusy =
    ...
    authorizationFlow.state === 'submitting';
  const review =
    authorizationFlow.state === 'reviewing' ||
    authorizationFlow.state === 'submitting'
  ...
  const redirectToBindingRecovery = React.useCallback(
    async (
      ...
      if (typeof window === 'undefined') {
        throw new Error(
          'NyxID authorization recovery requires a browser environment.',
        );
      }
      if ('draft' in target) {
        saveTeamAutomationAuthorizationDraft(
  ...
  const beginPreflight = React.useCallback(
    async (
      nextDraft: TeamAutomationCreateDraft,
      mode: AuthorizationMode,
      scheduleId?: string,
    ) => {
      ...
      try {
        const review = await retryTypedPreflight(() =>
          teamAutomationApi.preflightCreate(nextDraft),
        );
    ...
    [copy, redirectToBindingRecovery, toast],
  );

  React.useEffect(() => {
    if (!canQuery || !routeMemberAuthority || typeof window === 'undefined')
      return;
  ...
  const validateDraft = (): TeamAutomationCreateDraft | null => {
    ...
    if (!next.cronExpression) {
      toast.error(
        copy(
          'teams.automations.messages.cronRequired',
          'Enter a cron expression first.',
        ),
      );
      return null;
    }
  ...
  const reviewReauthorization = (view: TeamAutomationView) => {
    ...
    setFormOpen(true);
    void beginPreflight(
      authorizationDraft(view, nextDraft),
      'reauthorize',
      view.scheduleId,
    );
  };
    ...
    try {
      ...
      setMutationObservation({
        ...
        baselineStateVersion:
          mode === 'reauthorize' ? (editing?.stateVersion ?? 0) : 0,
  ...
  const updateAutomation = async () => {
    ...
    try {
      const receipt = await teamAutomationApi.update(
        exactAutomationRoute(editing),
        editing.scheduleId,
        {
      ...
      toast.info(
        copy(
          'teams.automations.messages.updateAccepted',
    ...
    try {
      ...
      if (action === 'retryRevocation') {
        ...
        toast.info(
          action === 'runNow'
            ? copy(
                'teams.automations.messages.runAccepted',
                'Run request accepted',
              )
            : action === 'delete'
              ? copy(
                  'teams.automations.messages.deleteAccepted',
                  'Delete request accepted',
                )
              : action === 'pause'
                ? copy(
                    'teams.automations.messages.pauseAccepted',
                    'Pause request accepted',
                  )
                : copy(
                    'teams.automations.messages.resumeAccepted',
      ...
      if (requiresBindingRecovery(error)) {
        try {
          await redirectToBindingRecovery({
            route: exactAutomationRoute(view),
          });
          return;
        } catch {
          toast.error(
  ...
  const renderStatusPill = (view: TeamAutomationView) => {
    ...
    return (
      <span
        ...
        role="status"
        style={{ ...automationStatusBadgeStyle, ...statusStyle }}
      >
        <span
          aria-hidden="true"
  ...
  const renderSummaryTile = ({
    ...
    return (
      <div style={{ ...automationSummaryTileStyle, ...toneStyle }}>
        <span
          style={{ alignItems: 'center', display: 'inline-flex', fontSize: 16 }}
        >
          {icon}
        </span>
      ...
      >
        ...
        {active
          ? renderAutomationActionButton({
              ...
              disabled: busy,
              onClick: () =>
                void runAction(view, view.enabled ? 'pause' : 'resume'),
            })
          : null}
        {canReauthorize
          ? renderAutomationActionButton({
              ...
              label: copy(
                'teams.automations.actions.reauthorize',
        ...
        {view.revocationPending
          ? renderAutomationActionButton({
              icon: <ReloadOutlined />,
              label: copy(
                'teams.automations.actions.retryRevocation',
                'Retry revocation',
              ),
              disabled: busy,
        ...
        {canDelete
          ? renderAutomationActionButton({
              ...
              onClick: () =>
                Modal.confirm({
                  title: copy(
                    'teams.automations.delete.title',
                    'Delete automation?',
                  ),
                  content: copy(
      ...
      >
        ...
        >
          <div style={automationNameLineStyle}>
            ...
            <Typography.Text ellipsis strong>
              {view.displayName ||
                copy('teams.automations.untitled', 'Untitled automation')}
            </Typography.Text>
          </div>
          <FactLine
            secondary
            text={copy(
              'teams.automations.row.target',
          ...
          {view.revocationPending ? (
            <Space size={8} wrap>
              <Typography.Text style={{ fontSize: 12 }} type="secondary">
                {copy('teams.automations.revocation.nyxId', 'NyxID: {status}', {
                  status: view.nyxIdRevocationStatus,
                })}
              </Typography.Text>
              <Typography.Text style={{ fontSize: 12 }} type="secondary">
                {copy('teams.automations.revocation.vault', 'Vault: {status}', {
                  status: view.vaultRevocationStatus,
                })}
              </Typography.Text>
            </Space>
          ) : null}
        ...
        >
          <FactLine
            ...
            tooltipText={`${cadence.detail} · ${view.cronExpression}`}
          />
          <Typography.Text style={{ fontSize: 12 }} type="secondary">
            {view.nextFireAt
  ...
  const cronPresets = [
    {
      label: copy(
        'teams.automations.form.preset.weekdaysMorning',
        'Weekdays · 09:00',
      ),
      value: 'weekdays-0900',
      cronExpression: '0 9 * * 1-5',
    ...
    {
      label: copy(
        'teams.automations.form.preset.dailyMorning',
        'Daily · 09:00',
      ),
      value: 'daily-0900',
      cronExpression: '0 9 * * *',
    ...
    {
      label: copy(
        'teams.automations.form.preset.weeklyMonday',
        'Monday · 09:00',
      ),
    ...
    {
      label: copy('teams.automations.form.preset.hourly', 'Hourly'),
      value: 'hourly',
      cronExpression: '0 * * * *',
    },
    {
      label: copy('teams.automations.form.preset.custom', 'Custom cron'),
      value: 'custom',
      cronExpression: '',
    },
  ] as const;
  const activePreset =
    cronPresets.find(
      (preset) =>
        preset.cronExpression === draft.cronExpression && preset.cronExpression,
    )?.value ?? 'custom';
  ...
  const formCronValidationMessage = !normalizedDraftCron
    ...
    : normalizedDraftCron.split(' ').length !== 5
      ...
      : '';
  const formCadence = describeDraftCadence(
      ...
      >
        <div style={{ display: 'grid', gap: 5, minWidth: 0 }}>
          ...
          <FactLine
            ...
            secondary
            text={
              expectedDraft.prompt ||
              copy('teams.automations.row.noPrompt', 'No recurring prompt')
            }
          />
        </div>
        <div style={{ display: 'grid', gap: 5, minWidth: 0 }}>
  ...
  return (
    <>
      ...
      >
        <AevatarPanel>
          <div
            className="team-automations-panel-header"
            style={panelHeaderStyle}
          >
            <div style={titleGroupStyle}>
              <Typography.Title
                id="team-automations-title"
                level={3}
                style={titleStyle}
              >
            ...
            <Space>
              ...
              >
                <Button
                  aria-label={copy(
                    'teams.automations.actions.refresh',
                    'Refresh',
                  )}
                  icon={<ReloadOutlined />}
          ...
          >
            ...
            {automationsQuery.isError ? (
              <AevatarInspectorEmpty
                compact
                title={copy(
                  'teams.automations.error.title',
                  'Automations could not load',
                )}
                description={copy(
            ...
            {mutationObservationExpired ? (
              <Alert
                ...
                message={copy(
                  'teams.automations.pending.title',
                  'Still pending',
                )}
                showIcon
                type="warning"
            ...
            {automationItems.length || acceptedCreateObservation ? (
              <div style={commitmentGridStyle}>
                <div
                  className="team-automation-summary"
                  style={automationSummaryGridStyle}
                >
                  {renderSummaryTile({
                  ...
                  {renderSummaryTile({
                    ...
                    label: copy(
                      'teams.automations.summary.needsAttention',
                      'Need attention',
                    ),
                    tone: 'error',
            ...
            <div style={{ display: 'grid', gap: 14 }}>
              <div style={{ display: 'grid', gap: 4 }}>
                <Typography.Title level={3} style={titleStyle}>
                  {copy(
                    'teams.automations.createPanel.title',
                    'Give a member recurring work',
                  )}
                </Typography.Title>
              ...
              {routeMember ? (
                ...
                >
                  ...
                  <DetailPill
                    compact
                    style={routeMember.lifecycleStyle}
                    text={routeMember.lifecycleLabel}
                  />
                </div>
                ...
                <AevatarInspectorEmpty
                  compact
                  title={copy(
                    'teams.automations.noPublishedMember.title',
                    'Publish a member first',
                  )}
                  description={copy(
              ...
              >
                {copy(
                  'teams.automations.actions.addRecurringWork',
                  'Add recurring work',
                )}
              </Button>
            </div>
          ...
          <AevatarPanel>
            <div style={{ display: 'grid', gap: 12 }}>
              ...
              {upcomingAutomations.length ? (
                    ...
                    <div style={{ display: 'grid', gap: 2, minWidth: 0 }}>
                      ...
                      >
                        {copy(
                          ...
                          {
                            memberName:
                              membersById.get(trim(automation.memberId))
                                ?.name ??
                              copy(
                                'teams.automations.columns.member',
                                'Member',
                              ),
                          },
                        )}
                ...
                <Typography.Text type="secondary">
                  {copy(
                    'teams.automations.upcoming.empty',
                    'No upcoming runs are visible yet.',
                  )}
                </Typography.Text>
              )}
            </div>
          ...
          {unavailableMembers.length ? (
            <AevatarPanel>
              <div style={{ display: 'grid', gap: 10 }}>
                <Typography.Title level={3} style={titleStyle}>
                  {copy(
                    'teams.automations.unavailable.title',
          ...
          footer={
            review ? (
              <Space>
                <Button
                  disabled={flowBusy}
                  onClick={() => setAuthorizationFlow({ state: 'idle' })}
                >
                  {copy('teams.automations.authorization.back', 'Back')}
                </Button>
                <Button
                  loading={flowBusy}
                  onClick={() => void submitAuthorization()}
                  type="primary"
                >
                  {copy(
                    'teams.automations.authorization.confirm',
            ...
            <div style={{ display: 'grid', gap: 16 }}>
              ...
              >
                <div style={{ display: 'grid', gap: 2 }}>
                  <Typography.Text strong>
                    {copy(
                      'teams.automations.form.section.target',
                      '1. Target member',
                    )}
                  </Typography.Text>
                  <Typography.Text
                    id="team-automation-form-description"
                    style={{ fontSize: 12 }}
                    type="secondary"
                  >
                    {copy(
                ...
                <div style={modalFieldStyle}>
                  <Typography.Text strong>
                    {copy('teams.automations.form.member', 'Member')}
                  </Typography.Text>
                  <Select
                    aria-label={copy(
                      'teams.automations.form.memberAria',
                      'Automation member',
                    )}
                    disabled={flowBusy || formMode === 'edit'}
                    onChange={(memberId) =>
                      setDraft((current) => ({ ...current, memberId }))
                    }
                    options={members
              ...
              >
                <div style={{ display: 'grid', gap: 2 }}>
                  <Typography.Text strong>
                    {copy(
                      'teams.automations.form.section.work',
                      '2. Work to run',
                    )}
                  </Typography.Text>
                  <Typography.Text style={{ fontSize: 12 }} type="secondary">
                ...
                <div style={modalFieldStyle}>
                  <Typography.Text strong>
                    {copy('teams.automations.form.displayName', 'Name')}
                  </Typography.Text>
                  <Input
                    aria-label={copy(
                      'teams.automations.form.displayNameAria',
                      'Automation name',
                    )}
                    disabled={flowBusy}
                    onChange={(event) =>
                    ...
                    placeholder={copy(
                      'teams.automations.form.displayNamePlaceholder',
                ...
                <div style={modalFieldStyle}>
                  <Typography.Text strong>
                    {copy(
                      'teams.automations.form.prompt',
                      'Recurring prompt (optional)',
                    )}
                  </Typography.Text>
                  <Input.TextArea
                    aria-label={copy(
                      'teams.automations.form.promptAria',
                      'Recurring prompt',
                    )}
              ...
              >
                <div style={{ display: 'grid', gap: 2 }}>
                  <Typography.Text strong>
                    {copy(
                      'teams.automations.form.section.schedule',
                      '3. Schedule',
                    )}
                  </Typography.Text>
                  <Typography.Text style={{ fontSize: 12 }} type="secondary">
                ...
                <div style={modalFieldStyle}>
                  <Typography.Text strong>
                    {copy('teams.automations.form.cadence', 'Cadence')}
                  </Typography.Text>
                  <Segmented
                    aria-label={copy(
                      'teams.automations.form.cadenceAria',
                      'Automation cadence',
                    )}
                    block
                    disabled={flowBusy}
                    onChange={(presetValue) => {
                      const preset = cronPresets.find(
                        (candidate) => candidate.value === String(presetValue),
                      );
                ...
                >
                  <div style={modalFieldStyle}>
                    ...
                    <Input
                      aria-label={copy(
                        'teams.automations.form.cronAria',
                        'Cron expression',
                      )}
                      disabled={flowBusy}
                      onChange={(event) =>
                  ...
                  <div style={modalFieldStyle}>
                    ...
                    <Input
                      aria-label={copy(
                        'teams.automations.form.timezoneAria',
                        'Timezone',
                      )}
                      disabled={flowBusy}
                      onChange={(event) =>
                ...
                >
                  ...
                  >
                    <Typography.Text style={{ fontSize: 12 }} type="secondary">
                      {copy(
                        'teams.automations.form.scheduleReadsAs',
                        'Schedule reads as',
                      )}
                    </Typography.Text>
                    <Typography.Text strong>
                  ...
                  <div style={enabledFieldStyle}>
                    <Typography.Text strong>
                      {copy('teams.automations.form.enabled', 'Enabled')}
                    </Typography.Text>
                    <Switch
                      aria-label={copy(
                        'teams.automations.form.enabled',
                        'Enabled',
                      )}
                ...
                >
                  <Space align="center" wrap>
                    ...
                    >
                      {copy(
                        'teams.automations.form.preview',
                        'Preview next runs',
                      )}
                    </Button>
                    <Typography.Text type="secondary">
                ...
                <Alert
                  action={
                    <Button
                      onClick={() =>
                        void beginPreflight(
                          authorizationFlow.draft,
                          authorizationFlow.mode,
                          authorizationFlow.scheduleId,
                        )
                      }
                    >
                      {copy(
                        'teams.automations.authorization.reviewAgain',
                        'Review again',
                      )}
                    </Button>
                  }
                  ...
                  message={copy(
                    'teams.automations.planChanged.title',
                    'Authorization plan changed',
                  )}
                  showIcon
                  type="warning"
                />
