...
const PROMPT_HISTORY_STORAGE_KEY =
  'aevatar-console-playground-prompt-history';
...
function mockBuildWorkflowYaml(document: typeof mockWorkflowDocument): string {
  ...
  return [`name: ${document.name}`, 'roles:', ...roleLines, 'steps:', ...stepLines, ''].join(
    '\n',
  );
}
...
jest.mock('@/shared/studio/api', () => ({
  studioApi: {
    ...
    testRuntimeConnection: jest.fn(async (input: { runtimeBaseUrl?: string }) => ({
      runtimeBaseUrl: input.runtimeBaseUrl || mockSettings.runtimeBaseUrl,
      reachable: true,
      checkedUrl: `${input.runtimeBaseUrl || mockSettings.runtimeBaseUrl}/health`,
      statusCode: 200,
...
describe('StudioPage', () => {
  ...
  it('starts a blank draft when the Studio route requests draft mode', async () => {
    ...
    expect((await screen.findByLabelText('Workflow name')) as HTMLInputElement).toHaveValue(
      'draft',
    );
    expect((await screen.findByLabelText('Workflow YAML')) as HTMLTextAreaElement).toHaveValue(
      'name: draft\nsteps: []\n',
    );
  });
  ...
  it('hydrates the Studio execution prompt from the route query', async () => {
    ...
    await waitFor(() => {
      expect(studioApi.getTemplateWorkflow).toHaveBeenCalledWith('published-demo');
    });
  ...
  it('stops the selected Studio execution from the execution view', async () => {
    renderStudioPage('/studio?workflow=workflow-1&tab=executions&execution=execution-1');
  ...
  it('generates workflow YAML with Studio AI and applies it to the draft', async () => {
    ...
    fireEvent.change(await screen.findByLabelText('Studio AI workflow prompt'), {
      target: {
  ...
  it('saves edited connector catalog entries through the Studio API', async () => {
    ...
    expect(await screen.findByDisplayValue('https://example.test')).toBeTruthy();
