...
const mockWorkflowActivityApi = jest.requireMock("@/shared/api/workflowActivityApi")
  ...
  return {
    ...
    steps: [
      ...
      },
      {
        ...
        toolApproval: null,
        usage: { promptTokens: 4, completionTokens: 8, totalTokens: 12, cost: 0.02 },
      },
    ],
    timeline: [],
    statistics: {
    ...
    usageTotals: { promptTokens: 4, completionTokens: 8, totalTokens: 12, cost: 0.02 },
  };
}
...
describe("Workflow Activity vNext run detail recovery", () => {
  ...
  it("confirms the immutable source and explicit step before submitting a retry", async () => {
    renderWithQueryClient(<RunDetailPage runId="run-source-alpha" scopeId="scope-alpha" />);
    ...
    expect(screen.getByRole("dialog", { name: "Confirm new run" })).toBeInTheDocument();
    expect(screen.getAllByText("run-source-alpha").length).toBeGreaterThan(0);
    expect(screen.getAllByText("step-failed").length).toBeGreaterThan(0);
    expect(screen.getAllByText("Investigate checkout latency").length).toBeGreaterThan(0);
    ...
    expect(await screen.findByText("actor-new-alpha")).toBeInTheDocument();
    expect(screen.getByText("command-alpha")).toBeInTheDocument();
    expect(screen.getByText("correlation-alpha")).toBeInTheDocument();
    expect(screen.getByText("/api/workflow/runs/status/command-alpha")).toBeInTheDocument();
    expect(screen.getAllByText("run-source-alpha").length).toBeGreaterThan(0);
  });

  it("keeps committed detail visible and disables run again when graph evidence fails", async () => {
    mockWorkflowActivityApi.getRunGraph.mockRejectedValue(new Error("graph offline"));

    renderWithQueryClient(<RunDetailPage runId="run-source-alpha" scopeId="scope-alpha" />);

    expect(await screen.findByText("Incident review")).toBeInTheDocument();
    fireEvent.click(screen.getByRole("tab", { name: "Graph" }));
    expect(await screen.findByText("Run graph unavailable; committed detail remains visible")).toBeInTheDocument();
  ...
  it("renders returned diagnostics, request parameters, statistics, and usage facts", async () => {
    ...
    expect(await screen.findByText(/"prompt": "Investigate"/)).toBeInTheDocument();
    fireEvent.click(screen.getByRole("tab", { name: "Diagnostics" }));
    expect(screen.getByText("APPROVAL_TIMEOUT")).toBeInTheDocument();
    expect(screen.getByText("Approval did not arrive before the deadline")).toBeInTheDocument();
