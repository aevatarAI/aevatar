...
const WorkflowActivityVNextPage: React.FC = () => {
  ...
  const scopeMatch = /^\/scopes\/([^/]+)\/workflow-activity-vnext(?:\/|$)/.exec(pathname);
  ...
  if (workflowMatch) {
    return <WorkflowEditorPage scopeId={scopeId} workflowId={decodeURIComponent(workflowMatch[1])} />;
  }

  const runMatch = /\/activity\/([^/]+)$/.exec(pathname);
  if (runMatch) {
    return <RunDetailPage runId={decodeURIComponent(runMatch[1])} scopeId={scopeId} />;
  }
  ...
  return (
    <WorkflowActivityVNextShell
      activeSection={pathname.includes("/activity") ? "activity" : pathname.includes("/settings") ? "settings" : "workflows"}
      description={t("workflowActivityVNext.unavailable.description", "This vNext surface is not available yet.")}
      scopeId={scopeId}
      title={t("workflowActivityVNext.unavailable.title", "Unavailable")}
    >
      <div className="wa-vnext__state" role="status">
        <p>{t("workflowActivityVNext.unavailable.body", "The requested capability is not currently available from the frontend route.")}</p>
      </div>
    </WorkflowActivityVNextShell>
  );
};
