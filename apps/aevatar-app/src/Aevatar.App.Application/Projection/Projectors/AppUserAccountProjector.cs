...
public sealed class AppUserAccountProjector : IProjectionProjector<AppProjectionContext, object?>
{
    private readonly IProjectionDocumentStore<AppUserAccountReadModel, string> _store;
    private readonly IReadOnlyDictionary<string, IReadOnlyList<IProjectionEventReducer<AppUserAccountReadModel, AppProjectionContext>>> _reducersByType;

    public AppUserAccountProjector(
        ...
        IEnumerable<IProjectionEventReducer<AppUserAccountReadModel, AppProjectionContext>> reducers)
    {
        ...
        _reducersByType = reducers
            ...
            .ToDictionary(
                x => x.Key,
                x => (IReadOnlyList<IProjectionEventReducer<AppUserAccountReadModel, AppProjectionContext>>)x.ToList(),
                StringComparer.Ordinal);
    }

    private const string ActorPrefix = "useraccount:";
    ...
    public ValueTask InitializeAsync(AppProjectionContext context, CancellationToken ct = default)
    {
        ...
        var model = new AppUserAccountReadModel { Id = context.ActorId };
