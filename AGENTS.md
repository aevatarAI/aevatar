...
## 项目结构与模块组织
- `src/`：生产代码，按能力与分层组织（`Aevatar.Foundation.*`、`Aevatar.Workflows.Core`、`Aevatar.AI.*`、`Aevatar.CQRS.Projections*`、`Aevatar.Hosts.*`）。
...
- `dotnet test test/Aevatar.Hosts.Api.Tests/Aevatar.Hosts.Api.Tests.csproj --collect:"XPlat Code Coverage"`：单项目覆盖率。
- `dotnet run --project src/Aevatar.Hosts.Api`：启动 API（`/api/chat`、`/api/ws/chat`）。
...
- CI 守卫：禁止 `GetAwaiter().GetResult()`；禁止 `TypeUrl.Contains(...)` 字符串路由；禁止 `Aevatar.Workflows.Core` 依赖 `Aevatar.AI.Core`。
