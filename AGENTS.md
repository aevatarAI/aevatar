...
## 架构评分口径（强制）
- 当用户要求“架构方案/需求文档打分”时，必须采用“方案评分模式”：仅评估架构设计质量与文档完整性，不因“代码未实施、测试未补齐、CI 未接入、守卫脚本 skip”扣分。
- 当用户明确要求“代码实现审计/落地评分”时，才采用“实施评分模式”：可基于构建、测试、门禁与运行结果扣分。
- 方案评分模式下，可将实施状态记录为“后续风险/改进项”，但不得作为扣分项。
