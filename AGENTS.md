...
- 禁止绕道与自我豁免：不得通过临时脚本、`Makefile`、手工 CI job/重启入口、远程 exec、sandbox、非标准部署操作或其他间接工具链绕过上述生产 pod 变更边界；任务紧急或会话内口头“这次可以重启”均不构成豁免。
- 标准源码交付不属于上述绕道：允许按本仓库既定流程 commit/push 到约定分支，并由既有 CI/CD 自动完成构建与部署。该许可不允许 agent 在流水线之外直接操作生产 pod，也不允许手工重启、改写或绕过流水线门禁。
